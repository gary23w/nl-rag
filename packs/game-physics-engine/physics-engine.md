...
source: https://en.wikipedia.org/wiki/Physics_engine
domain: game-physics-engine
...
Tire manufacturers use physics simulations to examine how new tire tread types will perform under wet and dry conditions, using new tire materials of varying flexibility and under different levels of weight loading. The simulations optimize tire operations, material selection, costs, and enhance time efficiency.
...
| Open source Advanced Simulation Library - open source hardware accelerated multiphysics simulation software Box2D Bullet Chipmunk physics engine - 2D physics engine Jolt Physics - Horizon Forbidden West physics engine Newton Game Dynamics Open Dynamics Engine PAL (Physics Abstraction Layer) - A uniform API that supports multiple physics engines PhysX (Formerly owned by Ageia) - Derivative technology from NovodeX physics from NovodeX AG Project Chrono - An open source simulation engine for multi-physics applications. Rapier - a 2D and 3D physics engine written in Rust Siconos Modeling and the simulation of mechanical systems with contact, impact and Coulomb's friction SOFA (Simulation Open Framework Architecture) Tokamak Physics Public domain Phyz (Dax Phyz) - 2.5D physics simulator/editor. Closed source/limited free distribution Digital Molecular Matter Havok (Derivative technology from Ipion Virtual Physics) Chaos by Epic Games Vortex by CMLabs Simulations (Derivative MathEngine Karma physics technology in part) AGX Multiphysics by Algoryx Simulation AB Algodoo by Algoryx Simulation AB Rubikon by Valve Corporation CryPhysics by CryTek Euphoria by NaturalMotion - A real-time procedural animation dynamic blending system with realistic ragdoll physics simulation. Natively integrated into Rockstar's proprietary Rockstar Advanced Game Engine Engine. Discontinued Physics Engines MathEngine Karma by MathEngine PLC - Originally marketed as MathEngine Dynamics Toolkit in 2000, rebranded to Karma in March 2001 after the Dot-com bubble burst in 2000. Natively integrated into Unreal Engine 2. Acquired by Criterion Software in 2002 (Integrated into Renderware; Rebranded as RenderWare Physics), dissolved into Vortex. Ipion Virtual Physics by Ipion Software - Developed in late 1990s, acquired and absorbed into Havok in June 2000. Natively integrated into Valve's Source Engine (albeit with heavy modifications with some additional Havok code for constraints and MOPP (Memory Optimized Partial Polytype). NovodeX Physics by NovodeX AG - Natively integrated into Unreal Engine 3. Direct precursor to PhysX, acquired by Ageia in 2004. Dissolved. Meqon - A physics engine from Meqon Research AB acquired by Ageia on September 1, 2005 |
|---|

- idPhysics by id Software - Natively integrated into id Tech 4. Discontinued since id Tech 7.
- Endorphin by NaturalMotion - An interactive physics engine utilizing ragdoll physics, and rigid-body dynamics. Not to be confused with Euphoria. Discontinued in 2014.
