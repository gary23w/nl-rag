...
. The set of all words is denoted by
...
of the automaton on an input

$a_{1}a_{2}...a_{n}\in \Sigma ^{*}$
...
has been read, leaving the machine in the
...
${\overline {\delta }}:Q\times \Sigma ^{*}\to Q$
...
, an automaton may be considered as an

acceptor

, replacing the output alphabet and function
...
- $q_{0}\in Q$ , a designated *start state*, and
...
$w=a_{1}a_{2}...a_{n}\in \Sigma ^{*}$

is an

accepting word

for the automaton if

${\overline {\delta }}(q_{0},w)\in F$
...
$L=\{w\in \Sigma ^{*}\ |\ {\overline {\delta }}(q_{0},w)\in F\}$
...
the recognizable languages are
