...
source: https://en.wikipedia.org/wiki/Network_monitoring
...
Network monitoring plays a central role in meeting regulatory requirements across multiple industries.

In healthcare, the Health Insurance Portability and Accountability Act (HIPAA) Security Rule requires covered entities to implement hardware, software, and procedural mechanisms to record and examine activity in information systems that contain or use electronic protected health information (45 CFR 164.312(b)). The December 2024 Notice of proposed rulemaking (NPRM) to overhaul the HIPAA Security Rule would mandate continuous monitoring of all information systems containing ePHI, including deployment of technology assets that can detect and respond to threats in real time.

The Payment Card Industry Data Security Standard (PCI DSS) requires organizations to track and monitor all access to network resources and cardholder data (Requirement 10), including the use of network monitoring tools to detect unauthorized wireless access points. For U.S. federal agencies, NIST Special Publication 800-53 mandates continuous monitoring under the CA-7 control family as part of the Risk Management Framework.
