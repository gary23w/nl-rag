---
title: "Using the Notifications API - Web APIs"
...
**Note:** The examples in this article uses the `Notification()` constructor to create notifications. This is fine for desktop, but on most mobile browsers this will throw a `TypeError`. If you are targeting mobile devices, you should register a service worker and use `ServiceWorkerRegistration.showNotification()` instead.
...
**Note:** The events listed above apply to non-persistent notifications created with the `Notification()` constructor. Persistent notifications created via `ServiceWorkerRegistration.showNotification()` instead fire the `notificationclick` and `notificationclose` events on the `ServiceWorkerGlobalScope`.

### Navigating on activation

Instead of handling click events, you can set the `navigate` option to automatically open a URL when the user activates the notification. This bypasses both the `click` and `notificationclick` events. See `Notification.navigate` for details.
