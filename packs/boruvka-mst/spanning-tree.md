---
title: "Spanning tree"
source: https://en.wikipedia.org/wiki/Spanning_tree
domain: boruvka-mst
...
A collection of disjoint (unconnected) trees is described as a *forest*. A *spanning forest* in a graph is a subgraph that is a forest with an additional requirement. There are two incompatible requirements in use, of which one is relatively rare.
...
- A few graph theory authors define a spanning forest to be a maximal acyclic subgraph of the given graph, or equivalently a subgraph consisting of a spanning tree in each connected component of the graph.
...
- If *G* is the complete bipartite graph $K_{p,q}$ ,then $t(G)=p^{q-1}q^{p-1}$ .
- For the *n*-dimensional hypercube graph $Q_{n}$ , the number of spanning trees is $t(G)=2^{2^{n}-n-1}\prod _{k=2}^{n}k^{n \choose k}$ .
...
In the other direction, given a family of sets, it is possible to construct an infinite connected graph such that every spanning tree of the graph corresponds to a choice function of the family of sets. Therefore, if every infinite connected graph has a spanning tree, then the axiom of choice is true.
