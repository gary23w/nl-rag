...
tags: ospf, link-state routing, interior gateway protocol, dijkstra
fetched: 2026-07-02
---
...
| Network type | Point to point (P2P) | Broadcast (default) | Non-broadcast multi-access (NBMA) | Point to multipoint | Point to multipoint non broadcast (P2MP-NB) | Passive |
|---|---|---|---|---|---|---|
...
| LSAs are sent to | 224.0.0.5 | DR/BDR: 224.0.0.6 All: 224.0.0.5 | DR/BDR: 224.0.0.6 All: 224.0.0.5 | Unicast | Unicast | na |
...
, indicating "External routing: not capable"
...
has all its interfaces belonging to the same area.
...
is a router that connects one or more areas to the main backbone network. It is considered a member of all areas it is connected to. An ABR keeps multiple

instances

of the link-state database in memory, one for each area to which that router is connected.
...
has an interface to the backbone area. Backbone routers may also be area border routers, but do not have to be.
...
is a router that is connected by using more than one routing protocol and that exchanges routing information with routers autonomous systems. ASBRs typically also run an exterior routing protocol (e.g.,

BGP

), or use static routes, or both. An ASBR is used to
...
messages are used by one router to request updated information about a portion of the LSDB from another router. The message specifies the link(s) for which the requesting device wants more current information.
...
messages contain updated information about the state of certain links on the LSDB. They are sent in response to a link state request message, and also broadcast or multicast by routers on a regular basis. Their contents are used to update the information in the LSDBs of routers that receive them.
...
messages provide reliability to the link-state exchange process, by explicitly acknowledging receipt of a Link State Update message.
...
To achieve this, a modified OSPF-BGP redistribution is used. All OSPF routes retain the source LSA type and metric. To prevent loops, an optional DN bit is set by the service provider in LSAs from the provider equipment to indicate that a route has already been sent to the customer's equipment.
...
- Allied Telesis implements OSPFv2 and OSPFv3 in Allied Ware Plus (AW+)
