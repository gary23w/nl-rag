...
source: https://en.wikipedia.org/wiki/APL_(programming_language)
...
| m ← +/3+⍳4 m 22 | These operations can be combined into one statement, remembering that APL evaluates expressions right to left: first `⍳4` creates an array, `[1,2,3,4]`, then 3 is added to each component, which are summed together and the result stored in variable `m`, finally displayed. In normal mathematical notation, it is equivalent to: $\displaystyle m=\sum \limits _{i=1}^{4}(i+3)$ . Recall that mathematical expressions are not read or evaluated from right-to-left. |
...
| ambivalent or monadic | capable of use in a monadic or dyadic context, permitting its left argument to be elided |
...
Today, APL remains in use in a wide range of commercial and scientific applications, asset management, health care, and DNA profiling.
