...
source: https://en.wikipedia.org/wiki/Modulation
domain: li-fi
...
If the alphabet consists of $M=2^{N}$ alternative symbols, each symbol represents a message consisting of *N* bits. If the symbol rate (also known as the baud rate) is $f_{S}$ symbols/second (or baud), the data rate is $Nf_{S}$ bit/second.
