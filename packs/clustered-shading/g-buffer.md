---
title: "Glossary of computer graphics"
source: https://en.wikipedia.org/wiki/G-buffer
...
image channels directly onto a rotated
...
encodes three 10-bit

floating-point

color channels, each with seven bits of
...
(sometimes called "axis oriented"), a

bounding box

stored in world coordinates; one of the simplest
...
, used for various effects. Also known as
...
for a phenomenon to be

simulated

, e.g. some approximation to
...
of 2D texture slices selectable by a 3rd 'W'

texture coordinate

; used to reduce state changes in
...
(holding one or more image channels, typical values being 4,
...
combine to produce the a higher-
...
, describing a formula for combining source and destination
...
is a tree structure on a set of geometric objects.
...
, aligned with the viewer's direction, after the application of the
...
. A clip window may be used to enclose a region around a
...
holding a set of instructions for a
...
programming, or managed by low level

rendering APIs

, or handled internally by
...
for efficiently processing large amounts of data.
...
, possibly held in index buffers. describes geometry as a
...
, a method for generating complex
...
repeated at high frequency combined with a
...
holding depth values. Similar to a
...
3D API, with similar architecture to
...
and displacement of the resulting
...
Also reflection mapping, a technique of approximating reflections of environment on complex surfaces of

3D models

in real time. A complete 360 degree view of environment needs to be prerendered and stored in a texture using a specific mapping (e.g.
...
, bypassing some of the disadvantages of
...
fail an intersection test with the
...
, surfaces show more specular reflections when viewed at near-grazing incidence. This effect is often simulated in computer graphics.
...
, geometry shader is an optional stage able to process 3D model geometry in more advanced ways than a vertex or tessellation shaders (e.g. turn primitives into other primitives).
...
used to accelerate graphical computations.
...
values. Allows additional realism with
...
; one of multiple components per
...
; taking a 3D scene (or other form of encoded data) and producing a
...
for image generation, almost synonymous with a

graphics processing unit

, but historically used to refer to devices aimed at
...
used for fast intersection tests; a
...
. The user typically has more control over (and responsibility for)
...
and 3D scenes are composed. Also known as a
...
, is often generated and added on purpose to add realism.
...
requesting the testing of

bounding volume

geometry against the

depth buffer

to determine if any contents in the
...
(sometimes called object aligned); a

bounding box

stored in some object's
...
. Primitives may be trivially rejected if the
...
are interleaved contiguously in

memory

, possibly containing multiple channels within single
...
for bitmap data. Contrasts with
...
of a vertex attribute with respect to screen space
...
, allowing one vertex plus radius to define a

billboard

; corner vertices are automatically generated. Typically used for
...
, defining a visible window between adjacent
...
, performed by a command list asynchronously form the
...
; capable of mixing alpha blending with
...
, 3D geometry or whole scenes by algorithms (as opposed to manually).
...
through a 3D scene, may be used for
...
, composed of modes and parameters, including resource identifiers, and
...
(or texture buffer) for further use as a resource in subsequent render passes. Used for
...
filters. Requires the ability to use a
...
. Provides an abstraction layer for a
...
, typically held in a command buffer, or submitted programmatically through a
...
, including vertex buffers, index buffers,
...
, index buffers, or other pieces of
...
an RGBA color value encoded as
...
(SSAO) taking direction into account to sample the
...
; it is typically rendered onto other geometry in the main rendering pass.
...
and computing parameter gradients in
...
, hence a long/thin shape, which has undesirable properties during some
...
) for its computations, i.e. only uses
...
, more prominent on metallic surfaces. These highlights depend on the viewer's position as well as the position of the light source and surface normal.
...
, incurring a performance overhead. This overhead is typically minimised by scene
...
stored out of the natural pixel order; see
...
. Performed by a texture sampling unit in a
...
performing texture sampling; also known as a
...
(or resource) used as both a render target and a
...
, usually corresponding to UV coordinates in a
...
that performs triangle setup (and may perform
...
Extension of bilinear filtering that additionally linearly interpolates between different

mipmap

levels of the texture, eliminating sharp transitions.
...
contents without further tests for
...
based on a cheap calculation performed early in a
...
, assigned as vertex attributes and/or calculated in
...
, defining the mapping from texture space to a
...
's surface into a flat

2D plane

in a contiguous, spatially coherent manner for
...
for buffering indexed vertex buffer reads.
...
transforming world space coordinates into
...
and the point of interest on a surface.
...
) completely replaces the user's view of the real world.
...
for greater interactive speed, and clarity for
...
in a 3D scene, reached by applying a
