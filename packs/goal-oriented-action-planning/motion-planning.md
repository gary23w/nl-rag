...
source: https://en.wikipedia.org/wiki/Motion_planning
domain: goal-oriented-action-planning
...
- If the robot is a 2D shape that can translate and rotate, the workspace is still 2-dimensional. However, C is the special Euclidean group **SE**(2) = **R**2 $\times$ **SO**(2) (where **SO**(2) is the special orthogonal group of 2D rotations), and a configuration can be represented using 3 parameters (x, y, θ).
- If the robot is a solid 3D shape that can translate and rotate, the workspace is 3-dimensional, but C is the special Euclidean group **SE(3)** = **R**3 $\times$ **SO**(3), then the configuration requires 6 parameters: (x, y, z) for translation, and Euler angles (α, β, γ).
