...
A **name** in IPNS is the hash of a public key. It is associated with an **IPNS record** (opens new window) containing the content path (`/ipfs/CID`) it links to and other information such as the expiration, the version number, and a cryptographic signature signed by the corresponding private key. New records can be signed and published at any time by the holder of the private key.

For example, the following is an IPNS name represented by a CIDv1 of public key: `k51qzi5uqu5dlvj2baxnqndepeb86cbk3ng7n3i46uzyxzyqj2xjonzllnv0v8` (opens new window).

> **Note:** Kubo uses the `self` key (ed25519 private key used for the PeerID) as the default IPNS name. But you can generate multiple keys via `ipfs key gen` (opens new window), and use them for managing multiple IPNS names.

#### # How IPNS names relate to content paths

IPNS record can point at an immutable or a mutable path. The meaning behind CID used in a path depends on used namespace:

- `/ipfs/<cid>` – an immutable content on IPFS (opens new window) (since the CID contains a multihash)
- `/ipns/<cid-of-libp2p-key>` – a mutable, cryptographic IPNS name (opens new window) which corresponds to a libp2p public key.
...
The self-certifying nature of IPNS records means that they are not tied to a specific transport protocol. In practice, most IPFS implementations rely on the **DHT** and **libp2p PubSub** (opens new window) to publish and resolve IPNS records.
...
> **Note:** This trade-off is best explained by CAP theorem (opens new window).

#### # IPNS over the DHT

The DHT is the default transport mechanism for IPNS records in many IPFS implementations.

Due to the ephemeral nature of the DHT, records expire after 48 hours (opens new window). This applies to any record in the DHT, irrespective of the `validity` (also referred to as `lifetime`) field in the IPNS record.
...
By default, Kubo will republish IPNS records to the DHT based on the `Ipns.RepublishPeriod` (opens new window) configuration which defaults to 4 hours. Republishing (opens new window) involves two steps:

1. Creating an updated IPNS record with the `validity` timestamp field updated (by default based on `Ipns.RecordLifetime` (opens new window)), and signing it with the private key. The `sequence` number will only be incremented if the content path changes.
2. Publish the IPNS record to the DHT (opens new window)
...
IPNS over PubSub uses the Libp2p PubSub (opens new window) to publish records and resolve names amongst **interested peers**.

This is achieved by deriving the PubSub topic name from the IPNS name so that each IPNS name has a unique topic.

Because PubSub doesn't have the notion of persistence (messages are ephemeral and dropped after propagation), IPNS over PubSub adds a persistence layer (opens new window) to ensure that IPNS records are always available to the network.

In Kubo, IPNS over PubSub is not enabled by default and can be enabled using the `Ipns.UsePubsub` (opens new window) configuration.
...
5. Whenever a new peer joins the topic (opens new window) (specifically your mesh), ask them for the record. If they respond with a newer record, update it locally and publish the updated record to the topic.
...
> Further details about the IPNS over PubSub protocol can be found in the IPNS over PubSub Spec (opens new window)
...
When setting the `validity` (referred to as `lifetime` by Kubo (opens new window)) field of an IPNS record, you typically need to choose whether you favor **consistency** (short validity period, e.g. 48 hours) or **availability** (long validity period, e.g. 1 month), due to the inherent trade-off between the two.
...
- **Kubo**: Refer to the `--ttl` option in `ipfs name publish --help` (opens new window) for details on adjusting this setting.
...
- **Kubo**: Configure this using the `Ipns.MaxCacheTTL` (opens new window) setting.
- **Rainbow**: Adjust this with the `RAINBOW_IPNS_MAX_CACHE_TTL` (opens new window) environment variable.
...
See the following guide on publishing IPNS names with Kubo and Helia.
