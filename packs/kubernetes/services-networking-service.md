...
where the service has no selector will fail due to this constraint. This prevents the Kubernetes API server from being used as a proxy to endpoints the caller may not be authorized to access.
...
for a Service. You can expose the Service to the public internet using an
...
). To make the node port available, Kubernetes sets up a cluster IP address, the same as if you had requested a Service of
...
field (for example, to the hostname
...
This is an older alias for

PreferSameZone

that is less clear about the semantics.
...
; these are the responsibility of the cluster administrator.
...
Last modified June 03, 2026 at 10:40 AM PST:

Update HTTP/2 RFC reference from 7540 to 9113 (90d449e0c3)
