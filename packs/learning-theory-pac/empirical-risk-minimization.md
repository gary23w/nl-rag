...
Specifically, let $\epsilon >0$ and consider a sample size n and classification rule $\phi _{n}$ , there exists a distribution of $(X,Y)$ with risk $L^{*}=0$ (meaning that perfect prediction is possible) such that: $\mathbb {E} L_{n}\geq 1/2-\epsilon .$
