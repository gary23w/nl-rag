...
source: https://en.wikipedia.org/wiki/Email_authentication
domain: dkim-signing
license: CC-BY-SA-4.0
tags: domainkeys identified mail, dkim signature signing, email cryptographic signature, message integrity verification, email sender authentication
fetched: 2026-07-02
---
...
Hinging on domain ownership is a stance that emerged in the early 2000. It implies a coarse-grained authentication, given that domains appear on the right part of email addresses, after the at sign. Fine-grained authentication, at a user level, can be achieved by other means, such as Pretty Good Privacy and S/MIME. At present, each individual must ultimately manage their own digital identity.
...
- Outbound connections to port 25 can be intercepted and tunnelled to a transparent proxy.
...
4. In this case the author's ADMD is not involved at all.
5. Some ISPs block port 587, although RFC 5068 clearly says: Access Providers MUST NOT block users from accessing the external Internet using the SUBMISSION port 587.
...
RFC 8601 defines a trace header field `Authentication-Results:` where a receiver can record the results of email authentication checks that it carried out. Multiple results for multiple *methods* can be reported in the same field, separated by semicolons and wrapped as appropriate.
