...
is configured to request an IP address from the DHCP

server

during network initialization. The request-and-grant process uses a lease concept with a controllable time period, allowing the DHCP server to reclaim and then reallocate IP addresses that are not renewed.
...
This method is also variously called
...
. An administrator maps a unique identifier (a
...
IPv4 packet, containing a UDP PDU with DHCP payload...
...
: Option 53 (DHCP Message Type) 1 octet (containing DHCPDISCOVER)
...
: Option 50 (Request IP address) 4 octets (containing
...
: Option: 55 (Parameter Request List) 4 octets
...
: Option 53 (DHCP Message Type) 1 octet (containing DHCPOFFER)
...
: Option 1 (Subnet mask) 4 octets (containing
...
: Option: 3 (Router) 4 octets (containing
...
: Option 51 (Address time) 4 octets (a 86400 second lease time)
...
0x060c09070a0f09070a1009070a13

:

Option 6 (Domain Server) 14 octets (containing
...
: Option 53 (DHCP Message Type) 1 octet (containing DHCPREQUEST)
...
: Option: 54 (DHCP Server) 4 octets (containing
...
: Option 53 (DHCP Message Type) 1 octet (containing DHCPACK)
...
| 16 | Swap server | 4 octets | The IP address of a server where a swap service (e.g., swap over NFS) is provided for diskless workstations |
...
| 85 | Novell Directory Service (NDS) servers | Minimum of 4 octets, multiple of 4 octets | 2241 |
...
| 4 | Data-Over-Cable Service Interface Specifications (DOCSIS) device class | 4 octets | 3256 |
...
Another extension, Authentication for DHCP Messages (RFC 3118), provides a mechanism for authenticating DHCP messages. As of 2002, this extension had not seen widespread adoption because of the problems of managing keys for large numbers of DHCP clients. A 2007 book about DSL technologies remarked that:

> [T]here were numerous security vulnerabilities identified against the security measures proposed by RFC 3118. This fact, combined with the introduction of 802.1X, slowed the deployment and take-rate of authenticated DHCP, and it has never been widely deployed.
...
- RFC 2131 – "*Dynamic Host Configuration Protocol,*" *Draft Standard.*
- RFC 2132 – "*DHCP Options and BOOTP Vendor Extensions,*" *Draft Standard.*
- RFC 3046 – "*DHCP Relay Agent Information Option,*" *Proposed Standard.*
- RFC 3203 – "*DHCP reconfigure extension,*" *Proposed Standard.*
- RFC 3397 – "*Dynamic Host Configuration Protocol (DHCP) Domain Search Option,*" *Proposed Standard.*
- RFC 3442 – "*The Classless Static Route Option for Dynamic Host Configuration Protocol (DHCP) version 4,*" *Proposed Standard.*
- RFC 3942 – "*Reclassifying Dynamic Host Configuration Protocol version 4 (DHCPv4) Options,*" *Proposed Standard.*
- RFC 4361 – "*Node-specific Client Identifiers for Dynamic Host Configuration Protocol Version Four (DHCPv4),*" *Proposed Standard.*
- RFC 4388 – "*Dynamic Host Configuration Protocol (DHCP) Leasequery,*" *Proposed Standard.*
- RFC 4436 – "*Detecting Network Attachment in IPv4 (DNAv4),*" *Proposed Standard.*
- RFC 6926 – "*DHCPv4 Bulk Leasequery,*" *Proposed Standard.*
- RFC 7724 – "*Active DHCPv4 Lease Query,*" *Proposed Standard.*
- RFC 8415 – "*Dynamic Host Configuration Protocol for IPv6 (DHCPv6),*" *Proposed Standard.*
