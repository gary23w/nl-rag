---
title: "Getting Started · tmux/tmux Wiki · GitHub (part 1/2)"
...
- A user connects to a remote server using *ssh(1)* from an *xterm(1)* on their work computer and runs several programs: an editor, a compiler and a few shells.
...
tmux may be installed from package management systems on most major platforms. See this document for instructions on how to install tmux or how to build it from source.

### Other documentation and help

Here are several places to find documentation and help about tmux:

- The manual page has detailed reference documentation on tmux and a description of every command, flag and option. Once tmux is installed it is also available in section 1: `$ man 1 tmux`
...
- `-n` allows a name for the new window to be given. For example using the command prompt to create a window called `mynewwindow` without making it the current window: `:neww -dnmynewwindow`
- The `-t` flag specifies a target for the window. Command targets have a special syntax, but for simple use with `new-window` it is enough just to give a window index. This creates a window at index 999: `:neww -t999`
