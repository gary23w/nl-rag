...
source: https://en.wikipedia.org/wiki/Molecular_orbital
...
The highest occupied molecular orbital and lowest unoccupied molecular orbital are often referred to as the HOMO and LUMO, respectively. The difference of the energies of the HOMO and LUMO is called the HOMO–LUMO gap. This notion is often the matter of confusion in literature and should be considered with caution. For Hartree–Fock calculations where Koopmans' theorem is applicable, the HOMO–LUMO gap can be interpreted as an approximation to the difference between the vertical ionization potential and vertical electron affinity (where vertical denotes processes without molecular geometry change), is usually located below the fundamental gap (difference between the ionization potential and electron affinity), and is usually located above the optical gap (minimum photon energy required to promote an electron to the lowest excited state). In addition, the HOMO–LUMO gap can be related to a bulk-material band gap or transport gap, which is usually much smaller than the single-molecule fundamental gap.
