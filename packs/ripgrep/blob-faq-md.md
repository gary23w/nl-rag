---
title: "ripgrep/FAQ.md at master · BurntSushi/ripgrep · GitHub"
source: https://github.com/BurntSushi/ripgrep/blob/master/FAQ.md
domain: ripgrep
...
# ripgrep/FAQ.md at master · BurntSushi/ripgrep · GitHub
...
- Why does ripgrep get slower when I enabled PCRE2 regexes?
...
Yes. If you installed ripgrep through a package manager on a Unix system, then it would have ideally been installed for you in the proper location. In which case, `man rg` should just work.
...
Or, if your version of `man` supports the `-l/--local-file` flag, then this will suffice:
...
Yes! If you installed ripgrep through a package manager on a Unix system, then the shell completion files included in the release archive should have been installed for you automatically. If not, you can generate completions using ripgrep's command line interface.
...
And then add `$HOME/.zsh-complete` to your `fpath` in, e.g., your `$HOME/.zshrc` file:

```highlight
fpath=($HOME/.zsh-complete $fpath)
```

Or if you'd prefer to load and generate completions at the same time, you can add the following to your `$HOME/.zshrc` file:

```highlight
$ source <(rg --generate complete-zsh)
```

Note though that while this approach is easier to setup, is generally slower than the previous method, and will add more time to loading your shell prompt.
...
And then add `. _rg.ps1` to your PowerShell profile (note the leading period). If the `_rg.ps1` file is not on your `PATH`, do `. /path/to/_rg.ps1` instead.
...
There is more discussion on this topic here: #152
...
- `{value}` is determined by the value of `{attribute}`. If `{attribute}` is `style`, then `{value}` should be one of `nobold`, `bold`, `nointense`, `intense`, `nounderline`, `underline`, `noitalic` or `italic`. If `{attribute}` is `fg` or `bg`, then `{value}` should be a color.
...
Here's an example that highlights the matches with a nice blue background with bolded white text:
...
3. Temporarily disable path translation by setting `MSYS_NO_PATHCONV=1`. e.g., `MSYS_NO_PATHCONV=1 rg /foo`.

For more details, see #1277
...
will replace all instances of 'foo' with 'bar' in the files in which ripgrep finds the foo pattern. The `-i` flag to sed indicates that you are editing files in place, and `s/foo/bar/g` says that you are performing a **s**ubstitution of the pattern `foo` for `bar`, and that you are doing this substitution **g**lobally (all occurrences of the pattern in each file).
...
2. I, as ripgrep's author, would like as many people to use ripgrep as possible. Since the Unlicense is not a proven or well known license, ripgrep is also offered under the MIT license, which is ubiquitous and accepted by almost everyone.
...
Or if you'd prefer, donating to a charitable organization that you like would also be most welcome. My favorites are:
