...
source: https://en.wikipedia.org/wiki/React_(software)
domain: remix
license: CC-BY-SA-4.0 / MIT (remix.run)
...
**React Server Components** (**RSC**) are function components that run exclusively on the server. The concept was first introduced in the talk "Data Fetching with Server Components". Currently, server components are most readily usable with Next.js. With Next.js, it's possible to write components for both the server and the client (browser). When a server rendered component is received by the browser, React in the browser takes over and creates the virtual DOM and attaches event handlers. This is called hydration.
