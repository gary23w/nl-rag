...
source: https://en.wikipedia.org/wiki/Image_frequency
domain: mixer-rf
...
Herold described the years from about 1907 to 1927 as the first major period of vacuum-tube development. Early triodes allowed amplification, oscillation, regeneration, heterodyning, and the superheterodyne. Receiver design during this period was shaped by what could be done with early tubes and by how much performance could be obtained from a small number of them. Engineers developed several approaches to obtaining gain and selectivity:
...
Developments in the superheterodyne receiver and carrier telephony were closely related. Both depended on heterodyning, filters, and stable amplification, and many of the theoretical and practical advances originated from the same research efforts. During the same period, developments in carrier telephony progressed from the first commercial single-channel systems of 1921 to multi-channel multiplex systems by 1928, relying on many of the same advances in frequency conversion, amplification, and filtering that supported the superheterodyne receiver.
...
Heterodyne reception was already present in earlier vacuum-tube designs. What distinguished the superheterodyne was the selection of a fixed intermediate frequency above the audio range, allowing subsequent stages to be optimized for a single frequency. In early conference disscusion, engineers noted that heterodyne frequencies need not be audible.
...
Power supplies were also changing. Early sets relied on expensive batteries, but during the late 1920s battery eliminators and, later, line-powered receivers reduced operating cost. Houck was also involved in this transition; contemporary accounts describe litigation with Cornell-Dubilier over battery eliminator technology with damages reported at approximately $20 million, showing the commercial importance of these developments.
...
By 1930, improved vacuum tubes such as the screen-grid tetrode were incorporated into superheterodyne receivers, further improving gain and stability. During the same period, RCA expanded licensing of superheterodyne patents to other manufacturers amid antitrust pressures, extending access to the design while maintaining royalty arrangements. The superheterodyne receiver played a significant role in the complex patents and licensing in early radio.

The "

All American Five

" AM broadcast receiver from 1940s was inexpensive because it only required five tubes and did not use a power supply transformer.
...
Later commentary distinguished between patent disclosure and practical development, and in practice the superheterodyne was adopted through cross-licensing among major manufacturers. Contemporary technical commentary reflected a distinction between legal priority and practical development. In 1926, Walter Schottky stated that Lévy’s patent described the essential elements of the method and therefore represented its origin from a patent-law perspective, while attributing the practical development of the superheterodyne receiver to Armstrong and his collaborators.
...
If the same RF range were converted directly to 1.4 MHz, the local oscillator would need to tune from 1.4 to 31.4 MHz, requiring an impractically wide tuning range for a single tuned circuit. By converting first to a high IF, image rejection is simplified and oscillator design is eased. For example, with a 1 MHz input signal, the first LO is at 82.4 MHz and the image occurs at $f_{\mathrm {RF} }+2f_{\mathrm {IF} }=163.8\ {\text{MHz}}$ , which is far removed and easily filtered.

The first IF stage typically includes a narrow filter (for example, a crystal filter with a bandwidth of about 12 kHz), which limits the signal bandwidth before further conversion and attenuates many potential interferers. This reduces overload and intermodulation in subsequent stages. A second conversion then translates the signal to a lower IF (for example, mixing 81.4 MHz with 80 MHz to produce 1.4 MHz), where higher selectivity can be obtained.
