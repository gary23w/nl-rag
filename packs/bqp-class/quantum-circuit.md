...
source: https://en.wikipedia.org/wiki/Quantum_circuit
domain: bqp-class
...
We now provide a mathematical model for how quantum circuits can simulate *probabilistic* but classical computations. Consider an *r*-qubit circuit *U* with register space *H*QB(*r*). *U* is thus a unitary map
...
The contents *x* = *x*1, ..., *x**m* of the classical input register are used to initialize the qubit register in some way. Ideally, this would be done with the computational basis state
...
The function *F*:*X* → *Y* is computed by a circuit *U*:*H*QB(*r*) → *H*QB(*r*) to within ε if and only if for all bitstrings *x* of length *m*
