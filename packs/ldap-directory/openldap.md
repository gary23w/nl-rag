---
title: "OpenLDAP"
source: https://en.wikipedia.org/wiki/OpenLDAP
...
- Data storage backends – these actually store data
  - back-bdb: the first transactional backend for OpenLDAP, built on Berkeley DB, removed with OpenLDAP 2.5.
  - back-hdb: a variant of back-bdb that is fully hierarchical and supports subtree renames, removed with OpenLDAP 2.5.
  ...
  - back-ndb: a transactional backend built on MySQL's NDB cluster engine, removed with OpenLDAP 2.6.
  - back-wiredtiger: an experimental transactional backend built on WiredTiger, introduced with OpenLDAP 2.5.
- Proxy backends – these act as gateways to other data storage systems
  - back-asyncmeta: an asynchronous proxy with meta-directory features, introduced with OpenLDAP 2.5.
  ...
  - back-sql: talks to arbitrary SQL databases, deprecated with OpenLDAP 2.5.
- Dynamic backends – these generate data on the fly
  ...
  - back-perl: invokes arbitrary perl modules in response to LDAP requests, deprecated with OpenLDAP 2.5.
  - back-shell: invokes shell scripts for LDAP requests, removed with OpenLDAP 2.5.
...
- Native slapd modules
  ...
  - passwd/ – additional password hashing mechanisms. Currently includes Kerberos, Netscape, RADIUS, and SHA-2.
- SLAPI plugins
...
- OpenLDAP Version 2.5, released in April 2021, introduced the LDAP load balancing proxy server, LDAP transaction support, HA proxy protocol v2 support, plus much more.
