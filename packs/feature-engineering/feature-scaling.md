...
where x is an original value, $x'$ is the normalized value, ${\bar {x}}={\text{average}}(x)$ is the mean of that feature vector. There is another form of the means normalization which divides by the standard deviation which is also called standardization.
