...
A **PCIe switch** is like a network switch, except that it works with PCIe connections. In the tree topology is one *upstream* port where it connects to a higher-level device (another switch or directly a PCIe bus) and a number of *downstream* ports. The switch is responsible for receiving a packet from one port and forwarding it to an appropriate, different port. By doing packet-switching, the bandwidth is effectively shared across multiple downstream ports (compared to static lane bifurcation). A more advanced multi-host topology contains multiple upstream ports.
...
| 6-pin power connector (75 W) Pin Description 01 +12 V 02 Not connected (usually +12 V as well) 03 +12 V —N/a 04 Ground 05 Sense 06 Ground —N/a | 8-pin power connector (150 W) Pin Description 01 +12 V 02 +12 V 03 +12 V 04 Sense 1 (8-pin connected) 05 Ground 06 Sense 0 (6-pin or 8-pin connected) 07 Ground 08 Ground |   |
|---|---|---|
...
In 2021, PCIe Card Electromechanical (CEM, pronounced like "chem" in "chemistry") Specification introduced a connector for 48 Volts with two current-carrying contacts and four sense pins. It was retained into PCIe-CEM 5.1 of 2023. The contacts are rated for 15 Amps continuous current. The 48VHPWR connector can carry 720 watts.
...
The most recent version of OCuLink, OCuLink-2, supports 8 GB/s or 16 GB/s (PCIe 4.0 ×4 or ×8) while the maximum bandwidth of a USB 80Gbit/s (USB4 Gen 4) or Thunderbolt 5 connection is 10 GB/s.

OCulink is principally intended for PCIe (or SATA breakout) interconnections in servers, but also finds limited adoption on laptops for the connection of external GPU boxes.

#### CopprLink

CopprLink uses SFF-TA-1032 connectors to support external PCIe cable for PCIe 5.0 x16 and PCIe 6.0 x16.
...
On 18 November 2010, the PCI-SIG officially published the finalized PCI Express 3.0 specification to its members to build devices based on this new version of PCI Express.
...
On 29 May 2019, PCI-SIG officially announced the release of the final PCI Express 5.0 specification. The PCI Express 5.0 retained backward compatibility with previous versions of PCI Express specifications.
...
On 5 August 2025, PCI-SIG announced the development of PCI Express 8.0. The specification is planned for release by year 2028. It will deliver double the speed of the previous version, 256.0 GT/s raw bit rate and up to 1 TB/s bi-directionally via ×16 configuration.
