...
- This will find both buttons and throw a strictness violation error: `await page.locator('button').click();`
- This will only find a second button, because it is visible, and then click it. `await page.locator('button').filter({ visible: true }).click();`
