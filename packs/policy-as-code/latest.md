...
OPA is proud to be a graduated Cloud Native Computing Foundation (CNCF) project.

This page covers core concepts in OPA's policy language (Rego) as well as how to download and run OPA.
...
This section covers the building blocks of writing policies in Rego. You can see how these concepts come together to solve the network security policy in the Complete Example.
...
While plain iteration is a fundamental building block, Rego also features ways to express *FOR SOME* and *FOR ALL* more explicitly.
...
Learn more about the Every Keyword.
...
Complete rules are if-then statements that assign a single value to a variable. Every rule consists of a head and a body. In Rego, the rule head is true *if* the rule body is true for some set of variable assignments.
...
Check out this blog post that goes into much more detail on this topic showing different methods to express OR in idiomatic Rego for different use cases.
...
This section explains how you can query OPA directly and interact with it on your own machine. If you just want to quickly get a feel for the language without installing anything, check out the OPA Playground.
...
OPA binaries can be installed on macOS using Homebrew. The formula can be reviewed on brew.sh. This method supports both ARM64 and AMD64 architectures.
...
See all available binaries on the GitHub releases. Checksums for all binaries are available in the download path by appending `.sha256` to the binary filename.
...
The simplest way to interact with OPA is via the command-line using the `opa eval` sub-command. It can be used to evaluate arbitrary Rego expressions and policies. `opa eval` supports a large number of options for controlling evaluation. Commonly used flags include:
