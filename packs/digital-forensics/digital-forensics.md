...
source: https://en.wikipedia.org/wiki/Digital_forensics
...
the offender's alibi was disproved when mobile phone records of the person he claimed to be with showed she was out of town at the time.
...
included references to a site discussing
...
One major limitation to a forensic investigation is the use of encryption; this disrupts initial examination where pertinent evidence might be located using keywords. Laws to compel individuals to disclose encryption keys are still relatively new and controversial. But always more frequently there are solutions to brute force passwords or bypass encryption, such as in smartphones or PCs where by means of bootloader techniques the content of the device can be first acquired and later forced in order to find the password or encryption key. Concerns have been raised regarding the scientific validation of digital forensic tools and methodologies, which includes the lack of standardized testing frameworks and reproducibility across investigations. It is estimated that about 60% of cases that involve encrypted devices, often go unprocessed because there is no way to access the potential evidence.
...
IoT forensics is a branch of Digital forensics that has the goal of identifying and extracting digital information from devices belonging to the Internet of things field, to be used for forensics investigations as potential source of evidence.

### Cloud Forensics

**Cloud Forensics** – The application of digital forensic principles to cloud computing environments, addressing challenges such as multi-tenancy, data remanence, and jurisdictional issues. The National Institute of Standards and Technology (NIST) has published a guide outlining cloud forensic science challenges.
