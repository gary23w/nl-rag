...
domain: qemu-emulator
license: CC-BY-SA-4.0
tags: qemu, binary translation, machine emulator, system emulation
fetched: 2026-07-02
---
...
QEMU is free software originally developed by Fabrice Bellard; the first preview release was in 2003. Different components of QEMU are licensed under the GNU General Public License (GPL), BSD license, GNU Lesser General Public License (LGPL), or other GPL-compatible licenses.
...
Since QEMU Version 0.10.0 the Tiny Code Generator (TCG) replaced the previous *DynGen* system, which relied on GCC 3.x to work.

As QEMU runs basic blocks of target code are translated into an machine-independent intermediate representation known as *TCG ops*. The operations then go through a simple optimization passes before being passed to the backend which will emit target code for the host's architecture. TCG requires dedicated code written to support every architecture it runs on, so that the JIT knows what to translate the *TCG ops* to. If no dedicated JIT code is available for the architecture, TCG falls back to a slow interpreter mode called TCG Interpreter (TCI).
...
QEMU supports a number of disk image formats.

- Raw images (`.img`) simplest image that contain sector-by-sector contents of a disk
- QCOW2 (`.qcow2`) - native QEMU format supporting a number of features like snapshots, compression, and encryption
- VirtualBox Virtual Disk Image (`.vdi`)
- VMware Virtual Machine Disk (`.vmdk`)
- Virtual PC Virtual Hard Disk (`.vhd, .vhdx`)

It is capable of read only access with a number of other formats including:

- Bochs - native format of the Bochs emulator
- Linux cloop - Linux Compressed Loop image
...
QEMU can also directly access host devices and partitions which allows it to work with other storage technologies like iSCSI, LVM, LUKS, NBD, NVMe and ZFS.

It can also support exporting a file-system into a Virtual VFAT device.
