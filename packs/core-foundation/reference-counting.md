...
source: https://en.wikipedia.org/wiki/Reference_counting
...
For example, the type `Rc<T>` provides shared ownership of a value of type `T`, allocated on the heap for multiple references to its data.
...
Interior mutability without `UnsafeCell` has performance costs, too, so, for maximum performance, some applications may call for additional complexity.
