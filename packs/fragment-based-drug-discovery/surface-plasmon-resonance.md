...
source: https://en.wikipedia.org/wiki/Surface_plasmon_resonance
...
where k( $\omega$ ) is the wave vector, $\varepsilon$ is the relative permittivity, and $\mu$ is the relative permeability of the material (1: the glass block, 2: the metal film), while $\omega$ is angular frequency and ${c}$ is the speed of light in vacuum.
...
Surface plasmon resonance can be implemented in analytical instrumentation. SPR instruments consist of a light source, an input scheme, a prism with analyte interface, a detector, and computer. Any quantitative derivation relies on modelling the surface based on the boundary conditions of the Maxwell's equations, for plane films using the Fresnel equations.
...
### Inverted Surface Plasmon Resonance (ISPR) features

SPR curves usually show a strong minimum. With the use of a combined chromium-gold film or other carefully chosen material combinations a sharp resonance maximum can be observed when exciting a surface plasmon in the Kretschmann configuration. This may be useful in sensors applications where a strong and sharp resonance above noise is required and less heat is desired at the surface.
