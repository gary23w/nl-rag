...
|---|---|---|---|---|---|
| Pease-Shostak-Lamport | Synchronous | Oral | $n>3f$ | $f+1$ | total communication $O(n^{f})$ |
| Pease-Shostak-Lamport | Synchronous | Written | $n>f+1$ | $f+1$ | total communication $O(n^{f})$ |
| Ben-Or | Asynchronous | Oral | $n>5f$ | $O(2^{n})$ (expected) | expected $O(1)$ rounds when $f<{\sqrt {n}}$ |
| Cerberus | Partially Synchronous | Written | $n>3f$ |   | Multi-shard; braided synchronization |
| Dolev et al. | Synchronous | Oral | $n>3f$ | $2f+3$ | total communication $O(f^{3}\log f)$ |
| Dolev-Strong | Synchronous | Written | $n>f+1$ | $f+1$ | total communication $O(n^{2})$ |
| Dolev-Strong | Synchronous | Written | $n>f+1$ | $f+2$ | total communication $O(nf)$ |
| Feldman-Micali | Synchronous | Oral | $n>3f$ | $O(1)$ (expected) |   |
| Katz-Koo | Synchronous | Written | $n>2f$ | $O(1)$ (expected) | Requires Public Key Infrastructure (PKI) |
| PBFT | Asynchronous (safety) Synchronous (liveness) | Oral | $n>3f$ |   |   |
| HoneyBadger | Asynchronous | Oral | $n>3f$ | $O(\log n)$ (expected) | per tx communication $O(n)$ - requires public-key encryption |
| Abraham et al. | Synchronous | Written | $n>2f$ | 8 |   |
| Byzantine Agreement Made Trivial | Synchronous | Signatures | $n>3f$ | 9 (expected) | Requires digital signatures |
| Locher | Synchronous | Signatures | $n>4f$ | 6 (expected) | Requires digital signatures |
