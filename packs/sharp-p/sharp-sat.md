---
title: "♯SAT"
source: https://en.wikipedia.org/wiki/Sharp-SAT
domain: sharp-p
...
Even though 2SAT (deciding whether a 2CNF formula has a solution) is polynomial, counting the number of solutions is #P-complete. The #P-completeness already in the monotone case, i.e., when there are no negations (#MONOTONE-2-CNF).
...
Weighted model counting (WMC) generalizes #SAT by computing a linear combination of the models instead of just counting the models. In the literal-weighted variant of WMC, each literal gets assigned a weight, such that ${\text{WMC}}(\phi ;w)=\sum _{M\models \phi }\prod _{l\in M}w(l)$ .
