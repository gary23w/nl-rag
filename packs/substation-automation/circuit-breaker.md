...
Low-voltage **miniature circuit breakers** (**MCB**) use air alone to extinguish the arc. These circuit breakers contain so-called arc chutes, a stack of mutually insulated parallel metal plates that divide and cool the arc. Splitting the arc into smaller arcs makes the arc cooler as the arc voltage increases and serves as an additional impedance that limits the current through the circuit breaker. The current-carrying parts near the contacts provide easy deflection of the arc into the arc chutes by a magnetic force of a current path, although magnetic blowout coils or permanent magnets could also deflect the arc into the arc chute (used on circuit breakers for higher ratings). The number of plates in the arc chute is dependent on the short-circuit rating and nominal voltage of the circuit breaker.
...
Vacuum circuit breakers have minimal arcing (as there is nothing to ionize other than the contact material). The arc is quenched when it is stretched a very small amount (less than 3 mm (0.1 in)). Vacuum circuit breakers are frequently used in modern medium-voltage switch gear to 38,000 volts.
...
Typical domestic panel circuit breakers are rated to interrupt 10 kA (10,000 A) short-circuit current.
...
| C | 5–10 times *In* |
| D | 10–20 times *In* |
| K | 8–15 times *In* For the protection of loads that cause frequent short-duration (approximately 400 ms to 2 s) current peaks in normal operation |
| Z | 2–3 times *In* for durations on the order of tens of seconds. For the protection of loads such as semiconductor devices or measuring circuits using current transformers. |
...
A smart circuit breaker is an electronic overcurrent protection device that integrates high-frequency sensors and Internet-of-Things (IoT) connectivity to provide intelligent control and real-time monitoring of electrical circuits. Unlike traditional circuit breakers, smart breakers utilize digital logic and communication to augment their operation. Protocols such as Wi-Fi, Zigbee, or cellular networks are used to gather and transmit granular electrical data including voltage, current, and power quality . These devices are implemented either as modular components that replace standard breakers in a compatible load center or as core elements of a comprehensive smart electrical panel. This technology transforms the breaker box into an active "energy hub," often capable of decision-making and precise circuit-level metering.

One of the primary use case for smart circuit breakers in residential environments is the optimization of home energy management systems (HEMS), especially those incorporating solar arrays, electric vehicle (EV) chargers, and battery storage . Through mobile applications, users can remotely actuate circuits, schedule high-intensity loads to run during off-peak hours for cost savings, and implement dynamic load shedding to prioritize essential appliances during power outages. Using these can provide significant benefits in reducing battery cycling and increasing the power capacity of existing electrical service . In commercial and industrial settings, smart breakers are integrated with Building Management Systems (BMS) to enable advanced functions such as automated tenant billing, demand response participation, and predictive maintenance through anomaly detection. By intelligently managing demand, these systems allow facilities to scale energy-intensive infrastructure without necessitating costly utility service upgrades.
