...
Let $A=a_{1}a_{2}...a_{n}$ and $B=b_{1}b_{2}...b_{m}$ be the sequences to be aligned, where n and m are the lengths of A and B respectively.
...
2. Construct a scoring matrix H and initialize its first row and first column. The size of the scoring matrix is $(n+1)*(m+1)$ . The matrix uses 0-based indexing. $H_{k0}=H_{0l}=0\quad for\quad 0\leq k\leq n\quad and\quad 0\leq l\leq m$
3. Fill the scoring matrix using the equation below. $H_{ij}=\max {\begin{cases}H_{i-1,j-1}+s(a_{i},b_{j}),\\\max _{k\geq 1}\{H_{i-k,j}-W_{k}\},\\\max _{l\geq 1}\{H_{i,j-l}-W_{l}\},\\0\end{cases}}\qquad (1\leq i\leq n,1\leq j\leq m)$ where $H_{i-1,j-1}+s(a_{i},b_{j})$ is the score of aligning $a_{i}$ and $b_{j}$ , $H_{i-k,j}-W_{k}$ is the score if $a_{i}$ is at the end of a gap of length k , $H_{i,j-l}-W_{l}$ is the score if $b_{j}$ is at the end of a gap of length l , 0 means there is no similarity up to $a_{i}$ and $b_{j}$ .
...
This substitution matrix can be described as: $s(a_{i},b_{j})={\begin{cases}+1,\quad a_{i}=b_{j}\\-1,\quad a_{i}\neq b_{j}\end{cases}}$
...
Here we discuss two common strategies for gap penalty. See Gap penalty for more strategies. Let $W_{k}$ be the gap penalty function for a gap of length k :
...
where $W_{1}$ is the cost of a single gap.
...
The simplified algorithm uses $O(mn)$ steps. When an element is being scored, only the gap penalties from the elements that are directly adjacent to this element need to be considered.
...
- Substitution matrix: $s(a_{i},b_{j})={\begin{cases}+3,\quad a_{i}=b_{j}\\-3,\quad a_{i}\neq b_{j}\end{cases}}$
- Gap penalty: $W_{k}=2k$ (a linear gap penalty of $W_{1}=2$ )
