...
The standard distribution delivers a standard toolkit and protocol stack for building graphical user interfaces on most Unix-like operating systems and OpenVMS, and has been ported to many other contemporary general purpose operating systems.
...
- use `ssh -X` command to connect to the remote machine
...
The X.Org implementation is the canonical implementation of X. Owing to liberal licensing, a number of variations, both free and open-source (FOSS) and proprietary, have appeared. Commercial Unix vendors have tended to take the reference implementation and adapt it for their hardware, usually customizing it and adding proprietary extensions.
...
X's design lacks sandboxing, so any application can access and manipulate; keyboard, mouse, display and clipboard content.

#### Privilege escalation

Poorly configured systems may run X11 as root, allowing privilege escalation; in the past this was exploited by unprivileged apps to gain privilege.
...
- The Wayland protocol is being developed by several X.Org developers as a replacement (with the Wayland compositors) for X. It works directly with the GPU hardware, via DRI. Wayland compositors can use XWayland, an X server which can be rootless. The project reached version 1.0 in 2012. Like Android, Wayland is EGL-based.
...
— The email in which X was introduced to the
...
By the late 1980s X was "Athena's most important single achievement to date," as Simson Garfinkel wrote in 1989. DEC reportedly believed that its development alone had made the company's donation to MIT worthwhile. Gettys joined the design team for the VAXstation 2000 to ensure that X, which DEC called DECwindows, would run on it, and the company assigned 1,200 employees to port X to both Ultrix and VMS. By 1990 IBM and Motorola announced their own X terminals. Bill Joy of Sun Microsystems, which made diskless workstations competing with X terminals, argued that X was technically flawed and could overwhelm networks.
...
XFree86 development continued for a few more years, with 4.8.0 being released on 15 December 2008.
...
An area into which graphics may be rendered, either through software alone into system memory as with

VNC

, or within a graphics device, some of which can render into more than one screen simultaneously, either viewable simultaneously or interchangeably. Interchangeable screens are often set up to be notionally left and right from one another, flipping from one to the next as the mouse pointer reaches the edge of the monitor.
...
-based workstations are usually capable of having multiple displays, among which the user can switch with a special keyboard combination such as control-alt-

function-key

, simultaneously flipping all the monitors from showing the screens of one display to the screens in another.
...
|---|---|---|
| Unsupported: X1 | June 1984 | First use of the name "X"; fundamental changes distinguishing the product from W. |
| Unsupported: X6 | January 1985 | First version licensed to a handful of outside companies. |
| Unsupported: X9 | September 1985 | Color. First release under MIT License. |
| Unsupported: X10 | November 1985 | IBM RT PC, AT (running DOS), and others. |
| Unsupported: X10R2 | January 1986 |   |
| Unsupported: X10R3 | February 1986 | First freely redistributable X release. Earlier releases required a BSD source license to cover code changes to init/getty to support login. uwm made standard window manager. |
| Unsupported: X10R4 | December 1986 | Last version of X10. |
| Unsupported: X11 | 15 September 1987 | First release of the current protocol. |
| Unsupported: X11R2 | February 1988 | First X Consortium release. |
| Unsupported: X11R3 | 25 October 1988 | XDM. |
| Unsupported: X11R4 | 22 December 1989 | XDMCP, twm (from the beginning known as Tom's window manager) brought in as standard window manager, application improvements, shape extension, new fonts. |
| Unsupported: X11R5 | 5 September 1991 | X386 1.2, PEX, Xcms (color management), font server, X video extension. |
| Unsupported: X11R6 | 16 May 1994 | ICCCM v2.0; Inter-Client Exchange; X Session Management; X Synchronization extension; X Image extension; XTEST extension; X Input; X Big Requests; XC-MISC; XFree86 changes. |
| Unsupported: X11R6.1 | 14 March 1996 | X Double Buffer extension; X keyboard extension; X Record extension. |
| Unsupported: X11R6.2 X11R6.3 | 23 December 1996 | Web functionality, LBX. Last X Consortium release. X11R6.2 is the tag for a subset of X11R6.3 (Broadway) with the only new features over R6.1 being XPrint and the Xlib implementation of vertical writing and user-defined character support. Broadway was the code name for running X applications on a web browser via a browser plugin and Low Bandwidth X. |
| Unsupported: X11R6.4 | 31 March 1998 | Xinerama. |
| Unsupported: X11R6.5 | 2000 | Internal X.org release; not made publicly available. |
| Unsupported: X11R6.5.1 | 20 August 2000 |   |
| Unsupported: X11R6.6 | 4 April 2001 | Bug fixes, XFree86 changes. |
| Unsupported: X11R6.7.0 | 6 April 2004 | First X.Org Foundation release, incorporating XFree86 4.4rc2. Full end-user distribution. Removal of XIE, PEX and libxml2. |
| Unsupported: X11R6.8.0 | 8 September 2004 | Window translucency, XDamage, Distributed Multihead X, XFixes, Composite, XEvIE. |
| Unsupported: X11R6.8.1 | 17 September 2004 | Security fix in libxpm. |
| Unsupported: X11R6.8.2 | 10 February 2005 | Bug fixes, driver updates. |
| Unsupported: X11R6.9 X11R7.0 | 21 December 2005 | XServer 1.0.1, EXA, major source code refactoring. From the same source-code base, the modular autotooled version became 7.0 and the monolithic imake version was frozen at 6.9. |
| Unsupported: X11R7.1 | 22 May 2006 | XServer 1.1.0, EXA enhancements, KDrive integrated, AIGLX, OS and platform support enhancements. |
| Unsupported: X11R7.2 | 15 February 2007 | XServer 1.2.0, Removal of LBX and the built-in keyboard driver, X-ACE, XCB, autoconfig improvements, cleanups. |
| Unsupported: X11R7.3 | 6 September 2007 | XServer 1.4.0, Input hotplug, output hotplug (RandR 1.2), DTrace probes, PCI domain support. |
| Unsupported: X11R7.4 | 23 September 2008 | XServer 1.5.1, XACE, PCI-rework, EXA speed-ups, _X_EXPORT, GLX 1.4, faster startup and shutdown. |
| Unsupported: X11R7.5 | 26 October 2009 | XServer 1.7.1, Xi 2, XGE, E-EDID support, RandR 1.3, MPX, predictable pointer acceleration, DRI2 memory manager, SELinux security module, further removal of obsolete libraries and extensions. |
| Unsupported: X11R7.6 | 20 December 2010 | X Server 1.9.3, XCB requirement. |
| Latest version: X11R7.7 | 6 June 2012 | X Server 1.12.2; Sync extension 3.1: adds Fence object support; Xi 2.2 multitouch support; XFixes 5.0: Pointer Barriers. |
