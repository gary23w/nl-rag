...
), but not yet healthy are considered disrupted and can be evicted regardless of whether the criteria in a PDB is met.
...
phase are always considered for eviction.
...
Last modified October 21, 2024 at 4:29 PM PST:

Clarify PDB `maxUnavailable` only works if all pods have same controller (ffb9e2dae1)
