...
by a scheduler in order to accommodate a new Pod with a higher priority. For more information, see
...
taint that the Pod does not tolerate; see
...
Pod, that is bound to a no longer existing Node, is due to be deleted by
...
A Pod disruption might be interrupted. The control plane might re-attempt to continue the disruption of the same Pod, but it is not guaranteed. As a result, the
...
Last modified May 16, 2025 at 1:29 PM PST:

Fix wrong link to Node Autoscaling docs (307d6a6855)
