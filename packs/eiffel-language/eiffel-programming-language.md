...
source: https://en.wikipedia.org/wiki/Eiffel_(programming_language)
...
```mw
-- Aliases and assigner commands

item alias "[]" (key: STRING): ELEMENT         -- ☝︎1
...
```mw
   -- using put() directly
   phone_book.put (New_person, "JILL SMITH") -- ☝︎2
```
...
```mw
   -- using alias
   phone_book ["JILL SMITH"] := New_person -- ☝︎3
```

(in the same way that `phone_book ["JILL SMITH"]` is a synonym for `number := phone_book.item ("JILL SMITH")`), provided the declaration of `item` now starts (replacement for ☝︎1) with

```mw
   item alias "[]" (key: STRING): ELEMENT assign put
```

This declares `put` as the assigner command associated with `item` and, combined with the bracket alias, makes ☝︎3 legal and equivalent to ☝︎2. (It could also be written, without taking advantage of the bracket, as `phone_book.item ("JILL SMITH") := New_person`.
