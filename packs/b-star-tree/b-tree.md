...
Internal nodes (also known as

inner nodes

) are all nodes except for leaf nodes and the root node. They are usually represented as an ordered set of elements and child pointers. Every internal node contains a
...
−1; therefore, each internal node is at least half full. The relationship between
...
−1 elements in the entire tree, the root will be the only node in the tree with no children at all.
...
/2-1 if they are not the root) and pointers (one for each key) to nodes carrying the data objects/chunks.
...
|   | *pt*0 | pti | pri |   |   |   |
|---|---|---|---|---|---|---|
| when | *k*0 exists | *k**i*-1 and ki exist | *k**i*-1 exists, and ki does not exist | *k**i*-1 and ki do not exist | ki exists | ki does not exist |
| Points to subtree in which all search keys, Pt : | *Pt* < *k*0 | *k**i*-1 < *Pt* < *ki* | *Pt* > *k**i*-1 | pti is empty. | Points to a record with a value *Pr* = *ki* | pri is empty. |

Each leaf node in a B-tree has the following format:

| *pr*0 | *k*0 | *pr*1 | *k*1 | ... | *pr**K*-1 | *k**K*-1 |
|---|---|---|---|---|---|---|

| pri when ki exists | pri when ki does not exist |
|---|---|
| Points to a record with a value equal to ki. | Here, pri is empty. |
...
If the database does not change, then compiling the index is simple to do, and the index need never be changed. If there are changes, managing the database and its index requires additional computation.
...
: The rebalancing operations are different for B+ trees (e.g., rotation is different because parent has copy of the key) and B

*

-tree (e.g., three siblings are merged into two siblings).
