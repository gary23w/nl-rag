---
title: "Basic Data Types and Pattern Matching · OCaml Documentation"
source: https://ocaml.org/docs/basic-data-types
domain: ocaml
...
The `int` type represents platform-dependent signed integers. This means `int` does not always have the same number of bits. It depends on underlying platform characteristics, such as processor architecture or operating system. Operations on `int` values are provided by the `Stdlib` and the `Int` modules.
...
In OCaml, `if … then … else …` is a *conditional expression*. It has the same type as its branches.
...
Operations on `bytes` values are provided by the `Stdlib` and the `Bytes` modules. Only the function `Bytes.get` allows direct access to the characters contained in a byte sequence. Unlike arrays, there is no direct access operator on byte sequences.
...
Note that types `int * char * bool`, `int * (char * bool)`, and `(int * char) * bool` are not the same. The values `(42, 'h', true)`, `(42, ('h', true))`, and `((42, 'h'), true)` are not equal. In mathematical language, the product type operator `*` is not *associative*.
...
1. Introduction
2. Predefined Types
  1. Integers, Floats, Booleans, and Characters
    1. Integers
    2. Floats and Type Conversions
    3. Booleans
    4. Characters
  2. Strings & Byte Sequences
    1. Strings
    2. Byte Sequences
  3. Arrays & Lists
    1. Arrays
...
3. User-Defined Types
  1. Variants
    ...
    2. Constructors With Data
    3. Recursive Variants
  2. Polymorphic Data Types
    1. Revisiting Predefined Types
    2. User-Defined Polymorphic Types
  3. Records
  4. Aliases
    1. Type Aliases
    2. Function Parameter Aliases
4. A Complete Example: Mathematical Expressions
5. Conclusion
