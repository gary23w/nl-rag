---
title: "Lists · OCaml Documentation"
source: https://ocaml.org/docs/lists
domain: ocaml
...
Notice the type of the function `f` in parentheses as part of the whole type. This `map` function, given a function of type `'a -> 'b` and a list of `'a`s, will build a list of `'b'`s. Sometimes `'a` and `'b` might be the same type, of course. Here are two examples showing the `map` function in use:
...
The standard library List module contains a wide range of useful utility functions, including pre-written versions of many of the functions we have written in this tutorial. A version of the module with labelled functions is available as part of StdLabels.
...
In addition, we have an imperative analogue to `map`, called `iter`. It takes an imperative function of type `'a -> unit` and an `'a list` and applies the function to each element in turn. A suitable function might be `print_endline`:
...
The function `Fun.flip` reverses a binary function parameter order.
...
In this example, the initial accumulator is the second list, and each element of the first is consed to it in turn. You can see the order of the fold right parameters is a little different:
...
The `length` function previously defined builds up an intermediate expression of a size proportional to its input list:
...
**Note** The above is not OCaml syntax, it is a sketch meant to explain how evaluation of `length [1; 2; 3]` takes place. The sign `=>` indicates an evaluation step.

For long lists, this may overflow the stack (be too large for the computer to handle). The solution is to write our function with an accumulating parameter, like this:
...
3. The Standard Library List Module
  1. Maps and Iterators
  2. List Scanning
  3. List Searching
  4. Association Lists
  5. Sorting Lists
  6. Folds
4. Lists and Tail Recursion
