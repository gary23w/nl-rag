...
) enables the creation of new syntactic constructs (e.g.

control structures

) indistinguishable from those built into the language. For instance, in a Lisp dialect that has
...
, it is possible to define the latter in terms of the former using macros. For example, Scheme has both
...
By the late 1950s the macro language was followed by the Macro Assemblers. This was a combination of both where one program served both functions, that of a macro pre-processor and an assembler in the same package. Early examples are FORTRAN Assembly Program (FAP) and Macro Assembly Program (IBMAP) on the IBM 709, 7094, 7040 and 7044, and Autocoder on the 7070/7072/7074.
