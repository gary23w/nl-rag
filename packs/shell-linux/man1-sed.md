...
q [exit-code]
       ...
       input, except that if auto-print is not disabled the
...
match that address; or with two addresses, in which case the
command will be executed for all input lines which match the
inclusive range of lines starting from the first address and
continuing to the second address.  Three things to note about
address ranges: the syntax is addr1,addr2 (i.e., the addresses are
...
regular expressions instead; it has been supported for years by
GNU sed, and is now included in POSIX.
