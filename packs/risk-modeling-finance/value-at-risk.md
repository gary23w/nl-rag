---
title: "Value at risk"
source: https://en.wikipedia.org/wiki/Value_at_risk
...
Value at risk can also be written as a distortion risk measure given by the distortion function $g(x)={\begin{cases}0&{\text{if }}0\leq x<1-\alpha \\1&{\text{if }}1-\alpha \leq x\leq 1\end{cases}}.$
...
For $X\in \mathbf {L} _{M^{+}}$ (with $\mathbf {L} _{M^{+}}$ the set of all Borel measurable functions whose moment-generating function exists for all positive real values) we have
...
in which $M_{X}(z)$ is the moment-generating function of X at z. In the above equations the variable X denotes the financial loss, rather than wealth as is typically the case.
