...
source: https://en.wikipedia.org/wiki/Interrupt_handler
...
During the execution of an interrupt service routine, it is possible for another interrupt to occur. How this second interrupt is handled is dependent on the hardware. In general, a developer will decide whether or not to let the current ISR be interrupted to handle a higher priority interrupt. If so, the current program counter (and often flags) will be saved to the stack before the address of the higher priority ISR is written to the program counter. The higher priority interrupt service routine must then save any registers it may alter. Upon completion of the higher priority ISR, the address and state from the lower priority ISR will be popped from the stack and execution of the lower priority ISR will continue.
...
A FLIH implements at minimum platform-specific interrupt handling similar to *interrupt routines*. In response to an interrupt, there is a context switch, and the code for the interrupt is executed. The job of a FLIH is to quickly service the interrupt, or to record platform-specific critical information which is only available at the time of the interrupt, and schedule the execution of a SLIH for further long-lived interrupt handling.
