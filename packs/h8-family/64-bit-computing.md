...
graphics workstations starting with the
...
architecture which was born from the
...
announces plans for the 64-bit

IA-64

architecture (jointly developed with

Hewlett-Packard

) as a successor to its 32-bit
...
. IBM releases the A10 and A30 microprocessors, the first 64-bit PowerPC AS processors.

IBM also releases a 64-bit AS/400 system upgrade, which can convert the operating system, database and applications.
...
, servers, workstations, and other appliances. Intel released

Core 2 Duo

as the first mainstream x86-64 processor for its mobile, desktop, and workstation line. Prior 64-bit extension processor lines were not widely available in the consumer retail market (most of 64-bit Pentium 4/D were OEM), 64-bit Pentium 4, Pentium D, and Celeron were not into mass production until late 2006 due to poor yield issue (most of good yield wafers were targeted at server and mainframe while mainstream still remain 130

nm 32-bit processor line until 2006) and soon became low end after Core 2 debuted. AMD released their first 64-bit mobile processor and manufactured in 90
...
announces ARMv8-A, the first 64-bit version of the
...
, which is the first 64-bit processor in a smartphone, the

A7

ARMv8-A-based system-on-a-chip, and the
...
, which are the first tablets with 64-bit processors.

**2014**

RISC-V

, with both 32-bit and 64-bit support, was published.
...
device to run on the 64-bit Tegra K1 chip.
...
, the first iPod Touch to use a 64-bit processor, the

A8

ARMv8-A-based system-on-a-chip, alongside the

Apple TV (4th generation)

, the first Apple TV to use a 64-bit processor.
...
, the first Apple Watch to use a 64-bit processor, the

S4

ARMv8-A-based system-on-a-chip.
...
, which lacks support for 32-bit applications.

**2023**

Qualcomm releases the
...
, which lack support for 32-bit ARM applications.
...
, the first 64-bit implementation of the
...
, a 64-bit operating system descended from
...
64-bit mainframes; 64-bit

Linux on z Systems

follows the CPU release almost immediately.

**2001**

Linux becomes the first OS kernel to fully support

x86-64

(on a simulator, as no x86-64 processors had been released yet).
...
's IA-64 architecture; it could run

32-bit applications

through an execution layer.
...
"Panther" operating system which adds support for native 64-bit integer arithmetic on
...
On January 4, Microsoft discontinues Windows XP 64-Bit Edition, as no PCs with IA-64 processors had been available since the previous September, and announces that it is developing x86-64 versions of Windows to replace it.

On January 31, Sun releases
...
"Tiger" which provides limited support for 64-bit command-line applications on machines with PowerPC 970 processors; later versions for Intel-based Macs supported 64-bit command-line applications on Macs with EM64T processors. On April 30, Microsoft releases
...
x64 Edition for AMD64 and EM64T processors.
...
, including a 64-bit version for AMD64/EM64T processors that retains 32-bit compatibility. In the 64-bit version, all Windows applications and components are 64-bit, although many also have their 32-bit versions included for compatibility with
...
"Leopard", which fully supports 64-bit applications on machines with PowerPC 970 or EM64T processors.
...
, which, like Windows Vista, includes a full 64-bit version for AMD64/Intel 64 processors; most new computers are loaded by default with a 64-bit version. Microsoft also releases
...
, "Snow Leopard", which ships with a 64-bit kernel for AMD64/Intel64 processors, although only certain recent models of Apple computers will run the 64-bit kernel by default. Most applications bundled with Mac OS X 10.6 are now also 64-bit.
...
, "Lion", which runs the 64-bit kernel by default on supported machines. Older machines that are unable to run the 64-bit kernel run the 32-bit kernel, but, as with earlier releases, can still run 64-bit applications; Lion does not support machines with 32-bit processors. Nearly all applications bundled with Mac OS X 10.7 are now also 64-bit, including iTunes.
...
, which makes the 64-bit kernel the default on some older previously unsupported machines and removes the 32-bit kernel.
...
, the first watchOS version to bring the 64-bit support.
...
"Catalina", dropping support for 32-bit Intel applications.
...
on October 5, which only supports 64-bit systems, dropping support for IA-32 and AArch32 systems.
...
, which drops support for 32-bit applications. Apple releases

watchOS 9

, the first watchOS version to run exclusively on the Apple Watch models with 64-bit processors (including Apple Watch Series 4 or newer,

Apple Watch SE (1st generation)

or newer and the newly introduced
...
, which drops support for 32-bit applications.
...
, ARM versions of which drop support for 32-bit ARM applications.
