...
> If there really were a machine with φ(*n*) ∼ *k*⋅*n* (or even ∼ *k*⋅*n*2), this would have consequences of the greatest importance. Namely, it would obviously mean that in spite of the undecidability of the Entscheidungsproblem, the mental work of a mathematician concerning Yes-or-No questions could be completely replaced by a machine. After all, one would simply have to choose the natural number *n* so large that when the machine does not deliver a result, it makes no sense to think more about the problem.
...
2. there exists $k\in N$ such that $T_{M}(n)\in O(n^{k})$ , where *O* refers to the big O notation and
...
1. *L* ∈ NP; and
2. any *L′* in NP is polynomial-time-reducible to *L* (written as $L'\leq _{p}L$ ), where $L'\leq _{p}L$ if, and only if, the following two conditions are satisfied:
...
- FPT vs. W[1] is an analogous problem in parameterized complexity.
