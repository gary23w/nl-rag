...
source: https://en.wikipedia.org/wiki/Reduction_(complexity)
...
- To show that a decision problem *A* is undecidable we must find a reduction from a decision problem that is already known to be undecidable to *A*. That reduction function must be a computable function. In particular, we often show that a problem *A* is undecidable by showing that the halting problem reduces to *A*.
