---
title: "Arthur–Merlin protocol"
source: https://en.wikipedia.org/wiki/Arthur%E2%80%93Merlin_protocol
...
- if *x* is not in *L*, then $\Pr \nolimits _{y\in \{0,1\}^{p(n)}}(\forall z\in \{0,1\}^{q(n)}\,M(x,y,z)=1)\leq 1/3.$
...
- **AM** is the same as the class **BP⋅NP** where **BP** denotes the bounded-error probabilistic operator. Also, **$\exists \cdot {\mathsf {BPP}}$** ( also written as **ExistsBPP**) is a subset of **MA**. Whether **MA** is equal to **$\exists \cdot {\mathsf {BPP}}$** is an open question.
...
- Both **MA** and **AM** are contained in the polynomial hierarchy. In particular, **MA** is contained in the intersection of Σ2P and Π2P and **AM** is contained in Π2P. Even more, **MA** is contained in subclass **SP 2**, a complexity class expressing "symmetric alternation". This is a generalization of Sipser–Lautemann theorem.
...
- It is known, assuming ERH, that for any *d* the problem "Given a collection of multivariate polynomials $f_{i}$ each with integer coefficients and of degree at most *d*, do they have a common complex zero?" is in **AM**.
