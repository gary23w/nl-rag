...
To demonstrate the power of these classes, consider the graph isomorphism problem, the problem of determining whether it is possible to permute the vertices of one graph so that it is identical to another graph. This problem is in **NP**, since the proof certificate is the permutation that makes the graphs equal. It turns out that the complement of the graph isomorphism problem, a co-**NP** problem not known to be in **NP**, has an **AM** algorithm and the best way to see it is via a private coins algorithm.
