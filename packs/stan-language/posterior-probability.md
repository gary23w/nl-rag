...
In Bayesian statistics, the posterior probability is the probability distribution of the parameters $\theta$ given the evidence X , and is denoted $p(\theta |X)$ .
...
for continuous $\theta$ , or by summing $p(x|\theta )p(\theta )$ over all possible values of $\theta$ for discrete $\theta$ .
