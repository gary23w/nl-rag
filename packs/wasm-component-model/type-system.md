...
The following table gives an overview over type theoretic concepts that are used in specialized type systems. The names M, N, O range over terms and the names $\sigma ,\tau$ range over types. The following notation will be used:

- $M:\sigma$ means that M has type $\sigma$ ;
- $M(N)$ is that application of M on N ;
- $\tau [\alpha :=\sigma ]$ (resp. $\tau [x:=N]$ ) describes the type which results from replacing all occurrences of the type variable α (resp. term variable x) in $\tau$ by the type σ (resp. term N).

| Type notion | Notation | Meaning |
|---|---|---|
| Function | $\sigma \to \tau$ | If $M:\sigma \to \tau$ and $N:\sigma$ , then $M(N):\tau$ . |
| Product | $\sigma \times \tau$ | If $M:\sigma \times \tau$ , then $M=(N,O)$ is a pair s.t. $N:\sigma$ and $O:\tau$ . |
| Sum | $\sigma +\tau$ | If $M:\sigma +\tau$ , then $M=\iota _{1}(N)$ is the first injection s.t. $N:\sigma$ , or $M=\iota _{2}(N)$ is the second injection s.t. $N:\tau$ . |
| Intersection | $\sigma \cap \tau$ | If $M:\sigma \cap \tau$ , then $M:\sigma$ and $M:\tau$ . |
| Union | $\sigma \cup \tau$ | If $M:\sigma \cup \tau$ , then $M:\sigma$ or $M:\tau$ . |
| Record | $\langle x:\tau \rangle$ | If $M:\langle x:\tau \rangle$ , then M has a member $x:\tau$ . |
| Polymorphic | $\forall {}\alpha .\tau$ | If $M:\forall {}\alpha .\tau$ , then $M:\tau [\alpha :=\sigma ]$ for any type σ. |
| Existential | $\exists {}\alpha .\tau$ | If $M:\exists {}\alpha .\tau$ , then $M:\tau [\alpha :=\sigma ]$ for some type σ. |
| Recursive | $\mu \alpha .\tau$ | If $M:\mu \alpha .\tau$ , then $M:\tau [\alpha :=\mu \alpha .\tau ]$ . |
| Dependent function | $(x:\sigma )\to \tau$ | If $M:(x:\sigma )\to \tau$ and $N:\sigma$ , then $M(N):\tau [x:=N]$ . |
| Dependent pair | $(x:\sigma )\times \tau$ | If $M:(x:\sigma )\times \tau$ , then $M=(N,O)$ is a pair s.t. $N:\sigma$ and $O:\tau [x:=N]$ . |
| Dependent intersection | $(x:\sigma )\cap \tau$ | If $M:(x:\sigma )\cap \tau$ , then $M:\sigma$ and $M:\tau [x:=M]$ . |
| Familial intersection | $\bigcap _{x:\sigma }\tau$ | If ${\textstyle M:\bigcap _{x:\sigma }\tau }$ , then $M:\tau [x:=N]$ for any term $N:\sigma$ . |
| Familial union | $\bigcup _{x:\sigma }\tau$ | If ${\textstyle M:\bigcup _{x:\sigma }\tau }$ , then $M:\tau [x:=N]$ for some term $N:\sigma$ . |

1. Also referred to as *dependent product type*, since ${\textstyle (x:\sigma )\to \tau =\prod _{x:\sigma }\tau }$ .
2. Also referred to as *dependent sum type*, since ${\textstyle (x:\sigma )\times \tau =\sum _{x:\sigma }\tau }$ .

### Dependent types

Dependent types are based on the idea of using scalars or values to more precisely describe the type of some other value. For example, $\mathrm {matrix} (3,3)$ might be the type of a $3\times 3$ matrix. We can then define typing rules such as the following rule for matrix multiplication:
...
- Given a type environment $\Gamma$ , a term e , and a type $\tau$ , decide whether the term e can be assigned the type $\tau$ in the type environment.
...
- Given a type environment $\Gamma$ and a type $\tau$ , decide whether there exists a term e that can be assigned the type $\tau$ in the type environment.
