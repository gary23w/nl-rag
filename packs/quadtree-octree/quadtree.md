...
The region quadtree represents a partition of space in two dimensions by decomposing the region into four equal quadrants, subquadrants, and so on with each leaf node containing data corresponding to a specific subregion. It can be also interpreted as a method for "subpaving the region", based on set of hierarchical grids; or as a method of image compression, when an image is discretizated as a mosaic of "equal color regions".
