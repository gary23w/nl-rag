...
Each subtree associates a graph vertex with a set of tree nodes. To define this formally, we represent each tree node as the set of vertices associated with it. Thus, given a graph *G* = (*V*, *E*), a tree decomposition is a pair (*X*, *T*), where *X* = {*X*1, …, *Xn*} is a family of subsets (sometimes called *bags*) of V, and T is a tree whose nodes are the subsets Xi, satisfying the following properties:
...
A tree decomposition (*X*, *T* = (*I*, *F*)) of treewidth k is *smooth*, if for all $i\in I:|X_{i}|=k+1$ , and for all $(i,j)\in F:|X_{i}\cap X_{j}|=k$ .
