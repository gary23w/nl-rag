...
Snapshots are recommended for backups. Backups made without snapshots run the risk of losing data that was recorded since the last TSDB block was created, which typically happens every two hours, covering the last three hours of samples. Excluding the WAL files (the `chunks_head/`, `wal/`, and `wbl/` directories in `storage.tsdb.path`) on backup or restore will ensure a coherent backup, in any case, at the cost of losing the time range covered by the WAL files.
