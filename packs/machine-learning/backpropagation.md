...
- x {\displaystyle x} ({\displaystyle x}): input (vector of features)
- y {\displaystyle y} ({\displaystyle y}): target output For classification, output will be a vector of class probabilities (e.g., ( 0.1 , 0.7 , 0.2 ) {\displaystyle (0.1,0.7,0.2)} ({\displaystyle (0.1,0.7,0.2)}), and target output is a specific class, encoded by the one-hot/dummy variable (e.g., ( 0 , 1 , 0 ) {\displaystyle (0,1,0)} ({\displaystyle (0,1,0)})).
- C {\displaystyle C} ({\displaystyle C}): loss function or "cost function" For classification, this is usually cross-entropy (XC, log loss), while for regression it is usually squared error loss (SEL).
- L {\displaystyle L} ({\displaystyle L}): the number of layers
- W l = ( w j k l ) {\displaystyle W^{l}=(w_{jk}^{l})} ({\displaystyle W^{l}=(w_{jk}^{l})}): the weights between layer l − 1 {\displaystyle l-1} ({\displaystyle l-1}) and l {\displaystyle l} ({\displaystyle l}), where w j k l {\displaystyle w_{jk}^{l}} ({\displaystyle w_{jk}^{l}}) is the weight between the k {\displaystyle k} ({\displaystyle k})-th node in layer l − 1 {\displaystyle l-1} ({\displaystyle l-1}) and the j {\displaystyle j} ({\displaystyle j})-th node in layer l {\displaystyle l} ({\displaystyle l})
- f l {\displaystyle f^{l}} ({\displaystyle f^{l}}): activation functions at layer l {\displaystyle l} ({\displaystyle l}) For classification the last layer is usually the logistic function for binary classification, and softmax (softargmax) for multi-class classification, while for the hidden layers this was traditionally a sigmoid function (logistic function or others) on each node (coordinate), but today is more varied, with rectifier (ramp, ReLU) being common.
- a j l {\displaystyle a_{j}^{l}} ({\displaystyle a_{j}^{l}}): activation of the j {\displaystyle j} ({\displaystyle j})-th node in layer l {\displaystyle l} ({\displaystyle l}).
...
For a training set there will be a set of input–output pairs, { ( x i , y i ) } {\displaystyle \left\{(x_{i},y_{i})\right\}} ({\displaystyle \left\{(x_{i},y_{i})\right\}}). For each input–output pair ( x i , y i ) {\displaystyle (x_{i},y_{i})} ({\displaystyle (x_{i},y_{i})}) in the training set, the loss of the model on that pair is the cost of the difference between the predicted output g ( x i ) {\displaystyle g(x_{i})} ({\displaystyle g(x_{i})}) and the target output y i {\displaystyle y_{i}} ({\displaystyle y_{i}}):
...
Backpropagation computes the gradient for a *fixed* input–output pair ( x i , y i ) {\displaystyle (x_{i},y_{i})} ({\displaystyle (x_{i},y_{i})}), where the weights w j k l {\displaystyle w_{jk}^{l}} ({\displaystyle w_{jk}^{l}}) can vary. Each individual component of the gradient, ∂ C / ∂ w j k l , {\displaystyle \partial C/\partial w_{jk}^{l},} ({\displaystyle \partial C/\partial w_{jk}^{l},}) can be computed by the chain rule; but doing this separately for each weight is inefficient. Backpropagation efficiently computes the gradient by avoiding duplicate calculations and not computing unnecessary intermediate values, by computing the gradient of each layer – specifically the gradient of the weighted *input* of each layer, denoted by δ l {\displaystyle \delta ^{l}} ({\displaystyle \delta ^{l}}) – from back to front.

Informally, the key point is that since the only way a weight in W l {\displaystyle W^{l}} ({\displaystyle W^{l}}) affects the loss is through its effect on the *next* layer, and it does so *linearly*, δ l {\displaystyle \delta ^{l}} ({\displaystyle \delta ^{l}}) are the only data you need to compute the gradients of the weights at layer l {\displaystyle l} ({\displaystyle l}), and then the gradients of weights of previous layer can be computed by δ l − 1 {\displaystyle \delta ^{l-1}} ({\displaystyle \delta ^{l-1}}) and repeated recursively. This avoids inefficiency in two ways. First, it avoids duplication because when computing the gradient at layer l {\displaystyle l} ({\displaystyle l}), it is unnecessary to recompute all derivatives on later layers l + 1 , l + 2 , … {\displaystyle l+1,l+2,\ldots } ({\displaystyle l+1,l+2,\ldots }) each time. Second, it avoids unnecessary intermediate calculations, because at each stage it directly computes the gradient of the weights with respect to the ultimate output (the loss), rather than unnecessarily computing the derivatives of the values of hidden layers with respect to changes in weights ∂ a j ′ l ′ / ∂ w j k l {\displaystyle \partial a_{j'}^{l'}/\partial w_{jk}^{l}} ({\displaystyle \partial a_{j'}^{l'}/\partial w_{jk}^{l}}).
...
Given an input–output pair ( x , y ) {\displaystyle (x,y)} ({\displaystyle (x,y)}), the loss is:
...
To compute this, one starts with the input x {\displaystyle x} ({\displaystyle x}) and works forward; denote the weighted input of each hidden layer as z l {\displaystyle z^{l}} ({\displaystyle z^{l}}) and the output of hidden layer l {\displaystyle l} ({\displaystyle l}) as the activation a l {\displaystyle a^{l}} ({\displaystyle a^{l}}). For backpropagation, the activation a l {\displaystyle a^{l}} ({\displaystyle a^{l}}) as well as the derivatives ( f l ) ′ {\displaystyle (f^{l})'} ({\displaystyle (f^{l})'}) (evaluated at z l {\displaystyle z^{l}} ({\displaystyle z^{l}})) must be cached for use during the backwards pass.

The derivative of the loss in terms of the inputs is given by the chain rule; note that each term is a total derivative, evaluated at the value of the network (at each node) on the input x {\displaystyle x} ({\displaystyle x}):
...
where d a L d z L {\displaystyle {\frac {da^{L}}{dz^{L}}}} ({\displaystyle {\frac {da^{L}}{dz^{L}}}}) is a diagonal matrix.
...
Introducing the auxiliary quantity δ l {\displaystyle \delta ^{l}} ({\displaystyle \delta ^{l}}) for the partial products (multiplying from right to left), interpreted as the "error at level l {\displaystyle l} ({\displaystyle l})" and defined as the gradient of the input values at level l {\displaystyle l} ({\displaystyle l}):
...
Note that δ l {\displaystyle \delta ^{l}} ({\displaystyle \delta ^{l}}) is a vector, of length equal to the number of nodes in level l {\displaystyle l} ({\displaystyle l}); each component is interpreted as the "cost attributable to (the value of) that node".

The gradient of the weights in layer l {\displaystyle l} ({\displaystyle l}) is then:
...
The factor of a l − 1 {\displaystyle a^{l-1}} ({\displaystyle a^{l-1}}) is because the weights W l {\displaystyle W^{l}} ({\displaystyle W^{l}}) between level l − 1 {\displaystyle l-1} ({\displaystyle l-1}) and l {\displaystyle l} ({\displaystyle l}) affect level l {\displaystyle l} ({\displaystyle l}) proportionally to the inputs (activations): the inputs are fixed, the weights vary.

The δ l {\displaystyle \delta ^{l}} ({\displaystyle \delta ^{l}}) can easily be computed recursively, going from right to left, as:
...
Compared with naively computing forwards (using the δ l {\displaystyle \delta ^{l}} ({\displaystyle \delta ^{l}}) for illustration):
...
1. Computing δ l − 1 {\displaystyle \delta ^{l-1}} ({\displaystyle \delta ^{l-1}}) in terms of δ l {\displaystyle \delta ^{l}} ({\displaystyle \delta ^{l}}) avoids the obvious duplicate multiplication of layers l {\displaystyle l} ({\displaystyle l}) and beyond.
2. Multiplying starting from ∇ a L C {\displaystyle \nabla _{a^{L}}C} ({\displaystyle \nabla _{a^{L}}C}) – propagating the error *backwards* – means that each step simply multiplies a vector ( δ l {\displaystyle \delta ^{l}} ({\displaystyle \delta ^{l}})) by the matrices of weights ( W l ) T {\displaystyle (W^{l})^{T}} ({\displaystyle (W^{l})^{T}}) and derivatives of activations ( f l − 1 ) ′ {\displaystyle (f^{l-1})'} ({\displaystyle (f^{l-1})'}). By contrast, multiplying forwards, starting from the changes at an earlier layer, means that each multiplication multiplies a *matrix* by a *matrix*. This is much more expensive, and corresponds to tracking every possible path of a change in one layer l {\displaystyle l} ({\displaystyle l}) forward to changes in the layer l + 2 {\displaystyle l+2} ({\displaystyle l+2}) (for multiplying W l + 1 {\displaystyle W^{l+1}} ({\displaystyle W^{l+1}}) by W l + 2 {\displaystyle W^{l+2}} ({\displaystyle W^{l+2}}), with additional multiplications for the derivatives of the activations), which unnecessarily computes the intermediate quantities of how weight changes affect the values of hidden nodes.
...
Initially, before training, the weights will be set randomly. Then the neuron learns from training examples, which in this case consist of a set of tuples ( x 1 , x 2 , t ) {\displaystyle (x_{1},x_{2},t)} ({\displaystyle (x_{1},x_{2},t)}) where x 1 {\displaystyle x_{1}} ({\displaystyle x_{1}}) and x 2 {\displaystyle x_{2}} ({\displaystyle x_{2}}) are the inputs to the network and t is the correct output (the output the network should produce given those inputs, when it has been trained). The initial network, given x 1 {\displaystyle x_{1}} ({\displaystyle x_{1}}) and x 2 {\displaystyle x_{2}} ({\displaystyle x_{2}}), will compute an output y that likely differs from t (given random weights). A loss function L ( t , y ) {\displaystyle L(t,y)} ({\displaystyle L(t,y)}) is used for measuring the discrepancy between the target output t and the computed output y. For regression analysis problems the squared error can be used as a loss function, for classification the categorical cross-entropy can be used.
...
where w 1 {\displaystyle w_{1}} ({\displaystyle w_{1}}) and w 2 {\displaystyle w_{2}} ({\displaystyle w_{2}}) are the weights on the connection from the input units to the output unit. Therefore, the error also depends on the incoming weights to the neuron, which is ultimately what needs to be changed in the network to enable learning.

In this example, upon injecting the training data ( 1 , 1 , 0 ) {\displaystyle (1,1,0)} ({\displaystyle (1,1,0)}), the loss function becomes

E = ( t − y ) 2 = y 2 = ( x 1 w 1 + x 2 w 2 ) 2 = ( w 1 + w 2 ) 2 . {\displaystyle E=(t-y)^{2}=y^{2}=(x_{1}w_{1}+x_{2}w_{2})^{2}=(w_{1}+w_{2})^{2}.} ({\displaystyle E=(t-y)^{2}=y^{2}=(x_{1}w_{1}+x_{2}w_{2})^{2}=(w_{1}+w_{2})^{2}.})

Then, the loss function E {\displaystyle E} ({\displaystyle E}) takes the form of a parabolic cylinder with its base directed along w 1 = − w 2 {\displaystyle w_{1}=-w_{2}} ({\displaystyle w_{1}=-w_{2}}). Since all sets of weights that satisfy w 1 = − w 2 {\displaystyle w_{1}=-w_{2}} ({\displaystyle w_{1}=-w_{2}}) minimize the loss function, in this case additional constraints are required to converge to a unique solution. Additional constraints could either be generated by setting specific conditions to the weights, or by injecting additional training data.
...
In this section, the order of the weight indexes are reversed relative to the prior section: w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}) is weight from the i {\displaystyle i} ({\displaystyle i})th to the j {\displaystyle j} ({\displaystyle j})th unit. For each neuron j {\displaystyle j} ({\displaystyle j}), its output o j {\displaystyle o_{j}} ({\displaystyle o_{j}}) is defined as
...
where the activation function φ {\displaystyle \varphi } ({\displaystyle \varphi }) is non-linear and differentiable over the activation region (the ReLU is not differentiable at one point). A historically used activation function is the logistic function:
...
The input net j {\displaystyle {\text{net}}_{j}} ({\displaystyle {\text{net}}_{j}}) to a neuron is the weighted sum of outputs o k {\displaystyle o_{k}} ({\displaystyle o_{k}}) of previous neurons. If the neuron is in the first layer after the input layer, the o k {\displaystyle o_{k}} ({\displaystyle o_{k}}) of the input layer are simply the inputs x k {\displaystyle x_{k}} ({\displaystyle x_{k}}) to the network. The number of input units to the neuron is n {\displaystyle n} ({\displaystyle n}). The variable w k j {\displaystyle w_{kj}} ({\displaystyle w_{kj}}) denotes the weight between neuron k {\displaystyle k} ({\displaystyle k}) of the previous layer and neuron j {\displaystyle j} ({\displaystyle j}) of the current layer.

### Finding the derivative of the error

Calculating the partial derivative of the error with respect to a weight w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}) is done using the chain rule twice:

| ∂ E ∂ w i j = ∂ E ∂ o j ∂ o j ∂ w i j = ∂ E ∂ o j ∂ o j ∂ net j ∂ net j ∂ w i j {\displaystyle {\frac {\partial E}{\partial w_{ij}}}={\frac {\partial E}{\partial o_{j}}}{\frac {\partial o_{j}}{\partial w_{ij}}}={\frac {\partial E}{\partial o_{j}}}{\frac {\partial o_{j}}{\partial {\text{net}}_{j}}}{\frac {\partial {\text{net}}_{j}}{\partial w_{ij}}}} ({\displaystyle {\frac {\partial E}{\partial w_{ij}}}={\frac {\partial E}{\partial o_{j}}}{\frac {\partial o_{j}}{\partial w_{ij}}}={\frac {\partial E}{\partial o_{j}}}{\frac {\partial o_{j}}{\partial {\text{net}}_{j}}}{\frac {\partial {\text{net}}_{j}}{\partial w_{ij}}}}) |   | Eq. 1 |
|---|---|---|

In the last factor of the right-hand side of the above, only one term in the sum net j {\displaystyle {\text{net}}_{j}} ({\displaystyle {\text{net}}_{j}}) depends on w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}), so that

| ∂ net j ∂ w i j = ∂ ∂ w i j ( ∑ k = 1 n w k j o k ) = ∂ ∂ w i j w i j o i = o i . {\displaystyle {\frac {\partial {\text{net}}_{j}}{\partial w_{ij}}}={\frac {\partial }{\partial w_{ij}}}\left(\sum _{k=1}^{n}w_{kj}o_{k}\right)={\frac {\partial }{\partial w_{ij}}}w_{ij}o_{i}=o_{i}.} ({\displaystyle {\frac {\partial {\text{net}}_{j}}{\partial w_{ij}}}={\frac {\partial }{\partial w_{ij}}}\left(\sum _{k=1}^{n}w_{kj}o_{k}\right)={\frac {\partial }{\partial w_{ij}}}w_{ij}o_{i}=o_{i}.}) |   | Eq. 2 |
|---|---|---|

If the neuron is in the first layer after the input layer, o i {\displaystyle o_{i}} ({\displaystyle o_{i}}) is just x i {\displaystyle x_{i}} ({\displaystyle x_{i}}).

The derivative of the output of neuron j {\displaystyle j} ({\displaystyle j}) with respect to its input is simply the partial derivative of the activation function:

| ∂ o j ∂ net j = ∂ φ ( net j ) ∂ net j {\displaystyle {\frac {\partial o_{j}}{\partial {\text{net}}_{j}}}={\frac {\partial \varphi ({\text{net}}_{j})}{\partial {\text{net}}_{j}}}} ({\displaystyle {\frac {\partial o_{j}}{\partial {\text{net}}_{j}}}={\frac {\partial \varphi ({\text{net}}_{j})}{\partial {\text{net}}_{j}}}}) |   | Eq. 3 |
|---|---|---|
...
The first factor is straightforward to evaluate if the neuron is in the output layer, because then o j = y {\displaystyle o_{j}=y} ({\displaystyle o_{j}=y}) and

| ∂ E ∂ o j = ∂ E ∂ y {\displaystyle {\frac {\partial E}{\partial o_{j}}}={\frac {\partial E}{\partial y}}} ({\displaystyle {\frac {\partial E}{\partial o_{j}}}={\frac {\partial E}{\partial y}}}) |   | Eq. 4 |
|---|---|---|
...
However, if j {\displaystyle j} ({\displaystyle j}) is in an arbitrary inner layer of the network, finding the derivative E {\displaystyle E} ({\displaystyle E}) with respect to o j {\displaystyle o_{j}} ({\displaystyle o_{j}}) is less obvious.

Considering E {\displaystyle E} ({\displaystyle E}) as a function with the inputs being all neurons L = { u , v , … , w } {\displaystyle L=\{u,v,\dots ,w\}} ({\displaystyle L=\{u,v,\dots ,w\}}) receiving input from neuron j {\displaystyle j} ({\displaystyle j}),
...
and taking the total derivative with respect to o j {\displaystyle o_{j}} ({\displaystyle o_{j}}), a recursive expression for the derivative is obtained:

| ∂ E ∂ o j = ∑ ℓ ∈ L ( ∂ E ∂ net ℓ ∂ net ℓ ∂ o j ) = ∑ ℓ ∈ L ( ∂ E ∂ o ℓ ∂ o ℓ ∂ net ℓ ∂ net ℓ ∂ o j ) = ∑ ℓ ∈ L ( ∂ E ∂ o ℓ ∂ o ℓ ∂ net ℓ w j ℓ ) {\displaystyle {\frac {\partial E}{\partial o_{j}}}=\sum _{\ell \in L}\left({\frac {\partial E}{\partial {\text{net}}_{\ell }}}{\frac {\partial {\text{net}}_{\ell }}{\partial o_{j}}}\right)=\sum _{\ell \in L}\left({\frac {\partial E}{\partial o_{\ell }}}{\frac {\partial o_{\ell }}{\partial {\text{net}}_{\ell }}}{\frac {\partial {\text{net}}_{\ell }}{\partial o_{j}}}\right)=\sum _{\ell \in L}\left({\frac {\partial E}{\partial o_{\ell }}}{\frac {\partial o_{\ell }}{\partial {\text{net}}_{\ell }}}w_{j\ell }\right)} ({\displaystyle {\frac {\partial E}{\partial o_{j}}}=\sum _{\ell \in L}\left({\frac {\partial E}{\partial {\text{net}}_{\ell }}}{\frac {\partial {\text{net}}_{\ell }}{\partial o_{j}}}\right)=\sum _{\ell \in L}\left({\frac {\partial E}{\partial o_{\ell }}}{\frac {\partial o_{\ell }}{\partial {\text{net}}_{\ell }}}{\frac {\partial {\text{net}}_{\ell }}{\partial o_{j}}}\right)=\sum _{\ell \in L}\left({\frac {\partial E}{\partial o_{\ell }}}{\frac {\partial o_{\ell }}{\partial {\text{net}}_{\ell }}}w_{j\ell }\right)}) |   | Eq. 5 |
|---|---|---|

Therefore, the derivative with respect to o j {\displaystyle o_{j}} ({\displaystyle o_{j}}) can be calculated if all the derivatives with respect to the outputs o ℓ {\displaystyle o_{\ell }} ({\displaystyle o_{\ell }}) of the next layer – the ones closer to the output neuron – are known. [Note, if any of the neurons in set L {\displaystyle L} ({\displaystyle L}) were not connected to neuron j {\displaystyle j} ({\displaystyle j}), they would be independent of w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}) and the corresponding partial derivative under the summation would vanish to 0.]
...
if φ {\displaystyle \varphi } ({\displaystyle \varphi }) is the logistic function, and the error is the square error:
...
To update the weight w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}) using gradient descent, one must choose a learning rate, η > 0 {\displaystyle \eta >0} ({\displaystyle \eta >0}). The change in weight needs to reflect the impact on E {\displaystyle E} ({\displaystyle E}) of an increase or decrease in w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}). If ∂ E ∂ w i j > 0 {\displaystyle {\frac {\partial E}{\partial w_{ij}}}>0} ({\displaystyle {\frac {\partial E}{\partial w_{ij}}}>0}), an increase in w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}) increases E {\displaystyle E} ({\displaystyle E}); conversely, if ∂ E ∂ w i j < 0 {\displaystyle {\frac {\partial E}{\partial w_{ij}}}<0} ({\displaystyle {\frac {\partial E}{\partial w_{ij}}}<0}), an increase in w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}) decreases E {\displaystyle E} ({\displaystyle E}). The new Δ w i j {\displaystyle \Delta w_{ij}} ({\displaystyle \Delta w_{ij}}) is added to the old weight, and the product of the learning rate and the gradient, multiplied by − 1 {\displaystyle -1} ({\displaystyle -1}) guarantees that w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}) changes in a way that always decreases E {\displaystyle E} ({\displaystyle E}). In other words, in the equation immediately below, − η ∂ E ∂ w i j {\displaystyle -\eta {\frac {\partial E}{\partial w_{ij}}}} ({\displaystyle -\eta {\frac {\partial E}{\partial w_{ij}}}}) always changes w i j {\displaystyle w_{ij}} ({\displaystyle w_{ij}}) in such a way that E {\displaystyle E} ({\displaystyle E}) is decreased:
...
The mathematical expression of the loss function must fulfill two conditions in order for it to be possibly used in backpropagation. The first is that it can be written as an average E = 1 n ∑ x E x {\textstyle E={\frac {1}{n}}\sum _{x}E_{x}} ({\textstyle E={\frac {1}{n}}\sum _{x}E_{x}}) over error functions E x {\textstyle E_{x}} ({\textstyle E_{x}}), for n {\textstyle n} ({\textstyle n}) individual training examples, x {\textstyle x} ({\textstyle x}). The reason for this assumption is that the backpropagation algorithm calculates the gradient of the error function for a single training example, which needs to be generalized to the overall error function. The second assumption is that it can be written as a function of the outputs from the neural network.

### Example loss function

Let y , y ′ {\displaystyle y,y'} ({\displaystyle y,y'}) be vectors in R n {\displaystyle \mathbb {R} ^{n}} ({\displaystyle \mathbb {R} ^{n}}).

Select an error function E ( y , y ′ ) {\displaystyle E(y,y')} ({\displaystyle E(y,y')}) measuring the difference between two outputs. The standard choice is the square of the Euclidean distance between the vectors y {\displaystyle y} ({\displaystyle y}) and y ′ {\displaystyle y'} ({\displaystyle y'}): E ( y , y ′ ) = 1 2 ‖ y − y ′ ‖ 2 {\displaystyle E(y,y')={\tfrac {1}{2}}\lVert y-y'\rVert ^{2}} ({\displaystyle E(y,y')={\tfrac {1}{2}}\lVert y-y'\rVert ^{2}})The error function over n {\textstyle n} ({\textstyle n}) training examples can then be written as an average of losses over individual examples: E = 1 2 n ∑ x ‖ ( y ( x ) − y ′ ( x ) ) ‖ 2 {\displaystyle E={\frac {1}{2n}}\sum _{x}\lVert (y(x)-y'(x))\rVert ^{2}} ({\displaystyle E={\frac {1}{2n}}\sum _{x}\lVert (y(x)-y'(x))\rVert ^{2}})
