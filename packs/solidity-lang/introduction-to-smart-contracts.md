...
source: https://docs.soliditylang.org/en/latest/introduction-to-smart-contracts.html
...
The line `address public minter;` declares a state variable of type address. The `address` type is a 160-bit value that does not allow any arithmetic operations. It is suitable for storing addresses of contracts, or a hash of the public half of a keypair belonging to externally-owned accounts.
...
The `send` function can be used by anyone (who already has some of these coins) to send coins to anyone else. If the sender does *not* have enough coins to send, the condition in `require` evaluates to false, triggering a `revert` with the `InsufficientBalance` error. This error supplies the requested amount and available balance to the caller, which front-end applications or block explorers can surface for debugging.
