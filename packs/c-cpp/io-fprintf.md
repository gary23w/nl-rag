...
Writes the results to a character string

buffer

. The behavior is undefined if the string to be written (plus the terminating null character) exceeds the size of the array pointed to by
...
characters are written. The resulting character string will be terminated with a null character, unless
...
- For the `**z**` modifier, the expected argument type is the signed version of size_t.
...
Converts an **unsigned integer** into binary representation *bbbb*.

- *Precision* specifies the minimum number of digits to appear. The default precision is `1`.
- If both the converted value and the precision are `0` the conversion results in no characters.
- In the *alternative implementation* `**0b**` or `**0B**` is prefixed to results if the converted value is nonzero.
...
- Let `P` equal the precision if nonzero, `6` if the precision is not specified, or `1` if the precision is `0`. Then, if a conversion with style `E` would have an exponent of `X`:
  - If *P > X ≥ −4*, the conversion is with style `**f**` or `**F**`(since C99) and precision *P − 1 − X*.
...
- For the `**z**` modifier, the expected argument type is `S*`, where `S` is the signed version of `size_t`.
...
number of characters (not including the terminating null character) which would have been written to
...
number of characters transmitted to the output stream or negative value if an output error, a runtime constraints violation error, or an encoding error occurred.
...
, not counting the null character (which is always written as long as
...
is not zero and not greater than
...
), which would have been written to
