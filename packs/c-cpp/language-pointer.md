...
as a pointer to the type determined by the
...
as a pointer to non-static member of

C

of type determined by the declaration specifier sequence

S

.

| nested-name-specifier | - | a sequence of names and scope resolution operators `**::**`. It must designate a class type that is not an anonymous union |
|---|---|---|
