...
in which the interval between probes increases linearly (hence, the indices are described by a
...
is such a subsequent record.

k

is the raw hash where the record at
...
$EU_{n}={\frac {1}{2}}\left(1+{\frac {1}{1-\alpha }}\right)+\Theta ({\frac {1}{m}})$

$ES_{n}={\frac {1}{2}}\left(1+{\frac {1}{(1-\alpha )^{2}}}\right)+\Theta ({\frac {1}{m}})$

Here *n* is the number of elements in the table, *m* the table size, $\alpha =n/m$ the load factor, $U_{n}$ the number of probes in an unsuccessful search and $S_{n}$ the number of probes in a successful search. Note that the expectation deteriorates to infinity when the load factor approaches 1.
