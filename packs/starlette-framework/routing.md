...
`Route('/users/{username}', user)` By default this will capture characters up to the end of the path or the next `/`.
...
Path parameters, and reverse URL lookups for `WebSocketRoute` work the same as HTTP `Route`, which can be found in the HTTP Route section above.
