...
- This will find both buttons and throw a strictness violation error: SyncAsync`page.locator("button").click()``await page.locator("button").click()`
- This will only find a second button, because it is visible, and then click it. SyncAsync`page.locator("button").filter(visible=True).click()``await page.locator("button").filter(visible=True).click()`
