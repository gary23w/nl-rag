...
, while "Motor" represents the actual output to the real-world
...
| --[\]----[\]----+--[ ]--+---------( ) ES Stop \| Start \| Run \| \| +--[ ]--+ Run -------[ ]--------------( ) Run Motor The above realizes the function: Run = (NOT ES) AND (NOT Stop) AND (Start OR Run) |
|---|

Note, that when being really safe, the physical contacts for stop and emergency stop are wired such that the PLC is given HIGH (true) inputs, when the buttons are NOT pressed, hence the PLC logic should be reversed for those. In this case, the ladder diagram will look like this:

| --[ ]----[ ]----+--[ ]--+---------( ) ES Stop \| Start \| Run \| \| +--[ ]--+ Run -------[ ]--------------( ) Run Motor The above realizes the function: Run = ES AND Stop AND (Start OR Run) |
|---|
