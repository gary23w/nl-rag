...
source: https://en.wikipedia.org/wiki/Amazon_ElastiCache
...
ElastiCache for Redis supports publish-subscribe patterns, which enable the creation of chat rooms and message boards where messages are automatically distributed to interested users.
...
Many ElastiCache metrics can be collected from AWS via CloudWatch or directly from the cache engine, whether Redis or Memcached, with a monitoring tool integrating with it:
...
Users of Amazon ElastiCache include Airbnb, Expedia, Zynga, Tinder, FanDuel, and Mapbox
