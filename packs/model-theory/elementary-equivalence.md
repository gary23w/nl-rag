...
*N* is an **elementary substructure** or **elementary submodel** of *M* if *N* and *M* are structures of the same signature *σ* such that for all first-order *σ*-formulas *φ*(*x*1, …, *x**n*) with free variables *x*1, …, *x**n*, and all elements *a*1, …, *a*n of *N*, *φ*(*a*1, …, *a*n) holds in *N* if and only if it holds in *M*: $N\models \varphi (a_{1},\dots ,a_{n}){\text{ if and only if }}M\models \varphi (a_{1},\dots ,a_{n}).$
...
Let *M* be a structure of signature *σ* and *N* a substructure of *M*. Then *N* is an elementary substructure of *M* if and only if for every first-order formula *φ*(*x*, *y*1, …, *y**n*) over *σ* and all elements *b*1, …, *b**n* from *N*, if *M* $\models$ $\exists$ *x* *φ*(*x*, *b*1, …, *b**n*), then there is an element *a* in *N* such that *M* $\models$ *φ*(*a*, *b*1, …, *b**n*).
