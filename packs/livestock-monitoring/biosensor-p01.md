---
title: "Biosensor (part 1/2)"
source: https://en.wikipedia.org/wiki/Biosensor
domain: livestock-monitoring
...
A **biosensor** (or **nanobiosensor** in the nanoscale) is an analytical device, used for the detection of a chemical substance, that combines a biological component with a physicochemical detector.
...
Electrochemical biosensors combine the exquisite selectivity of biological recognition elements with the high sensitivity and rapid response times of electrochemical transduction. To understand their design and application, these systems can be systematically categorized by their electrical measurement techniques and their biorecognition mechanisms.

### 1. Classification by Electrochemical Measurement Technique

Electrochemical biosensors are broadly divided into **active techniques** (which force a current or voltage change upon the system to measure a dynamic response) and **passive techniques** (which measure a system at equilibrium or under resting conditions).

#### A. Active Measurement Techniques

- **Amperometric and Voltammetric Biosensors:** These systems apply a controlled electrical potential to a working electrode relative to a reference electrode. This potential drives an oxidation or reduction reaction of an electroactive species at the electrode surface. The resulting current is directly proportional to the concentration of the target analyte.
  - *Three-Electrode Configuration:* This is the traditional laboratory standard. It utilizes a **Working Electrode** (where the specific bio-reaction occurs), a **Reference Electrode** (which maintains a stable, known potential), and a **Counter/Auxiliary Electrode** (which carries the current so that no significant current passes through the reference electrode, preserving its stability).
  - *Two-Electrode Configuration:* While three-electrode setups offer superior potential control, **two-electrode amperometric sensors** are highly successful, robust, and dominant in commercial point-of-care devices (such as digital glucose meters). By modern microfabrication and screen-printing on plastic substrates, the counter and reference functions are combined into a single, high-surface-area pseudo-reference electrode. This drastically simplifies manufacturing while maintaining excellent precision for small-volume samples.**Impedimetric Biosensors**
  - **(Electrochemical Impedance Spectroscopy - EIS):** EIS sensors apply a small, alternating current (AC) voltage over a range of frequencies and measure the system's resistance to current flow (impedance). These sensors are highly sensitive to physical and chemical changes occurring at the sensor interface—such as molecular binding events that block ion transport.
  - Advanced designs leverage specialized geometries like **interdigitated electrodes (IDEs)** to maximize surface contact, or incorporate **four-electrode electrochemical cells** paired with nanoporous membranes (e.g., alumina) to detect incredibly low concentrations of target proteins (like human alpha-thrombin) even within dense, complex backgrounds like serum albumin.
- **Field-Effect Transistor (FET) Biosensors:** FET-based biosensors operate via direct electrical detection. Biological recognition elements are immobilized onto the gate region of a transistor. When the target analyte binds to the gate, its intrinsic electrical charge modifies the electric field across the semiconductor channel, altering the drain-source current. This enables label-free, ultra-sensitive detection of small peptides, DNA, and proteins. A subset of this technology includes **Biofunctionalized Ion-Sensitive Field-Effect Transistors (BioFETs/ISFETs)**, which are highly sensitive to space-charge variations at the interface.

#### B. Passive Measurement Techniques

- **Potentiometric Biosensors:** These sensors measure the electrical potential difference between a working electrode and a reference electrode under conditions of **zero current**. Because the system remains at equilibrium, the generated voltage is space-charge sensitive and exhibits a **logarithmic response** relative to the analyte concentration (governed by the Nernst equation). This allows potentiometric sensors to feature an exceptionally wide dynamic range. They are frequently manufactured by screen-printing electrode patterns onto flexible plastic substrates, which are coated with a conducting polymer layer. Variations in ionic strength, pH, hydration, or localized redox reactions at the surface alter the charge distribution of the conducting polymer, yielding a highly robust, low-cost sensor requiring minimal sample preparation.


## 2. Classification by Biorecognition Mechanism

Regardless of the electrical technique used to read out the signal, the specificity of the biosensor depends entirely on its biological layer.

- **Enzymatic Biosensors:** These rely on the catalytic properties of specific enzymes. Most commonly, **redox enzymes** are utilized to directly or indirectly consume or produce electrons via the enzymatic turnover of a substrate. For example, the enzyme can produce an electroactive byproduct (like hydrogen peroxide) which is then oxidized at an amperometric working electrode to generate a current.
- **Affinity-Based Biosensors (Immunosensors and Aptasensors):** Instead of processing a chemical reaction, these sensors rely on the purely physical, high-affinity binding between a receptor and a target molecule.
  - *Antibodies (Immunosensors):* Utilize the highly specific lock-and-key binding of an antibody to its antigen. The binding event typically triggers a change in surface impedance or shifts the baseline current/voltage via an attached enzyme label.
  - *Aptamers (Aptasensors):* Utilize engineered, single-stranded oligonucleotides (DNA or RNA) that fold into unique 3D architectures to bind specific targets. Aptamers offer distinct advantages over antibodies, including superior thermal stability, lower production costs, and easier chemical modification for seamless integration onto conducting polymer layers.

By combining an optimized measurement technique (such as a simplified two-electrode amperometric system or a logarithmic potentiometric layout) with an appropriate biorecognition element, modern electrochemical biosensors can achieve the analytical sensitivity previously only possible with rigorous, laboratory-bound techniques like HPLC and LC/MS, all while maintaining a portable, zero-sample-preparation workflow.
