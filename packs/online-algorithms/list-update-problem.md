...
source: https://en.wikipedia.org/wiki/List_update_problem
...
An adversary is an entity that gets to choose the request sequence $\sigma$ for an algorithm *ALG*. Depending on whether $\sigma$ can be changed based on the strategy of *ALG*, adversaries are given various powers, and the performance of *ALG* is measured against these adversaries.
...
Any deterministic algorithm has a lower bound of $2-{\frac {2}{l+1}}$ for a list of length *l*, and MTF is actually the optimum deterministic list update algorithm. The type of adversary doesn't matter in the case of deterministic algorithms, because the adversary can run a copy of the deterministic algorithm on their own to precompute the most disastrous sequence.
...
The list update problem where elements maybe inserted and deleted is called the dynamic list update problem, as opposed to the static list update problem where only accessing list elements are allowed. The lower bound of $2-{\frac {2}{l+1}}$ holds for the dynamic model as well.

There are different cost models as well. In the usual full cost model, an access to an element located at a position *i* costs *i*, but the last comparison is inevitable for any algorithm, i.e. there are *i-1* elements standing in the way of *i*. In the partial cost model, these final comparison costs totaling to the number of elements in the request sequence are ignored. For the costs of paid transpositions other than unity, **P****d** models are used.
