...
source: https://en.wikipedia.org/wiki/Digital_watermarking
...
- The message is an n-bit-long stream $\left(m=m_{1}\ldots m_{n},\;n\in \mathbb {N} \right.$ , with $\left.n=|m|\right)$ or $M=\{0,1\}^{n}$ and is modulated in the watermark. These kinds of schemes usually are referred to as multiple-bit watermarking or non-zero-bit watermarking schemes.
