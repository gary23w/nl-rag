...
- Expected errors (in `Task.throws`) Logged with severity `INFO`, traceback excluded.
...
Changed in version 5.7: Previously, if the `ignore_result` key was missing from the request message, `store_errors` would default to `True`, ignoring the task’s own `ignore_result` setting. The worker now correctly falls back to `Task.ignore_result` when no per-request override is present.
