...
Data can be seen as a random variable $X:\Omega \to {\mathcal {X}}$ , where $x\in {\mathcal {X}}$ appears with probability $\mathbb {P} [X=x]$ .
...
1. $C:{\mathcal {X}}\to \Sigma ^{*}$ is non-singular if injective.
2. $C:{\mathcal {X}}^{*}\to \Sigma ^{*}$ is uniquely decodable if injective.
3. $C:{\mathcal {X}}\to \Sigma ^{*}$ is instantaneous if $C(x_{1})$ is not a proper prefix of $C(x_{2})$ (and vice versa).
