...
Let *S* be the number of particles in the swarm, each having a position **x**i ∈ ℝ*n* in the search-space and a velocity **v**i ∈ ℝ*n*. Let **p**i be the best known position of particle *i* and let **g** be the best known position of the particle's neighborhood. A basic PSO algorithm to minimize the cost function is then:
