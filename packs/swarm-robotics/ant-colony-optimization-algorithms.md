...
where $\tau _{xy}$ is the amount of pheromone deposited for transition from state x to y , $\alpha$ ≥ 0 is a parameter to control the influence of $\tau _{xy}$ , $\eta _{xy}$ is the desirability of state transition $xy$ (*a priori* knowledge, typically $1/d_{xy}$ , where d is the distance) and $\beta$ ≥ 1 is a parameter to control the influence of $\eta _{xy}$ . $\tau _{xz}$ and $\eta _{xz}$ represent the trail level and attractiveness for the other possible state transitions.
...
where $\tau _{xy}$ is the amount of pheromone deposited for a state transition $xy$ , $\rho$ is the *pheromone evaporation coefficient*, m is the number of ants and $\Delta \tau _{xy}^{k}$ is the amount of pheromone deposited by k th ant, typically given for a TSP problem (with moves corresponding to arcs of the graph) by
...
*Step 1: Initialization.* Randomly place K ants on the image $I_{M_{1}M_{2}}$ where $K=(M_{1}*M_{2})^{\tfrac {1}{2}}$ . Pheromone matrix $\tau _{(i,j)}$ are initialized with a random value. The major challenge in the initialization process is determining the heuristic matrix.
...
$f(x)={\begin{cases}\sin({\frac {\pi x}{2\lambda }}),&{\text{for 0 ≤ x ≤}}\lambda {\text{; (3)}}\\0,&{\text{else}}\end{cases}}$

$f(x)={\begin{cases}\pi x\sin({\frac {\pi x}{2\lambda }}),&{\text{for 0 ≤ x ≤}}\lambda {\text{; (4)}}\\0,&{\text{else}}\end{cases}}$

The parameter $\lambda$ in each of above functions adjusts the functions' respective shapes.

*Step 2: Construction process.* The ant's movement is based on 4-connected pixels or 8-connected pixels. The probability with which the ant moves is given by the probability equation $P_{x,y}$

*Step 3 and step 5: Update process.* The pheromone matrix is updated twice. in step 3 the trail of the ant (given by $\tau _{(x,y)}$ ) is updated where as in step 5 the evaporation rate of the trail is updated which is given by:

$\tau _{new}\leftarrow (1-\psi )\tau _{old}+\psi \tau _{0}$
...
- Classification
- Long-term production planning of open-pit mines
...
that substitutes traditional reproduction operators by model-guided operators. Such models are learned from the population by employing machine learning techniques and represented as probabilistic graphical models, from which new solutions can be sampled

or generated from guided-crossover.
