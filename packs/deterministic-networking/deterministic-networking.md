...
1. Time synchronization at each node (routers/bridge)across the entire network, with accuracy from nanoseconds to microseconds.
2. Deterministic data flow, which shall support:
...
3. Scheduling, shaping, limiting, and controlling transmission at each node.
4. Protection against misbehaving nodes (in both the data and the control planes): a flow cannot affect other flows even under high load.
5. Reserving resources in nodes that carry the flow.
