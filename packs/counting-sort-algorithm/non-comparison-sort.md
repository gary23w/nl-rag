...
source: https://en.wikipedia.org/wiki/Non-comparison_sort
...
- Computational complexity
  - Best, worst and average case behavior in terms of the size of the list. For typical serial sorting algorithms, good behavior is O(*n* log *n*), with parallel sort in O(log2 *n*), and bad behavior is O(*n*2). Ideal behavior for a serial sort is O(*n*), but this is not possible in the average case. Optimal parallel sorting is O(log *n*).
...
using the Three Hungarians' Algorithm).
...
An optimization of Selection Sort, which uses a tournament tree to select the min/max.
...
variant of heapsort based on the
...
An adaptive branchless stable introsort.
...
An in-place, but unstable variant of Fluxsort.
...
Similar to a gapped insertion sort.
...
, in the worst case over sequences that have
...
A bi-directional variant of Bubblesort.
