...
source: https://en.wikipedia.org/wiki/Multiplayer_video_game
...
Some games feature an 'Invasion mode' which let real players enter another player’s single or multiplayer session to help, harass, or replace NPCs; the mechanic traces a clear lineage from early experiments to FromSoftware’s *Souls* series and into modern titles like *Deathloop*, *Sniper Elite 5*, and *Elden Ring*, and it carries persistent design trade‑offs around balance, matchmaking, and griefing, adding tension, unpredictability, and emergent social interaction to single‑player content. Deathloop featured opt‑in asymmetric PvP and Elden Ring’s evolved Souls invasions at scale.

Depending on the game, objectives and story, the invading player(s) goals, roles, and own objectives will be different. Hostile invasions have the invader with the goal to kill or disrupt the host (classic Souls formula), in a possession/control invasion the invader can take control of NPCs or civilians (e.g., *Mindjack* experimented with this), and a co‑op version the invader can cooperate with the host to accomplish objectives as a unit (seen in open‑world titles).
