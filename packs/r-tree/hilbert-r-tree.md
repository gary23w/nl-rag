...
source: https://en.wikipedia.org/wiki/Hilbert_R-tree
domain: r-tree
...
**Algorithm Search(node Root, rect w):** S1. Search nonleaf nodes:
...
**Algorithm HandleOverflow(node N, rect r):** /* return the new node if a split occurred. */ H1. Let ε be a set that contains all the entries from N

and its s - 1 cooperating siblings.

H2. Add r to ε. H3. If at least one of the s - 1 cooperating siblings is not full,
