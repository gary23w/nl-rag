...
The Hilbert basis is constructed as the family of functions $\{\psi _{jk}:\,j,\,k\,\in \,\mathbb {Z} \}$ by means of dyadic translations and dilations of $\psi \,$ , $\psi _{jk}(x)=2^{\frac {j}{2}}\psi \left(2^{j}x-k\right),$ for integers $j,\,k\,\in \,\mathbb {Z}$ .

If, under the standard inner product on $L^{2}\left(\mathbb {R} \right)$ , $\langle f,g\rangle =\int _{-\infty }^{\infty }f(x){\overline {g(x)}}dx,$ this family is orthonormal, then it is an orthonormal system: ${\begin{aligned}\langle \psi _{jk},\psi _{lm}\rangle &=\int _{-\infty }^{\infty }\psi _{jk}(x){\overline {\psi _{lm}(x)}}dx,\\&=\delta _{jl}\delta _{km},\end{aligned}}$ where $\delta _{jl}\,$ is the Kronecker delta.
...
The **integral wavelet transform** is the integral transform defined as $\left[W_{\psi }f\right](a,b)={\frac {1}{\sqrt {|a|}}}\int _{-\infty }^{\infty }{\overline {\psi \left({\frac {x-b}{a}}\right)}}f(x)dx\,$ The **wavelet coefficients** $c_{jk}$ are then given by $c_{jk}=\left[W_{\psi }f\right]\left(2^{-j},k2^{-j}\right)$
...
1. Discretizing of the $c-\tau$ axis Applied the following discretization of frequency and time: ${\begin{aligned}c_{n}&=c_{0}^{n}\\\tau _{m}&=m\cdot T\cdot c_{0}^{n}\end{aligned}}$ Leading to wavelets of the form, the discrete formula for the basis wavelet: $\Psi (k,n,m)={\frac {1}{\sqrt {c_{0}^{n}}}}\cdot \Psi \left[{\frac {k-mc_{0}^{n}}{c_{0}^{n}}}T\right]={\frac {1}{\sqrt {c_{0}^{n}}}}\cdot \Psi \left[\left({\frac {k}{c_{0}^{n}}}-m\right)T\right]$ Such discrete wavelets can be used for the transformation: $Y_{DW}(n,m)={\frac {1}{\sqrt {c_{0}^{n}}}}\cdot \sum _{k=0}^{K-1}y(k)\cdot \Psi \left[\left({\frac {k}{c_{0}^{n}}}-m\right)T\right]$
2. Implementation via the FFT (fast Fourier transform) As apparent from wavelet-transformation representation (shown below) $Y_{W}(c,\tau )={\frac {1}{\sqrt {c}}}\cdot \int _{-\infty }^{\infty }y(t)\cdot \Psi \left({\frac {t-\tau }{c}}\right)\,dt$ where c is scaling factor, $\tau$ represents time shift factor and as already mentioned in this context, the wavelet-transformation corresponds to a convolution of a function $y(t)$ and a wavelet-function. A convolution can be implemented as a multiplication in the frequency domain. With this the following approach of implementation results into: There are many different types of wavelet transforms for specific purposes. See also a full list of wavelet-related transforms but the common ones are listed below: Mexican hat wavelet, Haar Wavelet, Daubechies wavelet, triangular wavelet.
