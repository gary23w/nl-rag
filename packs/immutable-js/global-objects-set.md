...
|---|---|---|---|
| `A.difference(B)` | `Set` | $A\setminus B$ | (A Venn diagram where two circles overlap. The difference of A and B is the part of A that is not overlapping B.) |
| `A.intersection(B)` | `Set` | $A\cap B$ | (A Venn diagram where two circles overlap. The intersection of A and B is the part where they overlap.) |
| `A.symmetricDifference(B)` | `Set` | $(A\setminus B)\cup(B\setminus A)$ | (A Venn diagram where two circles overlap. The symmetric difference of A and B is the region contained by either circle but not both.) |
| `A.union(B)` | `Set` | $A\cup B$ | (A Venn diagram where two circles overlap. The union of A and B is the region contained by either or both circles.) |
| `A.isDisjointFrom(B)` | `Boolean` | $A\cap B = \empty$ | (A Venn diagram with two circles. A and B are disjoint because the circles have no region of overlap.) |
| `A.isSubsetOf(B)` | `Boolean` | $A\subseteq B$ | (A Venn diagram with two circles. A is a subset of B because A is completely contained in B.) |
| `A.isSupersetOf(B)` | `Boolean` | $A\supseteq B$ | (A Venn diagram with two circles. A is a superset of B because B is completely contained in A.) |
