---
title: "Share data between apps - Progressive web apps"
...
Progressive Web Apps (PWAs) also have the ability to share information using the same OS-orchestrated pattern. PWAs can both share data and accept shared data.
...
To make it possible for users to share data with other apps from your PWA, use the Web Share API. The Web Share API allows your app to share text, links, or files with other apps via the underlying operating system share mechanism.

To share data, use the `navigator.share()` method in response to a user action, such as a button click.

### Checking for support

Before displaying content-sharing UI in your application, check to ensure the Web Share API feature is supported. Even the browsers that support the Web Share API don't all support sharing all types of data. Therefore, it's a good practice to use the `navigator.canShare()` method first to validate whether the data you intend to share is, indeed, shareable from the browser that's running your app.
...
The `navigator.share()` method returns a `Promise` that may be rejected in cases such as when the shared data is incorrect, when the user aborts the share operation, or when the data transmission failed.
...
The following example demonstrates how to share a link and some text when a button in the app is clicked. The `canBrowserShareData` function used in the example is described in Checking for support and not repeated here.
...
For more information, see the sharing files example on the `navigator.share()` method page.

## Handling shared data from other apps

To register your PWA as a target of other apps' shared data, use the Web Share Target API and, in particular, the `share_target` web app manifest member.
...
When your app is selected by the user to handle another apps' shared content, your app is launched and the shared content is passed to it in a similar way to that in which `<form>` elements are submitted.

In the previous web app manifest code example, when the ChattyBox app is selected as a target, it is launched by making an HTTP `GET` request at the `/share-handler` URL, with the shared data passed as request parameters named `description` and `link`.

The `GET` request will look like this: `/shared-handler?description=...&link=...`.

Your app's main JavaScript code can then retrieve the shared data by using the URLSearchParams interface:
...
For more information, see the example Receive share data using GET on the `share_target` web app manifest member page.

### Handling shared files

In the previous example, text data was handled as a `GET` request. However, handling files requires the use of a `POST` request with a `multipart/form-data` encoding type.
...
As this example shows, each file object in the `files` property must have a `name` property and an `accept` property. The `accept` property must specify the accepted MIME types or file extensions.
