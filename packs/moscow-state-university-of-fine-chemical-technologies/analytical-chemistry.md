...
- $\varepsilon _{\rm {a}}$ is the absolute error.
- x is the true value.
...
Errors can be expressed relatively. Given the relative error ( $\varepsilon _{\rm {r}}$ ):
...
where *k*B is the Boltzmann constant, *T* is the temperature, *R* is the resistance, and $\Delta f$ is the bandwidth of the frequency f .
