...
source: https://en.wikipedia.org/wiki/Tunneling_protocol
domain: geneve-encapsulation
license: CC-BY-SA-4.0
tags: geneve encapsulation, generic encapsulation, tunnel headers, overlay transport
fetched: 2026-07-02
---
...
Other tunneling methods able to bypass network firewalls make use of different protocols such as DNS, MQTT, and SMS.
...
Tunneling a TCP-*encapsulating* payload (such as PPP) over a TCP-based connection (such as SSH's port forwarding) is known as "TCP-over-TCP", and doing so can induce a dramatic loss in transmission performance — known as the *TCP meltdown problem*, which is why virtual private network (VPN) software may instead use a protocol simpler than TCP for the tunnel connection.

The throughput of a TCP‑over‑TCP connection can be degraded because the control loops in the inner and outer TCPs interfere destructively with each other. Because TCP is designed to guarantee in‑order and complete packet delivery, the outer TCP masks the connection's health from the inner TCP. In normal TCP operation, packet loss is detected when the receiver replies with duplicate ACKs to a packet received before the missing packet in the sequence. In response, the sender retransmits the lost packet and halves the Congestion Window (CWND). In TCP‑over‑TCP, the outer TCP handles lost packets normally through retransmission and CWND reduction. Still, since the inner TCP has not received any information indicating latency in the connection, it continues to transmit packets at the rate currently governed by its CWND.

Because the inner TCP’s CWND is greater than the outer TCP’s, the outer TCP’s send buffer eventually fills. As the buffer inflates, packets experience longer and more variable queueing delays before transmission, thereby increasing the outer TCP’s measured round-trip time (RTT) and RTT variability. This inflates its retransmission timeout (RTO). Once the send buffer becomes full, the tunnel process can no longer forward inner‑TCP ACKs, so the inner TCP stops receiving acknowledgments entirely.

Because the inner TCP is no longer receiving ACKs, its RTO expires, causing it to retransmit the oldest unacknowledged segment and double its RTO. The inner TCP’s RTO then grows exponentially because it doubles with every timeout, and no ACKs are received to reset it.

The result of a TCP meltdown is an outer TCP with a severely reduced CWND, an inflated RTO, and a full send buffer. This indicates that the inner TCP cannot write and that ACKs are not being sent in either direction.
...
Over the years, tunneling and data encapsulation have in very rare cases been adopted for malicious reasons, in order to maliciously communicate outside of a protected network.
