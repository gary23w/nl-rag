---
title: "Box2D"
source: https://en.wikipedia.org/wiki/Box2D
...
On Aug 12, 2024, version 3.0 was launched, rewriting the library entirely in C, overhauling the API, increasing single and multi-threaded performance, and changing to a more stable physics solver.
...
The Chipmunk physics engine (written in C) and Phys2D (written in Java) are both partially based on Box2D Lite. The physics engine used in SpriteKit for iOS and OS X uses Box2D internally.

The LiquidFun physics engine is a fork of Box2D by Google, which adds fluid simulation to the engine.

LibGDX uses a Java wrapper around native Box2D code written in C as its physics engine.
