---
title: "Oversampling"
source: https://en.wikipedia.org/wiki/Oversampling
...
In practice, oversampling is implemented in order to reduce cost and improve performance of an analog-to-digital converter (ADC) or digital-to-analog converter (DAC). When oversampling by a factor of N, the signal-to-noise ratio (SNR) increases by ${\sqrt {N}}$ , because summing up uncorrelated noise increases its amplitude by ${\sqrt {N}}$ , while summing up a coherent signal increases its average by N.
...
If multiple samples are taken of the same quantity with uncorrelated noise added to each sample, then because, as discussed above, uncorrelated signals combine more weakly than correlated ones, averaging *N* samples reduces the noise power by a factor of *N*. If, for example, we oversample by a factor of 4, the signal-to-noise ratio in terms of power improves by a factor of four, which corresponds to a factor of two improvement in terms of voltage.
