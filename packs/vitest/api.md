...
Attaches custom metadata available in reporters.
...
Use `concurrent: false` instead when you need to override inherited or configured concurrency.
...
Use `test.override` to override fixture values for all tests within the current suite and its nested suites. This must be called at the top level of a `describe` block. See Overriding Fixture Values for more information.
...
`test.scoped` is deprecated in favor of `test.override` and will be removed in a future major version.

Alias of `test.override`
...
Alias for `suite`. See describe for more information.
...
Scoped `beforeAll` hook that inherits types from `test.extend`. See beforeAll for more information.

## test.afterAll 

Scoped `afterAll` hook that inherits types from `test.extend`. See afterAll for more information.
...
Scoped `aroundAll` hook that inherits types from `test.extend`. See aroundAll for more information.
