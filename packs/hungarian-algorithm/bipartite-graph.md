...
The *edge bipartization* problem is the algorithmic problem of deleting as few edges as possible to make a graph bipartite and is also an important problem in graph modification algorithmics. This problem is also fixed-parameter tractable, and can be solved in time ${\textstyle O\left(2^{k}m^{2}\right)}$ , where *k* is the number of edges to delete and *m* is the number of edges in the input graph.
