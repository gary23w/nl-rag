...
In GraphQL, there is no way to define a type so that the field’s data will only be considered valid if a non-empty list is provided. In other words,
...
is a useful abstraction for humans and droids.
