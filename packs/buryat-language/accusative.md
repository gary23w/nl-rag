...
source: https://en.wikipedia.org/wiki/Accusative
...
- *przyszłość → przyszłość* 'future'
- *noc → noc* 'night'
- *wieś → wieś* 'village'

The noun ***pani*** 'woman' is irregular and receives the ending ***-ą***:
...
- *oko → oko* 'eye'
- *auto → auto* 'car'
- *śniadanie → śniadanie* 'breakfast'
- *imię → imię* 'name'
- *muzeum → muzeum* 'museum'

### Usage

The direct object function of the accusative case in Polish is often used with verbs like *mieć* 'to have', *jeść* 'to eat', *pić* 'to drink', *czytać* 'to read' and *lubić* 'to like'. For example:
...
- The construction **na* + accusative* is used when referring to motion to:
  ...
  - Several non-geographic nouns such as:
    - *na cmentarz* ('cemetery')
    - *na film* ('a movie')
    - *na piwo* ('a beer')
    - *na uniwersytet* ('university')
    - *na pocztę* ('post office')
...
In Hebrew, if the object of the sentence is a pronoun (e.g., I, you, s/he) and the transitive verb requires a direct object, the word אֵת *et* is combined with the pronoun into an object pronoun. The combined words are:
...
| *Araba**yı** gördüm.* | 'I saw the car.' | (accusative case, definite direct object) |
