---
title: "Architecture"
source: https://apple.github.io/foundationdb/architecture.html
domain: foundationdb
...
The FoundationDB architecture chooses a decoupled design, where processes are assigned different heterogeneous roles (e.g., Coordinators, Storage Servers, Master). Cluster attempts to recruit different roles as separate processes, however, it is possible that multiple Stateless roles gets colocated (recruited) on a single process to meet the cluster recruitment goals. Scaling the database is achieved by horizontally expanding the number of processes for separate roles:
...
4. The commit proxy sends the read and write conflict ranges to the resolver(s) with the commit version included.
...
6. Once the mutations are durable on the logs, the commit proxy responds back success to the user.
