...
's proposed method of three filters, specifically red, green, and violet-blue
...
using three exposures with blue, green, and red filters
...
Outside Europe, RGB is not very popular as a video signal format; S-Video takes that spot in most non-European regions. However, almost all computer monitors around the world use RGB. The synchronization signal is necessary to establish the timing for displaying the color sample on the screen, governing both the horizontal direction (the progression of the image line) and the vertical direction (the jump to a new image line).

Synchronization can be transmitted primarily in three ways: Separate synchronization (RGBHV): With this method, there is one signal for horizontal synchronization (HSync) and another for vertical synchronization (VSync); both are independent of each other and of the color signals, resulting in a total of five signals in the transmission. Composite synchronization (RGBS): With this method, there is a single signal containing all horizontal and vertical synchronization information, independent of the color signals, resulting in a total of four signals in the transmission. Sync on Green (RGsB or SoG): With this method, there is a single signal containing all horizontal and vertical synchronization information multiplexed with the green color signal, resulting in a total of three signals in the transmission. Note that this model applies only to screens and light-based displays. It does not apply to pigments (paint), which follow the subtractive color model.
...
The RGB color model is one of the most common ways to encode color in computing, and several different digital representations are in use. The main characteristic of all of them is the quantization of the possible values per component (technically a *sample*) by using only integer numbers within some range, usually from 0 to some power of two minus one (2*n* − 1) to fit them into some bit groupings. Encodings of 1, 2, 4, 5, 8, and 16 bits per color are commonly found; the total number of bits used for an RGB color is typically called the color depth.
