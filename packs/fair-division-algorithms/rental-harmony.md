...
source: https://en.wikipedia.org/wiki/Rental_harmony
...
- In the cardinal utility version, each partner has a vector of monetary valuations. The partner should say, for each room, exactly how much money he is willing to pay for that room. The partner is assumed to have quasilinear utility, i.e., if he values the room as v and pays p , his net utility is $v-p$ .
...
Su suggests to weaken this assumption in the following way: each partner never chooses the most expensive room if there is a free room available. This does not require the person to choose the free room. In particular, this will hold if a person always prefers a free room to a room costing at least $1/(n-1)$ of the total rent. However, even this weakened assumption might be unrealistic, as in the above example.
...
- The number of indivisible items to divide (*m*) may differ from the number of partners (*n*).
...
4. Eliminate envy by compensating envious partners. There are at most $n-1$ rounds of compensation. The procedure is fully descriptive and says explicitly which compensations should be made, and in what order. Moreover, it is simple enough to be carried out without computer support.
...
"a housemate who is assigned a room with a negative room rent is subsidized by some of other housemates. In such a situation, some housemates may prefer to leave the room with negative room rent unused and to exclude the housemate who is assigned that room, because they may receive larger discount. In order to avoid such a situation, negative room rents must be avoided".
...
1. Initialize the price of each room to $1/n$ of the total house cost.
...
4. If the min-sum is less than the total cost, then increase all prices in a constant rate until the sum equals the total cost (i.e., add to each price: $(cost-minsum)/n$ ). Changing all prices by the same amount ensures that the assignment remains envy-free.
5. If the min-sum is more than the total cost, then there is no solution satisfying both NN and EF. There are several possible ways to proceed:
  - Decrease all prices in a constant rate until the sum equals the total cost (i.e., subtract from each price: $(minsum-cost)/n$ ). Some prices will necessarily be negative, as in the solution of Haake Raith and Su.
...
The runtime complexity of both finding maxsum allocation and finding minsum prices is $O(n^{3})$ .
...
The only maxsum allocation is giving room 1 to partner 1 and room 2 to partner 2. Let $p_{2}$ be the price of room 2 (so that the price of room 1 is $100-p_{2}$ ). To ensure partner 1 does not envy, we must have $p_{2}\geq x/2$ . To ensure partner 2 does not envy, we must have $p_{2}\leq y/2$ .

Suppose a deterministic protocol sets the price $p_{2}$ to some value in $[x/2,y/2]$ . If the price is more than $x/2$ , then partner 2 has an incentive to report a lower value of y , which is still above x , in order to push his payment down towards $x/2$ . Similarly, if the price is less than $y/2$ , then partner 1 has an incentive to report a higher value of x , which is still below y , in order to push the payment of partner 2 up towards $y/2$ (and thus push his own payment down). Hence, the mechanism cannot be strategyproof.
...
1. **Ex-ante Envy-Freeness** means that no partner envies the lottery of any other partner. This condition is trivial to achieve in a truthful mechanism: randomise over all possible allocations with equal probability and charge each partner $1/n$ of the total cost. But this condition is not appealing, since there is a large chance that in the outcome, many partners will be envious. They may not be comforted by the fact that the lottery has been fair.

2. **Guaranteed Probability of Envy-Freeness (GPEF)** means that there is a certain probability p such that, regardless of the partners' valuations, with probability at least p , the outcome will be envy-free. It is possible to achieve a GPEF of $1/n$ in the following way: find an envy-free assignment; choose an integer $i\in \{0,\dots ,n-1\}$ at random; and move each partner cyclically i rooms to the right. This randomized mechanism is truthful-in-expectation, since every partner has an equal probability to land in each room and the expected payment is $1/n$ of the total cost, regardless of the partner's bid. The probability of having an EF allocation is the probability that $i=0$ , which is exactly $1/n$ . This is not encouraging, since the probability of envy-freeness converges to 0 when the number of partners grows. But it is impossible to do better: in every truthful-in-expectation mechanism, the GPEF is at most $1/n$ .

3. **Expected Number of Envy-Free partners (ENEF)** means that there is a certain integer N such that, if we average the number of partners who do not envy in all possible outcomes of the mechanism, then regardless of the partners' valuations, the expectation is at least N . The ENEF criterion seems more appropriate than the GPEF criterion, because it measures not only the probability of entire envy-freeness, but also the quality of the cases in which the allocation is not entirely envy-free. The maximum ENEF of a truthful-in-expectation mechanism is at most $n-1+1/n$ . It is possible to attain this bound for $n=2$ . For $n>2$ , there is a truthful-in-expectation mechanism that almost attains this bound: the ENEF is $n-1$ . The general idea is as follows. Use the VCG mechanism to calculate a maxsum assignment and payments. Select one partner at random. Ignore that partner and use VCG again. Combine the outcomes in a way which guarantees that the total payment equals the total cost (see the paper for details). It is possible to show that: (a) the mechanism is truthful-in-expectation; (b) all partners except the ignored partner do not envy. Hence, the ENEF is $n-1$ . Simulations show that in about 80% of the cases, the GPEF of this mechanism is also at its maximum of $1/n$ .
