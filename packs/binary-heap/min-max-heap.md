...
In the following operations we assume that the min-max heap is represented in an array `A[1..N]`; The $ith$ location in the array will correspond to a node located on the level $\lfloor \log _{2}i\rfloor$ in the heap.
...
The `push-down` algorithm (or `trickle-down` as it is called in ) is as follows:
...
The `push-up` algorithm (or `bubble-up` as it is called in ) is as follows:
