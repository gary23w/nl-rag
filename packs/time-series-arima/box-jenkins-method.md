...
source: https://en.wikipedia.org/wiki/Box–Jenkins_method
...
The first step in developing a Box–Jenkins model is to determine whether the time series is stationary. In particular, it is necessary to check whether there is significant seasonality that needs to be modelled.

#### Detecting stationarity

Stationarity can be assessed from a run sequence plot. The run sequence plot should show constant location and scale, suggesting that mean and variance of the time series are constant. Furthermore, non-stationarity can also be detected from an autocorrelation plot with a very slow decay. One can also utilize a Dickey-Fuller test or Augmented Dickey-Fuller test.
...
) model if the ACF and PACF plots of the differenced data show the following patterns:
