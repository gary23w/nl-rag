...
As of May 2000, Subject Alternative Names are the preferred method of adding DNS names to certificates. The previous method of putting DNS names in the `commonName` field is now deprecated. Google Chrome version 58 (March 2017) removed support for checking the `commonName` field at all, instead only looking at the SANs. As shown in the picture of Wikimedia's section on the right, the SAN field can contain wildcards. Not all vendors support or endorse mixing wildcards into SAN certificates.

### Wildcard certificate

A public-key certificate which uses an asterisk `*` (the *wildcard*) in the domain name portion of it subject is called a Wildcard certificate. Through the use of `*`, a single certificate may be used for multiple sub-domains. It is commonly used for transport layer security in computer networking.
