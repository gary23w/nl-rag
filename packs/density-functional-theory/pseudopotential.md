...
source: https://en.wikipedia.org/wiki/Pseudopotential
...
First-principles pseudopotentials are derived from an atomic reference state, requiring that the pseudo- and all-electron valence eigenstates have the same energies and amplitude (and thus density) outside a chosen core cut-off radius $r_{c}$ .
...
1. Inside the cut-off radius $r_{c}$ , the norm of each pseudo-wavefunction be identical to its corresponding all-electron wavefunction:
...
2. All-electron and pseudo wavefunctions are identical outside cut-off radius $r_{c}$ .
