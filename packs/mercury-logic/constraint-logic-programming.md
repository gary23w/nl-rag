...
- an element of G is a constraint C, and we have $G'=G\backslash \{C\}$ and $S'=S\cup \{C\}$ ; in other words, a constraint can be moved from the goal to the constraint store
- an element of G is a literal $L(t_{1},\ldots ,t_{n})$ , there exists a clause that, rewritten using new variables, is $L(t_{1}',\ldots ,t_{n}')\mathrel {{:}{-}} B$ , the set $G'$ is G with $L(t_{1},\ldots ,t_{n})$ replaced by $t_{1}=t_{1}',\ldots ,t_{n}=t_{n}',B$ , and $S'=S$ ; in other words, a literal can be replaced by the body of a fresh variant of a clause having the same predicate in the head, adding the body of the fresh variant and the above equalities of terms to the goal
...
The pairwise equality of the arguments of two literals is often compactly denoted by $L(t_{1},\ldots ,t_{n})=L(t_{1}',\ldots ,t_{n}')$ : this is a shorthand for the constraints $t_{1}=t_{1}',\ldots ,t_{n}=t_{n}'$ . A common variant of the semantics for constraint logic programming adds $L(t_{1},\ldots ,t_{n})=L(t_{1}',\ldots ,t_{n}')$ directly to the constraint store rather than to the goal.
