...
source: https://docs.asciidoctor.org/asciidoc/latest/document-structure/
domain: asciidoc
...
An AsciiDoc document is a series of blocks that are stacked linewise. These blocks are typically offset from one another by an empty line. (While these empty lines aren’t always required, we do recommend using them for readability.)
...
The line is a significant building block in AsciiDoc. A line is defined as text that’s separated on either side by a newline character or the boundary of the document. Many aspects of the syntax must occupy a whole line. That’s why we say AsciiDoc is a line-oriented language.
