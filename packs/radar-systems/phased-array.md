...
$AF=\sum _{n=1}^{N}I_{n1}\left[\sum _{m=1}^{M}I_{m1}\mathrm {e} ^{j\left(m-1\right)\left(kd_{x}\sin \theta \cos \phi +\beta _{x}\right)}\right]\mathrm {e} ^{j\left(n-1\right)\left(kd_{y}\sin \theta \sin \phi +\beta _{y}\right)}$

Here, $\theta$ and $\phi$ are the directions which we are taking the array factor in, in the coordinate frame depicted to the right. The factors $\beta _{x}$ and $\beta _{y}$ are the *progressive phase shift* that is used to steer the beam electronically. The factors $I_{n1}$ and $I_{m1}$ are the excitation coefficients of the individual elements.

Beam steering is indicated in the same coordinate frame, however the direction of steering is indicated with $\theta _{0}$ and $\phi _{0}$ , which is used in calculation of progressive phase:

$\beta _{x}=-kd_{x}\sin \theta _{0}\cos \phi _{0}$

$\beta _{y}=-kd_{y}\sin \theta _{0}\sin \phi _{0}$
...
If we consider a $16\times 16$ phased array, this process provides the following values for $AF_{dB}$ , when steering to bore-sight ( $\theta _{0}=0^{\circ }$ , $\phi _{0}=0^{\circ }$ ):
