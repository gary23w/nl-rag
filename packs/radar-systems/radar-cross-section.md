...
source: https://en.wikipedia.org/wiki/Radar_cross-section
...
The NRCS has units of area per area, or ⁠m2/m2⁠ in MKS units.
...
- $P_{t}$ = transmitter's input power (watts)
- $G_{t}$ = gain of the radar transmit antenna (dimensionless)
...
- $P_{r}$ = power received back from the target by the radar (watts)

The ${\textstyle {{P_{t}G_{t}} \over {4\pi r^{2}}}}$ term in the radar equation represents the power density (watts per meter squared) that the radar transmitter produces at the target. This power density is intercepted by the target with radar cross section ${\textstyle \sigma }$ , which has units of area (meters squared). Thus, the product ${\textstyle {{P_{t}G_{t}} \over {4\pi r^{2}}}\sigma }$ has the dimensions of power (watts), and represents a hypothetical total power intercepted by the radar target. The second ${\textstyle {{1} \over {4\pi r^{2}}}}$ term represents isotropic spreading of this intercepted power from the target back to the radar receiver. Thus, the product ${\textstyle {{P_{t}G_{t}} \over {4\pi r^{2}}}\sigma {{1} \over {4\pi r^{2}}}}$ represents the reflected power density at the radar receiver (again watts per meter squared). The receiver antenna then collects this power density with effective area ${\textstyle A_{\mathrm {eff} }}$ , yielding the power received by the radar (watts) as given by the radar equation above.

The scattering of incident radar power by a radar target is never isotropic (even for a spherical target), and the RCS is a hypothetical area. In this light, RCS can be viewed as a correction factor that makes the radar equation "work out right" for the experimentally observed ratio of ${\textstyle P_{r}/P_{t}}$ . However, RCS is a property of the target alone and may be measured or calculated. Thus, RCS allows the performance of a radar system with a given target to be analysed independent of the radar and engagement parameters. In general, RCS is a function of the orientation of the radar and target. A target's RCS depends on its size, *reflectivity* of its surface, and the *directivity* of the radar return caused by the target's geometric shape.
...
- Insect: 0.00001 m2
- Bird: 0.01 m2
- Stealth aircraft: <0.1 m2 (e.g. F-117A: 0.001 m2)
...
Where $\sigma$ is the RCS, $S_{i}$ is the incident power density measured at the target, and $S_{s}$ is the scattered power density seen at a distance r away from the target.

In electromagnetic analysis this is also commonly written as

$\sigma =\lim _{r\to \infty }4\pi r^{2}{\frac {|E_{s}|^{2}}{|E_{i}|^{2}}}$

where $E_{s}$ and $E_{i}$ are the far field scattered and incident electric field intensities, respectively.
...
- Large volume RAM is usually resistive carbon loading added to fiberglass hexagonal cell aircraft structures or other non-conducting components. Fins of resistive materials can also be added. Thin resistive sheets spaced by foam or aerogel may be suitable for spacecraft.
