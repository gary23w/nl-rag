---
title: "Clos network"
source: https://en.wikipedia.org/wiki/Clos_network
domain: clos-network
license: CC-BY-SA-4.0
tags: clos network, non-blocking fabric, multistage switching, fat tree
fetched: 2026-07-02
---

# Clos network

In the field of telecommunications, a **Clos network** is a kind of multistage circuit-switching network that represents a theoretical idealization of practical, multistage switching systems. It was invented by Edson Erwin in 1938 and first formalized by the American engineer Charles Clos in 1952.
...
$\beta _{ij}={\frac {B}{A}}={\frac {\left({\begin{array}{c}i\\m-j\end{array}}\right)}{\left({\begin{array}{c}m\\j\end{array}}\right)}}={\frac {i!j!}{(i+j-m)!m!}}$

If *f**i* is the probability that *i* other calls are already active on the ingress switch, and *g**j* is the probability that *j* other calls are already active on the egress switch, the overall blocking probability is:

$P_{B}=\sum _{i=0}^{u}\sum _{j=0}^{u}f_{i}g_{j}\beta _{ij}$

This may be evaluated with *f**i* and *g**j* each being denoted by a binomial distribution. After considerable algebraic manipulation, this may be written as:
