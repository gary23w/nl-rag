...
The filename must be `.gitlab-ci.yml` and is case-sensitive, but you can configure a different filename.
...
If you use GitLab.com, runners on Linux, Windows, and macOS are already available for use. If needed, you can also register your own runners.
...
CI/CD expressions use the `$[[ ]]` syntax and are validated when you create a pipeline. You can also validate expressions in the pipeline editor before committing changes.
