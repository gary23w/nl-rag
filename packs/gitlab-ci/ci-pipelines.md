...
- Pipelines that use the `needs` keyword run based on dependencies between jobs and can run more quickly than basic pipelines.
...
- Workload pipelines run on ephemeral Git references for on-demand pipeline execution without creating temporary branches.
...
- **Run pipeline** name changed to **New pipeline** in GitLab 17.7.
- **Inputs** option introduced in GitLab 17.11 with a flag named `ci_inputs_for_pipelines`. Enabled by default.
- **Inputs** option generally available in GitLab 18.1. Feature flag `ci_inputs_for_pipelines` removed.
...
5. Optional. Enter any:
  - Inputs required for the pipeline to run. Default values for inputs are prefilled, but can be modified. Input values must follow the expected type.
  - CI/CD variables. You can configure variables to have their values prefilled in the form. Using inputs to control pipeline behavior offers improved security and flexibility over CI/CD variables.
...
- Introduced in GitLab 17.2 with a flag named `ci_show_manual_variables_in_pipeline`. Disabled by default.
- Generally available in GitLab 18.4 with a project setting. Feature flag `ci_show_manual_variables_in_pipeline` removed.
...
- Markdown rendering on the **Run pipeline** page Introduced in GitLab 17.11.
...
(Pipeline graph that shows four stages: build, test, canary, and production. First three stages show completed jobs with green checkmarks, while production stage shows a pending deploy job.)
...
- The `skip_ci` type in pipeline execution policies.
- The `skip_ci` type in scan execution policies.
...
(A pipeline graph showing jobs grouped under each stage)
