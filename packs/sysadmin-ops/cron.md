...
|---|---|---|---|---|
| Seconds | Sometimes | 0–59 | `*` `,` `-` | Not supported for standard 5-digit implementations. Required for 6-digit and 7-digit implementations where seconds are allowed. |
| Minutes | Yes | 0–59 | `*` `,` `-` |   |
| Hours | Yes | 0–23 | `*` `,` `-` |   |
...
| Year | No | 1970–2099 | `*` `,` `-` | Not supported for standard 5-digit implementations. Optional for some 6-digit and 7-digit implementations. |
...
Percent-signs (%) in the command, unless escaped with backslash (\), are changed into newline characters, and all data after the first
...
'L' stands for "last". When used in the day-of-week field, it allows specifying constructs such as "the last Friday" ("
...
For example, entering "5#3" in the day-of-week field corresponds to the third Friday of every month.

**Question mark (`?`)**

In some implementations, used instead of '

*

' for leaving either day-of-month or day-of-week blank. Other cron implementations substitute "?" with the start-up time of the cron daemon, so that
...
' indicates that the task is performed every hour at an unspecified but invariant time for each task. This allows spreading out tasks over time, rather than having all of them start at the same time and compete for resources.
