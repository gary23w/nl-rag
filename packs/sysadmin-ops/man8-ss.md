...
-n, --numeric
       ...
       values, instead of human-readable values.
...
-o, --options
       ...
       <timer_name>
              the name of the timer, there are five kinds of timer
              names:
...
-m, --memory
       ...
       <rcv_buf>
              the total memory that can be allocated for receiving
              packets

       <wmem_alloc>
              the memory used for sending packets (which has been
              sent to layer 3)

       <snd_buf>
              the total memory that can be allocated for sending
              packets

       <fwd_alloc>
              the memory allocated by the socket as cache, but not
              used for receiving/sending packets yet. If memory is
              needed to send/receive packets, the memory in this
              cache will be used before allocating additional
              memory.

       <wmem_queued>
              The memory allocated for sending packets (which has
              not been sent to layer 3)

       <opt_mem>
              The memory used for storing socket options, e.g.,
              the key for TCP MD5 signature

       <back_log>
              The memory used for the sk backlog queue. On a
              process context, if the process is receiving
              packets, and a new packet is received, it will be
              put into the sk backlog queue, so it can be received
...
-T, --threads
       Show threads using sockets. Implies -p.
...
-i, --info
       ...
       lastsnd:<lastsnd>
              how long it has been since the last packet sent, the
              unit is millisecond

       lastrcv:<lastrcv>
              how long it has been since the last packet received,
              the unit is millisecond

       lastack:<lastack>
              how long it has been since the last ack received,
              the unit is millisecond
...
STATE-FILTER allows one to construct an arbitrary set of states to
