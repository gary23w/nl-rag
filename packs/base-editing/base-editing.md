...
source: https://en.wikipedia.org/wiki/Base_editing
domain: base-editing
...
### Large genomic insertions

Larger modifications used to be inefficient and error-prone but recent developments allow fairly reliable larger modifications. For instance, the quadruple pegRNA strategy (QuadPE) allowed researchers to stably insert large DNA fragments into human cells ranging from 1.6 to 26 kb at an efficiency of about 40%.

### Editing by nucleobase modification (Base editing)

One of the earliest methods of efficiently editing nucleic acids employs nucleobase modifying enzymes directed by nucleic acid guide sequences. It was first described in the 1990s and has seen a resurgence more recently. This method has the advantage that it does not require breaking the genomic DNA strands, and thus avoids the random insertions and deletions associated with DNA strand breakage. It is only appropriate for precise editing requiring single nucleotide changes and has found to be highly efficient for this type of editing.

### ARCUT

ARCUT stands for artificial restriction DNA cutter, it is a technique developed by Makoto Komiyama. This method uses pseudo-complementary peptide nucleic acid (pcPNA), for identifying cleavage site within the chromosome. Once pcPNA specifies the site, excision is carried out by cerium (CE) and EDTA (chemical mixture), which performs the splicing function.
