...
Consider, for example, a two-input AND gate fed with the following logic: ${\text{output}}=A\wedge {\overline {A}}$ A logic signal A on one input and its Boolean negation ${\bar {A}}$ on another input can never, in theory, output a true value as $A\wedge {\overline {A}}\neq 1$ . If, however, changes in the value of A take longer to propagate to the second input than the first when A changes from false to true then a brief period will ensue during which both inputs are true, and so the gate's output will also be true.
