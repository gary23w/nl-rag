...
${\begin{aligned}\alpha _{o}&={\phantom {~C_{a}}}\alpha _{a}+{\phantom {C_{b}}}\alpha _{b}(1-\alpha _{a})\\C_{o}&={\frac {C_{a}\alpha _{a}+C_{b}\alpha _{b}(1-\alpha _{a})}{\alpha _{o}}}\end{aligned}}$

Here $C_{o}$ , $C_{a}$ and $C_{b}$ stand for the color components of the pixels in the result of the "over", image A, and image B respectively, applied to each color channel (red/green/blue) individually, whereas $\alpha _{o}$ , $\alpha _{a}$ and $\alpha _{b}$ are the alpha values of the respective pixels.
...
- With **premultiplied alpha**, the RGB components represent the emission of the object or pixel, and the alpha represents the occlusion. The **over** operator then becomes: $C_{o}=C_{a}+C_{b}(1-\alpha _{a})$ $\alpha _{o}=\alpha _{a}+\alpha _{b}(1-\alpha _{a})$
...
The most popular image formats that support the alpha channel are PNG and TIFF. GIF supports 1-bit transparency by designating one palette entry as transparent. Support for alpha channels is present in some video codecs, such as Animation and Apple ProRes 4444 of the QuickTime format, or in the Techsmith multi-format codec.
...
| GIF | 8-bit |   | All modern browsers | Image (.gif) | 1-bit transparency. |
