---
title: "Iterator"
source: https://en.wikipedia.org/wiki/Iterator
...
Iterators do not exist natively in C, but can be somewhat emulated with pointer arithmetic.

The following demonstrates a linked list of `int` with iterator.
...
LinkedListIterator list_begin(LinkedList* head) {
    LinkedListIterator it = { head };
    return it;
}

int* list_next(LinkedListIterator* it) {
    if (!it->current) {
        return NULL;
    }
...
int main() {
    // ...
    for (LinkedListIterator it = list_begin(head); ; ) {
        int* val = list_next(&it);
        if (!val) {
            break;
        }
        printf("%d\n", *val);
...
int main() {
    ...
    for (auto it = items.begin(), end = items.end(); it != end; ++it) {
        // And print value of 'items' for current index.
        std::print("{}", *it);
    }

    // Using foreach loops (added in C++11), 
    // the same can be done without using any explicit iterators:
    for (int x : items) {
        // Print value of each element 'x' of 'items'.
        std::print("{}", x);
    }
...
```mw
// Any standard container type of ItemType elements.
ContainerType<ItemType> c;

// Function that will process each item of the collection.
void processItem(const ItemType& i) {
    std::println("{}", i);
}

// A for-each iteration loop.
std::for_each(c.begin(), c.end(), processItem);
// using std::ranges:
std::ranges::for_each(c, processItem);
```
...
std::copy(c.begin(), c.end(), ostream_iterator<ItemType>(cout, "\n"));
// using std::ranges:
std::ranges::copy(c, ostream_iterator<ItemType>(cout, "\n"));
```
...
std::for_each(c.begin(), c.end(), [](const ItemType& i) -> void { std::println("{}", i); });

// Using std::ranges:
std::ranges::for_each(c, [](const ItemType& i) -> void { std::println("{}", i); });
```

The Java-style syntax for iterators (`Iterator<T>`, vs `T::iterator`) can be implemented in C++ in the following way:

```mw
import std;

namespace {
    // helper concept: if has iterator
    template <typename T>
...
template <typename Iterable>
    requires Has_iterator<Iterable>
...
int main() {
    ...
    for (Iterator<Vector<int>> it = v.begin(); it != v.end(); ++it) {
        std::println("{}", *it);
    }
}
```
...
// Iterator iter = list.iterator(); before J2SE 5.0
while (iter.hasNext()) {
...
Ruby can also iterate over fixed lists by using `Enumerator`s and either calling their `#next` method or doing a for each on them, as above.
