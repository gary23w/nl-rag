...
source: https://en.wikipedia.org/wiki/Closure_(computer_programming)
...
The term *closure* is often used as a synonym for anonymous function, though strictly, an anonymous function is a function literal without a name, while a closure is an instance of a function, a value, whose non-local variables have been bound either to values or storage locations (depending on the language; see the lexical environment section below).
...
Here is the same example rewritten in TypeScript, another popular language with support for closures:

```mw
// Return a list of all books with at least 'threshold' copies sold.
function bestSellingBooks(threshold: number): Book[] {
    return bookList.filter(book => book.sales >= threshold);
}
```

The arrow operator `=>` is used to define an arrow function expression, and an `Array.prototype.filter` method instead of a global `filter` function, but otherwise the structure and the effect of the code are the same.
...
function derivative(f: UnaryFunction, dx: number): UnaryFunction {
...
var unboundGet = module.get;
console.log(unboundGet()); // The function gets invoked at the global scope
// emits undefined as 'x' is not specified in global scope.

var boundGet = unboundGet.bind(module); // specify object module as the closure
console.log(boundGet()); // emits 42
```
...
public class EnclosingClass {
    ...
    public static void main(String[] args) {
        ...
        for (
            int i = enclosingClassInstance.getCounter();
...
As of Java 8, Java supports functions as first class objects. Lambda expressions of this form are considered of type `java.util.function.Function<T, U>` with T being the domain and U the image type. The expression can be called with its `Function.apply(T t)` method, but not with a standard method call.

```mw
import java.util.function.Function;

Function<String, Integer> length = s -> s.length();
System.out.println(length.apply("Hello, world!")); // prints '13'
```
...
C++ also has the object `std::function<R(Args...)>` for holding an anonymous function. However, because anonymous functions have compiler-generated names and are distinct, storing one as a `std::function` uses type erasure which causes minor overhead.
