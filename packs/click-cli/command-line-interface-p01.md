---
title: "Command-line interface (part 1/2)"
...
- command — input by the user, a command is usually one of two classes:
  1. *Internal* commands are recognized and processed by the command line interpreter. Internal commands are also called built-in commands.
  2. *External* commands run executables found in separate executable files. The command line interpreter searches for executable files with names matching the external command.
...
Example of prompt system displays

All the information that can be displayed by the "Liquidprompt" command prompt system, here using its "Jolly" theme

Liquidprompt display example, using its default theme

Liquidprompt display example, using its "Pureship" theme

Liquidprompt display example, using its "Powerline" theme
