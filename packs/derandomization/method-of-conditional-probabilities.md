...
source: https://en.wikipedia.org/wiki/Method_of_conditional_probabilities
...
... [We then] show that the probabilistic existence proof can be converted, in a very precise sense, into a deterministic approximation algorithm.
...
$\sum _{u\in V}{\frac {1}{d(u)+1}}~\geq ~{\frac {|V|}{D+1}}.$
...
$|S^{(t)}|~+~\sum _{w\in R^{(t)}}{\frac {1}{d(w)+1}}.$
...
$1-\sum _{w\in N^{(t)}(u)\cup \{u\}}{\frac {1}{d(w)+1}},$
...
$\sum _{w\in N^{(t)}(u)\cup \{u\}}{\frac {1}{d(w)+1}}\leq (d(u)+1){\frac {1}{d(u)+1}}=1$
