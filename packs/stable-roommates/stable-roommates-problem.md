...
source: https://en.wikipedia.org/wiki/Stable_roommates_problem
...
disjoint pairs of participants. A matching
...
, each of whom prefers the other to their partner in

M

. Such a pair is said to block

M

, or to be a blocking pair with respect to
...
(i) *p* is first on *q*'s reduced list if and only if *q* is last on *p*'s (ii) *p* is not on *q*'s reduced list if and only if *q* is not on *p*'s if and only if *q* prefers the last person on their list to *p*; or *p*, the last person on their list to *q* (iii) no reduced list is empty
...
- Stable fixures - a many-to-many extension of stable roommates.
...
- MATLAB: The algorithm is implemented in the `assignStableRoommates` function as part of the United States Naval Research Laboratory's free and open-source software Tracker Component Library on GitHub.
