---
title: "File carving"
source: https://en.wikipedia.org/wiki/File_carving
...
It is used in IT forensics and data recovery. Carving is typically used on storage media with a corrupted file system or for storage areas that are listed as free in the existing file system.
...
Snapshots of computers' volatile memory (i.e. RAM) can be carved. Memory-dump carving is routinely used in digital forensics, allowing investigators to access ephemeral evidence. Ephemeral evidence includes recently accessed images and Web pages, documents, chats and communications committed via social networks. For example, LiME can be used in conjunction with Volatility to do such a task. If an encrypted volume (TrueCrypt, BitLocker, PGP Disk) was used, binary keys to encrypted containers can be extracted and used to instantly mount such volumes. The content of volatile memory gets fragmented. A proprietary carving algorithm was developed by Belkasoft to enable carving fragmented memory sets (BelkaCarving).
