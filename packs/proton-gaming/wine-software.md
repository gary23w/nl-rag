---
title: "Wine (software)"
source: https://en.wikipedia.org/wiki/Wine_(software)
...
Wine is primarily developed for Linux, macOS and FreeBSD. Developers can compile Windows applications against WineLib to help port them to Unix-like systems. No code emulation or virtualization occurs, except on Apple silicon Mac computers, where Rosetta 2 is used to translate x86_64 code to ARM code.
...
Configuration of Wine is managed primarily through *control* (wine's implementation of the Windows Control Panel), *regedit* (wine's Windows Registry editor tool), and *winecfg* (a GUI configuration tool), in addition to a multitude of other programmes. By default Wine will store configuration files and installed windows applications (unless otherwise specified during install) at *~/.wine*, this directory location is referred to as a Wine Prefix or Bottle. The *wineprefix* environment variable can be used to specify a different prefix, and using it will thereby change which configuration you are using.
...
library (and Wine 4.13 included a fix for it) to implement the
...
Wine, since 4.0 (2019), supports

game controllers

through its builtin implementations of these libraries. They are built as Unix shared objects as they need to access the controller interfaces of the underlying OS, specifically through
...
Preliminary support for 64-bit Windows applications was added to Wine 1.1.10, in December 2008. As of April 2019, the support is considered stable. The two versions of Wine are built separately, and as a result only building wine64 produces an environment only capable of running x86-64 applications.
...
Early versions of Microsoft Windows run on top of MS-DOS, and Windows programs may depend on MS-DOS programs to be usable. Wine does not have good support for MS-DOS, but starting with development version 1.3.12, Wine tries running MS-DOS programs in DOSBox if DOSBox is available on the system. Due to a bug, Wine also incorrectly identifies Windows 1.x and Windows 2.x programs as MS-DOS programs, unsuccessfully attempting to run them in DOSBox.
...
Proton is fully open-source and available via GitHub. Forks of Proton such as 'GEproton' have also become popular.
...
- WineVDM, a.k.a. OTVDM, a 16-bit app compatibility layer for 64-bit Windows
- ReactOS, a project to write an operating system compatible with Windows NT versions 5.x and up (which includes Windows 2000 and its successors) down to the device driver level. ReactOS uses Wine source code considerably. However, due to architectural differences, ReactOS cannot directly reuse Wine's NTDLL, USER32, KERNEL32, GDI32, and ADVAPI32 components. In July 2009, Aleksey Bragin, the ReactOS project lead, started a new ReactOS branch called Arwinss, and it was officially announced in January 2010. Arwinss is an alternative implementation of the core Win32 components, and uses mostly unchanged versions of Wine's user32.dll and gdi32.dll.
...
### Wine versus native Unix applications
