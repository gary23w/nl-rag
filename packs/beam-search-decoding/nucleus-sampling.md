...
source: https://en.wikipedia.org/wiki/Nucleus_sampling
...
Formally, the nucleus, $V^{(p)}\subseteq V$ , is defined as the smallest set of tokens satisfying: $\sum _{x\in V^{(p)}}P(x|x_{1},\dots ,x_{t-1})\geq p$ In this formula, $P(x|x_{1},\dots ,x_{t-1})$ represents the probability of a token x given the preceding tokens $x_{1},\dots ,x_{t-1}$ .
