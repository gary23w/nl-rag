...
domain: microsegmentation-network
license: CC-BY-SA-4.0
tags: network microsegmentation, east-west isolation, workload segmentation, policy enforcement
fetched: 2026-07-02
---
...
To help alleviate this problem, Sincoskie invented VLANs by adding a tag (an individual label, such as A, B, C, and so on) to each Ethernet frame. In this scheme, each switch could be assigned to handle frames of a specific tag and ignore the rest. The networks could be interconnected with three spanning trees, one for each color. By sending a mix of different frame tags, the aggregate bandwidth could be improved. Sincoskie referred to this as a *multitree bridge*. He and Chase Cotton created and refined the algorithms necessary to make the system feasible. This *color* is what is now known in the Ethernet frame as the IEEE 802.1Q header, or the VLAN tag. While VLANs are commonly used in modern Ethernet networks, they are not used in the manner first envisioned here.
