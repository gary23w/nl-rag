...
2. At each step, the algorithm looks at neighboring nodes and moves to a neighbor that is closer to the query.
3. When it cannot find a closer neighbor in that layer, it moves down to the next layer.
4. In the bottom layer, it explores a wider set of candidate nodes and returns the nearest candidates found.
