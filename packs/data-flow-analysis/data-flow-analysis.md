...
In this, $trans_{b}$ is the **transfer function** of the block b . It works on the entry state $in_{b}$ , yielding the exit state $out_{b}$ . The join operation $join$ combines the exit states of the predecessors $p\in pred_{b}$ of b , yielding the entry state of b .
