...
source: https://docs.datomic.com/whatis/data-model.html
domain: datomic
...
- The built-in :db/unique attribute can be added at any time. The uniqueness properties of an existing attribute also can be changed at any time (so long as the uniqueness properties are valid for the data). The :db/unique attribute can be used in the following ways: `:db.unique/identity` allows transactions to work with domain notions of identity instead of entity ids. For example a primary contact email or SKU number can be modeled with a `:db.unique/identity` attribute. `:db.unique/value` enforces a single holder of an identifying key.
