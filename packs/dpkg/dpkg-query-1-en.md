---
title: "dpkg-query(1)"
source: https://manpages.debian.org/bookworm/dpkg/dpkg-query.1.en.html
domain: dpkg
...
List all known packages matching one or more patterns, regardless of their status, which includes any real or virtual package referenced in any dependency relationship field (such as
...
to prevent the shell from performing filename expansion. For example this will list all package names starting with “libc6”:
...
option this will list all packages matching the given patterns. However the output can be customized using the
...
Report status of specified packages. This just displays the entry in the installed package status database. If no

package-name

is specified it will display all package entries in the status database (since dpkg 1.19.1). When multiple

package-name

entries are listed, the requested status entries are separated by an empty line, with the same order as specified on the argument list.
...
s are listed, the requested lists of files are separated by an empty line, with the same order as specified on the argument list.
...
locally diverted to: *diverted-to* package diverts others to: *diverted-to* diverted by *pkg* to: *diverted-to*
...
(since dpkg 1.15.4). If

control-file

is specified then only list the path for that control file if it is present.
...
Search for packages that own files corresponding to the given patterns. Standard shell wildcard characters can be used in the pattern, where asterisk (
...
) will match a slash, and backslash (
...
is specified, it will display all package entries in the
...
, which sets the administrative directory to «

directory

/var/lib/dpkg» (since dpkg 1.21.0).

****--load-avail****

Also load the available file when using the
...
commands, which now default to only querying the status file (since dpkg 1.16.2).
...
will produce (short option since dpkg 1.13.1). The format is a string that will be output for each package listed.
...
It contains the binary package name with a possible architecture qualifier like “libc6:amd64” (since dpkg 1.16.2). An architecture qualifier will be present to make the package name unambiguous, for packages with a
...
or with a foreign architecture, which is an architecture that is neither the native one nor
...
It contains the abbreviated package status (as three characters), such as “ii ” or “iHR” (since dpkg 1.16.2). See the
...
The requested query failed either fully or partially, due to no file or package being found (except for
...
option has not been specified, it will be used as the filesystem root directory (since dpkg 1.21.0).
...
option, but not all these flags might have an effect on this program.
