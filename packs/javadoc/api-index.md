...
source: https://docs.oracle.com/en/java/javase/17/docs/api/index.html
domain: javadoc
...
Defines tools for manipulating Java Archive (JAR) files, including the
...
Defines tools for diagnostics and troubleshooting a JVM such as the
...
Defines the JMX graphical tool,
...
Defines tools for analysing dependencies in Java libraries and programs, including the
...
tool for creating run-time images, the

jmod

tool for creating and manipulating JMOD files, and the

jimage

tool for inspecting the JDK implementation-specific container file for classes and resources.
...
tool for evaluating snippets of Java code, and defines a JDK-specific API for modeling and executing snippets.
...
tool for starting a daemon for the jstat tool to monitor JVM statistics remotely.

jdk.localedata

Provides the locale data for locales other than
