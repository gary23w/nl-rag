...
source: https://en.wikipedia.org/wiki/Le_Chatelier_principle
...
This way compares ${\mathcal {P}}_{\mathrm {i} }$ with ${\mathcal {P}}_{\mathrm {n} },$ to compare the effects of the imposed the change $\Delta L$ with and without moderation. The protocol ${\mathcal {P}}_{\mathrm {n} }$ prevents 'moderation' by enforcing that $\Delta X=0$ through an adjustment $\Delta Y,$ and it observes the 'no-moderation' response $\Delta M.$ Provided that the observed response is indeed that $\delta _{\mathrm {i} }X\neq 0,$ then the principle states that $|\delta _{\mathrm {i} }M|<|\Delta M|$ .
...
Take, for example, the reversible reaction of nitrogen gas (N2) with hydrogen gas (H2) to form ammonia (NH3):
...
Changing total pressure by adding an inert gas at constant volume does not affect the equilibrium concentrations (see § Effect of adding an inert gas).
