---
title: "Single- and multi-dimensional Arrays · The Julia Language"
source: https://docs.julialang.org/en/v1/manual/arrays/
domain: julia
license: MIT
...
| `rand(T, dims...)` | an `Array` with random, iid and uniformly distributed values. For floating point types `T`, the values lie in the half-open interval $[0, 1)$. |
...
If all indices `I_k` are integers, then the value in location `I_1, I_2, ..., I_n` of `A` is overwritten with the value of `X`, `convert`ing to the `eltype` of `A` if necessary.
...
`BitArray`s are space-efficient "packed" boolean arrays, which store one bit per boolean value. They can be used similarly to `Array{Bool}` arrays (which store one byte per boolean value), and can be converted to/from the latter via `Array(bitarray)` and `BitArray(array)`, respectively.
