...
source: https://en.wikipedia.org/wiki/Completely_randomized_design
...
- *Y*i,j being any observation for which *X*1 = *i* (*i* and *j* denote the level of the factor and the replication within the level of the factor, respectively)
...
- Estimate for *T*i : ${\bar {Y}}_{i}-{\bar {Y}}$

with ${\bar {Y}}_{i}$ = average of all *Y* for which *X*1 = *i*.

Statistical tests for levels of *X*1 are those used for a one-way ANOVA and are detailed in the article on analysis of variance.
