...
source: https://en.wikipedia.org/wiki/Kiosk_software
...
Kiosk software may offer remote monitoring to manage multiple kiosks from another location. An e-mail or text alert may be automatically sent from the kiosk for daily activity reports, or generated in response to problems detected by the software. Other features allow for remote updates of the kiosk's content and the ability to upload data such as kiosk usage statistics via built-in telemetry tools. Kiosk software is used to manage a touchscreen, allowing users to touch the monitor screen to make selections. A virtual keyboard eliminates the need for a computer keyboard, thus reducing the surface areas vulnerable to vandalism. Kiosk software enables digital signage devices to operate in a dedicated mode, ensuring that the devices run for a specified purpose, thereby providing additional security compared to normal mode use.
