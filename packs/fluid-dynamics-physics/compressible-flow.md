...
source: https://en.wikipedia.org/wiki/Compressible_flow
...
where a represents the speed of sound in the gas and V represents the velocity of the object. Although named for Austrian physicist Ernst Mach, these oblique waves were first discovered by Christian Doppler.
...
Ultimately, because of the energy conservation law, a gas is limited to a certain maximum velocity based on its energy content. The maximum velocity, *V*max, that a gas can attain is:

$V_{\text{max}}={\sqrt {2c_{p}T_{t}}}$
...
${\frac {{\text{property}}_{1}}{{\text{property}}_{2}}}=f(M,\gamma )$
...
${\begin{aligned}M_{2x}^{*}&={\frac {V_{x}}{a^{*}}}\\M_{2y}^{*}&={\frac {V_{y}}{a^{*}}}\end{aligned}}$
