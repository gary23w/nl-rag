...
. Since they are even, they can be written as
...
is odd, since it leaves a remainder of
...
to an odd number results in an odd number. But
...
is odd, for all positive integers
...
For example, contraposition can be used to establish that, given an integer x , if $x^{2}$ is even, then x is even:
...
even, the supposition must be false, so
...
In proof by contradiction, also known by the Latin phrase *reductio ad absurdum* (by reduction to the absurd), it is shown that if some statement is assumed true, a logical contradiction occurs, hence the statement must be false. A famous example involves the proof that ${\sqrt {2}}$ is an irrational number:
...
. Since the expression on the left is an integer multiple of 2, the right expression is by definition divisible by 2. That is,
...
have no common factor, so we must conclude that

${\sqrt {2}}$

is an irrational number.

To paraphrase: if one could write ${\sqrt {2}}$ as a fraction, this fraction could never be written in lowest terms, since 2 could always be factored from numerator and denominator.
...
In order to prove that the statements $\varphi _{1},\ldots ,\varphi _{n}$ are each pairwise equivalent, proofs are given for the implications $\varphi _{1}\Rightarrow \varphi _{2}$ , $\varphi _{2}\Rightarrow \varphi _{3}$ , $\dots$ , $\varphi _{n-1}\Rightarrow \varphi _{n}$ and $\varphi _{n}\Rightarrow \varphi _{1}$ .
