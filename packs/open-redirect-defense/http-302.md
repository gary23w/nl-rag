...
source: https://en.wikipedia.org/wiki/HTTP_302
...
The server has received the request headers and the client should proceed to send the request body (in the case of a request for which a body needs to be sent, such as a
...
The message body that follows is by default an
...
). For example, this code could be used to present multiple video format options, to list files with different
...
The link target was moved such that the request and future similar requests should be redirected to the given

URI

. If a client has link-editing capabilities, it should update references to the request URL. The response is cacheable unless indicated otherwise. Except for a GET request, the body should contain a hyperlink to the new URL(s). Except for a GET or HEAD request, the client must ask the user before redirecting.

This code is considered best practice for upgrading users from HTTP to
...
recommend using this code to change the URL of a page as it is shown in search engine results, providing that URL will permanently change and is not due to be changed again any time soon.
...
header field. The HTTP/1.0 specification (which used reason phrase "Moved Temporarily") required the client to redirect with the same method,

but popular browsers instead changed the request to GET.
...
) added two status codes: 303 which requires changing the request to a GET and 307 which preserves the original request type. Despite the greater clarity provided by this disambiguation, the 302 code is still used in web frameworks to preserve compatibility with browsers that do not support HTTP/1.1.
...
If a server responds to a POST or other non-idempotent request with this code and a

location

header field, the client is expected to issue a GET request to the specified location. To trigger a request to the target resource using the same method, the server responds with 307 instead.

Use of this code has been proposed

as one way of responding to a request for a

URI

that identifies a real-world object according to

Semantic Web

theory (the other being the use of
...
identifies a person, Alice, then it would be inappropriate for a server to respond to a GET request with 200 OK, as the server could not deliver Alice herself. Instead, the server would respond with 303 to redirect to a URI that provides a description of the person Alice.
...
. 308 parallels the behavior of 301, but
...
. 401 semantically means "unauthenticated", the user does not have valid authentication credentials for the target resource.
...
uses this code when requesting resources with
...
is an open standard that repurposes the HTTP 402 "Payment Required" status code.
...
returns 403 in response to a request for

URL

paths that corresponded to a

file system

directory

when directory listing is disabled and there is no

Directory Index

directive to specify an existing file to be returned to the browser. Some administrators configure the

Mod proxy

extension to block such requests and this will also return 403.

IIS

responds in the same way when directory listings are denied in that server. In

WebDAV

, 403 is returned if the client issued a PROPFIND request but did not also issue the required Depth header or issued a Depth header of infinity.
...
provided was too long for the server to process. Often the result of too much data being encoded as a query-string of a GET request, in which case it should be converted to a POST request. Called "Request-URI Too Long" previously.

**415 Unsupported Media Type**

The request entity has a
...
), but the server cannot supply that portion. For example, if the client asked for a part of the file that lies beyond the end of the file. Called "Requested Range Not Satisfiable" previously.
...
, and is not expected to be implemented by actual HTTP servers. The RFC specifies this code should be returned by teapots requested to brew coffee.

This HTTP status is used as an
...
" used to require agreement to Terms of Service before granting full Internet access via a
...
The server cannot honor the request because the user has not provided the required information.

**450 Blocked by Windows Parental Controls**

Indicates that Windows Parental Controls block access to the requested webpage.
...
to instruct the server to return no information to the client and close the connection immediately.
...
Cloudflare was able to complete a TCP connection to the origin server, but the origin did not provide a timely HTTP response.
...
Error 527 indicated an interrupted connection between Cloudflare and the origin server's Railgun server.

This error is obsolete as Cloudflare has deprecated Railgun.

**530 Origin Unavailable**

Cloudflare was unable to resolve the origin hostname, preventing it from establishing a connection to the origin server. The body of the response contains an 1xxx error.
...
A CSRF Token is missing or expired.
...
The phrase "Enhance your calm" comes from the

1993 movie

Demolition Man

, and its association with this number is likely a
...
Indicates that Cloudflare can't resolve the requested DNS record.

**540 Temporarily Disabled**

Indicates that the requested endpoint has been temporarily disabled.

**783 Unexpected Token**

Indicates that the request includes a JSON syntax error.
...
when the server's account has exceeded the resources assigned to it, such as CPU/RAM usage or number of concurrent processes.
...
Signals that the site can not process the request.
...
Indicates a site that has been frozen due to inactivity.
...
Related to being blocked/walled or unable to access their webpages without first signing in.
...
setting is enabled, though the status code and behavior is not part of any official Apache specification. The association between this status code and Apache has been attributed to unsourced additions to Wikipedia, which were subsequently picked up by other reference material, creating
...
An informal convention used by some HTTP proxies to signal a network read timeout behind the proxy to a client in front of the proxy.
