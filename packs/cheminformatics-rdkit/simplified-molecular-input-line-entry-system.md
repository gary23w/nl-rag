...
The original SMILES specification was initiated in the 1980s. It has since been modified and extended. In 2007, an open standard called OpenSMILES was developed in the open source chemistry community to harmonize the different dialects.

## History

The original SMILES specification was initiated by David Weininger at the USEPA Mid-Continent Ecology Division Laboratory in Duluth, Minnesota in the 1980s. Acknowledged for their parts in the early development were "Gilman Veith and Rose Russo (USEPA) and Albert Leo and Corwin Hansch (Pomona College) for supporting the work, and Arthur Weininger (Pomona; Daylight CIS) and Jeremy Scofield (Cedar River Software, Renton, WA) for assistance in programming the system." The Environmental Protection Agency funded the initial project to develop SMILES.
...
For example, cyclohexane and (*p-*)dioxane may be written as `C1CCCCC1` and `O1CCOCC1` respectively. For a second ring, the label will be 2. For example, decalin (decahydronaphthalene) may be written as `C1CCCC2C1CCCC2`.
...
The one form of branch which does *not* require parentheses is ring-closing bonds: the SMILES fragment `C1N` is equivalent to `C(1)N`, both denoting a bond between the `C` and the `N`. Choosing ring-closing bonds adjacent to branch points can reduce the number of parentheses required. For example, toluene is normally written as `Cc1ccccc1` or `c1ccccc1C`, avoiding the parentheses required if written as `c1cc(C)ccc1` or `c1cc(ccc1)C`.
...
| Vanillin | (Molecular structure of vanillin) | `O=Cc1ccc(O)c(OC)c1` `COc1cc(C=O)ccc1O` |
| Melatonin (C13H16N2O2) | (Molecular structure of melatonin) | `CC(=O)NCCC1=CNc2c1cc(OC)cc2` `CC(=O)NCCc1c[nH]c2ccc(OC)cc12` |
| Flavopereirin (C17H15N2) | (Molecular structure of flavopereirin) | `CCc(c1)ccc2[n+]1ccc3c2[nH]c4c3cccc4` `CCc1c[n+]2ccc3c4ccccc4[nH]c3c2cc1` |
| Nicotine (C10H14N2) | (Molecular structure of nicotine) | `CN1CCC[C@H]1c2cccnc2` |
| Oenanthotoxin (C17H22O2) | (Molecular structure of oenanthotoxin) | `CCC[C@@H](O)CC\C=C\C=C\C#CC#C\C=C\CO` `CCC[C@@H](O)CC/C=C/C=C/C#CC#C/C=C/CO` |
...
| (2*S*,5*R*)-Chalcogran: a pheromone of the bark beetle *Pityogenes chalcographus* | ((2S,5R)-2-ethyl-1,6-dioxaspiro[4.4]nonane) | `CC[C@H](O1)CC[C@@]12CCCO2` |
| α-Thujone (C10H16O) | (Molecular structure of thujone) | `CC(C)[C@@]12C[C@@H]1[C@@H](C)C(=O)C2` |
| Thiamine (vitamin B1, C12H17N4OS+) | (Molecular structure of thiamin) | `OCCc1c(C)[n+](cs1)Cc2cnc(C)nc2N` |
...
The OpenSMILES specification also includes a large number of examples with commentary on their acceptance by different implementations.
