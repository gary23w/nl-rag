...
Determines and reports the operational status of computer hardware and software. For example, a
...
Copies data from a storage device for the purpose of recovering from data loss due to events such as
...
the content of a disk, increasing the capacity of the disk.

**Disk defragmenter**

Reorganizes the data of each

file

so that it is stored contiguously.
...
Outputs a stream or a single file when provided with a directory or a set of files. Archive
...
Transforms data from a source file to some other format, such as from a text file to a
...
Creates a file of test data according to specified patterns. For example,
...
programs, usually applications programs, for use in a specific computer environment. An
