...
source: https://en.wikipedia.org/wiki/Adaptive_immune_response
domain: t-cell
...
Most elements of the jawed fish system are in place thanks to the second round of whole-genome duplication (2R) that the jawed fish have undergone, providing an expanded repertoire of immune-related genes. The immunoprotease genes involved in antigen processing and presentation as well as the class I and class II genes, are closely linked within the MHC of almost all studied species. These systems, along with RAG, were set in place quickly after 2R had happened. The MHC region is paralogous to the Ig/TCR region. Comparing paralogous regions allow for the reconstruction of a *primordial immune complex*.

The cartilaginous fish system does have one critical difference compared to the system of bony fish (including mammals): the gene segments that undergo V(D)J recombination in bony fish are not organized in huge clusters like they are in bony fish. Instead, these parts are arranged as "miniclusters" and rearrangement by RAG is largely limited within these clusters. As a result, not as many combinations are available compared to bony fish.

The RAG transposon likely invaded a "UrIg2" immunoglobin gene, which had an unsplit VJ, in an ancestor of all extant jawed fish. Splitting the V and J exons is adaptive, as it enables greater variation through changing the size of the CDR3 loop. This feature is inherited by all the descendants of UrIg2 such as Ig and TCR.

### Jawless fish

The jawless fish are vertebrates like the jawed fish, but belong on a different branch. In jawless fish, three subsets of lymphocytes use variable lymphocyte receptors (VLRs) for antigen binding. The VLRs are derived from pattern recognition receptors (PRRs) of the innate immune system. Diversity is generated by a cytosine deaminase-mediated rearrangement of LRR-based DNA segments.
