...
source: https://en.wikipedia.org/wiki/Van_der_Waals_force
...
| ${\begin{aligned}&U(z;R_{1},R_{2})=-{\frac {A}{6}}\left({\frac {2R_{1}R_{2}}{z^{2}-(R_{1}+R_{2})^{2}}}+{\frac {2R_{1}R_{2}}{z^{2}-(R_{1}-R_{2})^{2}}}+\ln \left[{\frac {z^{2}-(R_{1}+R_{2})^{2}}{z^{2}-(R_{1}-R_{2})^{2}}}\right]\right)\end{aligned}}$ |   | 1 |
|---|---|---|

where A is the Hamaker coefficient, which is a constant (~10−19 − 10−20 J) that depends on the material properties (it can be positive or negative in sign depending on the intervening medium), and *z* is the center-to-center distance; i.e., the sum of *R*1, *R*2, and *r* (the distance between the surfaces): $\ z=R_{1}+R_{2}+r$ .

The Van der Waals *force* between two spheres of constant radii (*R*1 and *R*2 are treated as parameters) is then a function of separation since the force on an object is the negative of the derivative of the potential energy function, $\ F_{\rm {VdW}}(z)=-{\frac {d}{dz}}U(z)$ . This yields:

| $\ F_{\rm {VdW}}(z)=-{\frac {A}{6}}{\frac {64R_{1}^{3}R_{2}^{3}z}{[z^{2}-(R_{1}+R_{2})^{2}]^{2}[z^{2}-(R_{1}-R_{2})^{2}]^{2}}}$ |   | 2 |
|---|---|---|
