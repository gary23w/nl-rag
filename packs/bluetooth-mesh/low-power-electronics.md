...
source: https://en.wikipedia.org/wiki/Low-power_electronics
...
Most electronic watches today use 32.768 KHz quartz oscillators.
...
Implantable medical devices represent one of the most demanding applications of low-power electronics, as they must operate reliably for years or decades inside the human body without requiring battery replacement surgery. Cardiac pacemakers, cochlear implants, neurostimulators, and implantable cardioverter-defibrillators are among the devices that depend critically on ultra-low power circuit design.

Pacemakers, for example, consume only a few tens of microamperes of current, allowing lithium-iodide batteries to power the device for up to ten years or more without replacement. Design strategies include operating CMOS transistors in the weak inversion region to minimize current consumption, aggressive voltage scaling, power gating to shut down inactive circuit blocks, and the use of energy harvesting techniques such as converting mechanical energy from heartbeats or body motion into electrical power.

The tight power budget of implantable devices imposes stringent constraints on every subsystem, including signal acquisition front-ends, digital signal processors, and wireless telemetry links. As a result, implantable medical devices have driven significant innovation in ultra-low power integrated circuit design, and techniques developed for this field have had broad influence on low-power electronics in general.
