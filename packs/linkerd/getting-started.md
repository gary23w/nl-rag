...
source: https://linkerd.io/2/getting-started/
domain: linkerd
...
This page contains instructions using the latest edge release of Linkerd. For other types of releases, including stable releases, please see
...
If you’re using a GKE “private cluster”, or if you’re using Cilium as a CNI, there may be some
...
export LINKERD2_VERSION=edge-26.6.3
...
Several features in Linkerd are configured using CRDs from the Gateway API project, including authorization policy and dynamic request routing. In many Kubernetes clusters, these CRDs are already available, but if they aren’t, you will need to install it first.

Follow these instructions for determining if the Gateway API is installed or how to install it.
...
The CLI-based install presented here is quick and easy, but there are a variety of other ways to install Linkerd, including by

using Helm charts

or by using a marketplace install from your Kubernetes provider.
...
Perhaps that last step was a little unsatisfying. We’ve added Linkerd to Emojivoto, but there are no visible changes to the application! That is part of Linkerd’s design—it does its best not to interfere with a functioning application.
...
- Hop into the `#linkerd` channel on the Linkerd Slack and say hi!
