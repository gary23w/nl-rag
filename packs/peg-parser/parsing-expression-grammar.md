...
As a mathematical object, a parsing expression grammar is a tuple $(N,\Sigma ,P,e_{S})$ , where N is the set of nonterminal symbols, $\Sigma$ is the set of terminal symbols, P is a function from N to the set of parsing expressions on $N\cup \Sigma$ , and $e_{S}$ is the starting parsing expression. Some concrete syntax dialects give the starting expression explicitly, but the primary concrete syntax instead has the implicit rule that the first nonterminal defined is the starting expression.
...
The parsing expression `('a'/'b')*` matches and consumes an arbitrary-length sequence of a 's and b 's. The production rule `S ← ('a' S 'b')?` matches the simple context-free language $\{a^{n}b^{n}:n\geq 0\}$ .

The following parsing expression grammar describes the classic non-context-free language $\{a^{n}b^{n}c^{n}:n\geq 0\}$ :
...
One classical example of a formal language which is provably not context-free is the language $\{a^{n}b^{n}c^{n}:n\geq 0\}$ : an arbitrary number of a 's are followed by an equal number of b 's, which in turn are followed by an equal number of c 's. This, too, is a parsing expression language, matched by the grammar:
...
For `A` to match, the first stretch of a 's must be followed by exactly the same number of b 's and no more, and in addition `B` has to match where the a 's switch to b 's, which means those b 's are followed by an equal number of c 's.
...
The ordering of productions in a PEG grammar affects not only the resolution of ambiguity, but also the *language matched*. For example, consider the first PEG example in Ford's paper (example rewritten in pegjs.org/online notation, and labelled ⁠ $G_{1}$ ⁠ and ⁠ $G_{2}$ ⁠):

- ⁠ $G_{1}$ ⁠:  `A = "a" "b" / "a"`
- ⁠ $G_{2}$ ⁠:  `A = "a" / "a" "b"`

Ford notes that *The second alternative in the latter PEG rule will never succeed because the first choice is always taken if the input string ... begins with 'a'.*. Specifically, ⁠ $L(G_{1})$ ⁠ (i.e., the language matched by ⁠ $G_{1}$ ⁠) includes the input "ab", but ⁠ $L(G_{2})$ ⁠ does not. Thus, adding a new option to a PEG grammar can *remove* strings from the language matched, e.g. ⁠ $G_{2}$ ⁠ is the addition of a rule to the single-production grammar `A = "a" "b"`, which contains a string not matched by ⁠ $G_{2}$ ⁠. Furthermore, constructing a grammar to match ⁠ $L(G_{1})\cup L(G_{2})$ ⁠ from PEG grammars ⁠ $G_{1}$ ⁠ and ⁠ $G_{2}$ ⁠ is not always a trivial task. This is in stark contrast to CFG's, in which the addition of a new production cannot remove strings (though, it can introduce problems in the form of ambiguity), and a (potentially ambiguous) grammar for ⁠ $L(G_{1})\cup L(G_{2})$ ⁠ can be constructed
...
In stark contrast to the case for context-free grammars, it is not possible to generate elements of a parsing expression language from its grammar. Furthermore, it is algorithmically undecidable whether the language recognized by a parsing expression grammar is empty. One reason for this is that any instance of the Post correspondence problem reduces to an instance of the problem of deciding whether a parsing expression language is empty.

Recall that an instance of the Post correspondence problem consists of a list $(\alpha _{1},\beta _{1}),(\alpha _{2},\beta _{2}),\dotsc ,(\alpha _{n},\beta _{n})$ of pairs of strings (of terminal symbols). The problem is to determine whether there exists a sequence $\{k_{i}\}_{i=1}^{m}$ of indices in the range $\{1,\dotsc ,n\}$ such that $\alpha _{k_{1}}\alpha _{k_{2}}\dotsb \alpha _{k_{m}}=\beta _{k_{1}}\beta _{k_{2}}\dotsb \beta _{k_{m}}$ . To reduce this to a parsing expression grammar, let $\gamma _{0},\gamma _{1},\dotsc ,\gamma _{n}$ be arbitrary pairwise distinct equally long strings of terminal symbols (already with 2 distinct symbols in the terminal symbol alphabet, length $\lceil \log _{2}(n+1)\rceil$ suffices) and consider the parsing expression grammar ${\begin{aligned}S&\leftarrow \&(A\,!.)\&(B\,!.)(\gamma _{1}/\dotsb /\gamma _{n})^{+}\gamma _{0}\\A&\leftarrow \gamma _{0}/\gamma _{1}A\alpha _{1}/\dotsb /\gamma _{n}A\alpha _{n}\\B&\leftarrow \gamma _{0}/\gamma _{1}B\beta _{1}/\dotsb /\gamma _{n}B\beta _{n}\end{aligned}}$ Any string matched by the nonterminal A has the form $\gamma _{k_{m}}\dotsb \gamma _{k_{2}}\gamma _{k_{1}}\gamma _{0}\alpha _{k_{1}}\alpha _{k_{2}}\dotsb \alpha _{k_{m}}$ for some indices $k_{1},k_{2},\dotsc ,k_{m}$ . Likewise any string matched by the nonterminal B has the form $\gamma _{k_{m}}\dotsb \gamma _{k_{2}}\gamma _{k_{1}}\gamma _{0}\beta _{k_{1}}\beta _{k_{2}}\dotsb \beta _{k_{m}}$ . Thus any string matched by S will have the form $\gamma _{k_{m}}\dotsb \gamma _{k_{2}}\gamma _{k_{1}}\gamma _{0}\rho$ where $\rho =\alpha _{k_{1}}\alpha _{k_{2}}\dotsb \alpha _{k_{m}}=\beta _{k_{1}}\beta _{k_{2}}\dotsb \beta _{k_{m}}$ .
