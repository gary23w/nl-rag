...
source: https://python-rq.org/docs/
domain: rq-queue
...
- `unique`: when set to `True`, prevents duplicate jobs with the same `job_id` from being enqueued. See Unique Jobs below.
...
*New in version 1.9.0.* You can also enqueue multiple jobs in bulk with `queue.enqueue_many()` and `Queue.prepare_data()`:
...
*New in version 2.0.* Multiple jobs can be added to a Group to allow them to be tracked by a single ID:
...
RQ allows you to enforce job uniqueness and deduplicate jobs, preventing the same jobs from being executed multiple times. To enforce uniqueness, pass `unique=True` along with an explicit `job_id` to `queue.enqueue()`:
...
The uniqueness constraint lasts until the job is deleted. Once a job is deleted, the same `job_id` can be reused:
...
RQ lets you configure the method and timeout for each callback - success, failure, and stopped. To configure callback timeouts, use RQ’s `Callback` object that accepts `func` and `timeout` arguments. For example:
...
Webhooks notify an external HTTP endpoint when a job reaches a terminal state (`finished` or `failed`). They are useful for monitoring, status pings and audit trails.

Pass a sequence of `Webhook` objects via the `webhooks` argument; each fires only on its `job_status`:
...
To be notified on both outcomes, pass one `Webhook` for each. The `webhooks` argument is also accepted by `enqueue_call`, `enqueue_at` and `enqueue_in`, the `@job` decorator, and `Queue.prepare_data()` for `enqueue_many`. Cron jobs can use webhooks for heartbeat monitoring; see cron jobs.
...
- `job_status` (required): `'finished'` or `'failed'`. The webhook fires only when the job reaches this state.
...
- A `failed` webhook fires after the failure is persisted as a terminal failure (which also runs after `on_failure`). It is **not** sent for attempts that will be retried or for jobs that are stopped.

Delivery is best-effort: send errors (unreachable endpoint, timeout, HTTP error) are logged and swallowed, so a failing webhook never affects job execution. Sending is blocking, so a slow endpoint can delay the worker by up to `timeout` seconds.
