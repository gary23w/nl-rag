...
- The closest pair among the seed points necessarily must be found in two adjacent cells in the Voronoi diagram.
...
Voronoi tessellations of regular lattices of points in two or three dimensions give rise to many familiar tessellations. The corresponding cells are known as Wigner–Seitz cells.
...
Higher-order Voronoi diagrams can be generated recursively. To generate the *n*th-order Voronoi diagram from set *S*, start with the (*n* − 1)th-order diagram and replace each cell generated by *X* = {*x*1, *x*2, ..., *x**n*−1} with a Voronoi diagram generated on the set *S* − *X*.

### Farthest-point Voronoi diagram

For a set of *n* points, the (*n* − 1)th-order Voronoi diagram is called a farthest-point Voronoi diagram.

For a given set of points *P* = {*p*1, *p*2, ..., *p**n*}, the farthest-point Voronoi diagram divides the plane into cells in which the same point of *P* is the farthest point. A point of *P* has a cell in the farthest-point Voronoi diagram if and only if it is a vertex of the convex hull of *P*. Let *H* = {*h*1, *h*2, ..., *h**k*} be the convex hull of *P*; then the farthest-point Voronoi diagram is a subdivision of the plane into *k* cells, one for each point in *H*, with the property that a point *q* lies in the cell corresponding to a site *h**i* if and only if d(*q*, *h**i*) > d(*q*, *p**j*) for each *p**j* ∈ *P* with *h**i* ≠ *p**j*, where d(*p*, *q*) is the Euclidean distance between two points *p* and *q*.
...
- Zeroes of iterated derivatives of a rational function on the complex plane accumulate on the edges of the Voronoi diagam of the set of the poles (Pólya's shire theorem).
...
For discrete grid approximations (such as pixel rendering), the Jump Flooding Algorithm (JFA) and its variants are widely used on commodity graphics hardware (GPUs). Originally introduced by Rong Guodong in 2006, the standard JFA computes an approximate Voronoi diagram in $O(\log N)$ parallel passes over an $N\times N$ grid. Subsequent optimizations, such as the 2019 randomized JFA-star variant introduced by Maciej A. Czyzewski, shift the pass complexity from scaling with grid resolution to the total number of seed points ( n ), achieving faster convergence in roughly $O(\log ^{*}n)$ passes.
...
- (Random points in 3D for forming a 3D Voronoi partition) Random points in 3D for forming a 3D Voronoi partition
- (3D Voronoi mesh of 25 random points) 3D Voronoi mesh of 25 random points
- (3D Voronoi mesh of 25 random points with 0.3 opacity and points) 3D Voronoi mesh of 25 random points with 0.3 opacity and points
- (3D Voronoi mesh of 25 random points convex polyhedra pieces) 3D Voronoi mesh of 25 random points convex polyhedra pieces
