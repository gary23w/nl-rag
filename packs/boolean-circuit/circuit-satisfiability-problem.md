...
Suppose the original 3SAT formula has variables $x_{1},x_{2},\dots ,x_{n}$ , and operators (AND, OR, NOT) $y_{1},y_{2},\dots ,y_{k}$ . Design a circuit such that it has an input corresponding to every variable and a gate corresponding to every operator. Connect the gates according to the 3SAT formula. For instance, if the 3SAT formula is $(\lnot x_{1}\land x_{2})\lor x_{3},$ the circuit will have 3 inputs, one AND, one OR, and one NOT gate. The input corresponding to $x_{1}$ will be inverted before sending to an AND gate with $x_{2},$ and the output of the AND gate will be sent to an OR gate with $x_{3}.$
