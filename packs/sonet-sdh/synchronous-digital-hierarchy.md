...
Called regenerator section overhead (RSOH) in SDH terminology: 27 octets containing information about the frame structure required by the
...
and management software allows the proper filtering and

troubleshooting

of network-wide performance management, so that defects and outages can be quickly identified and resolved.
...
, sends SONET TL1 commands from a local management network physically housed in the
