...
source: https://en.wikipedia.org/wiki/Payment_gateway
...
2. The payment gateway may allow transaction data to be sent directly from the customer's browser to the gateway, bypassing the merchant's systems, using redirect or iframe. This reduces the merchant's PCI DSS compliance obligations without redirecting the customer away from the website.
