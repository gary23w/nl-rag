...
source: https://en.wikipedia.org/wiki/Denoising
...
In the case of photographic film and magnetic tape, noise (both visible and audible) is introduced due to the grain structure of the medium. In photographic film, the size of the grains in the film determines the film's sensitivity; more sensitive film has larger-sized grains. In magnetic tape, the larger the grains of the magnetic particles (usually ferric oxide or magnetite), the more prone the medium is to noise. To compensate for this, larger areas of film or magnetic tape may be used to lower the noise to an acceptable level.
...
Unlike Dolby and dbx Type I and Type II noise reduction systems, DNL and DNR are playback-only signal processing systems that do not require the source material to first be encoded. They can be used to remove background noise from any audio signal, including magnetic tape recordings and FM radio broadcasts, reducing noise by as much as 10 dB. They can also be used in conjunction with other noise reduction systems, provided that they are used before applying DNR to prevent DNR from causing the other noise reduction system to mistrack.
...
Let $\delta _{i}$ denote the pixels adjacent to the i -th pixel. Then the conditional distribution of the greyscale intensity (on a $[0,1]$ scale) at the i -th node is $\mathbb {P} {\big (}x(i)=c\mid x(j)\,\forall j\in \delta _{i}{\big )}\propto \exp \left(-{\frac {\beta }{2\lambda }}\sum _{j\in \delta _{i}}{\big (}c-x(j){\big )}^{2}\right)$ for a chosen parameter $\beta \geq 0$ and variance $\lambda$ . One method of denoising that uses the auto-normal model uses the image data as a Bayesian prior and the auto-normal density as a likelihood function, with the resulting posterior distribution offering a mean or mode as a denoised image.
