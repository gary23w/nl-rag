---
title: "fauna-js/README.md at main · fauna/fauna-js · GitHub"
source: https://github.com/fauna/fauna-js/blob/main/README.md
domain: fauna-db
...
814 lines (603 loc) · 22.7 KB

Outline

# Official JavaScript Driver for Fauna v10 (current)
...
To enable debug logging, set the `FAUNA_DEBUG` environment variable to a string-encoded `LOG_LEVELS` integer:
...
Logs are output to `console` methods. If `FAUNA_DEBUG` is not set or is invalid, logging is disabled.

For advanced logging, you can pass a custom log handler using the client configuration's `logger` property:
