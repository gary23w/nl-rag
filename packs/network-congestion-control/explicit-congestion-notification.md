...
source: https://en.wikipedia.org/wiki/Explicit_Congestion_Notification
domain: network-congestion-control
license: CC-BY-SA-4.0
tags: congestion control, tcp congestion avoidance, congestion window, flow throttling
fetched: 2026-07-02
---
...
Note that this article describes classic ECN as originally defined; the L4S protocol, published in 2023 and described in RFC 9330, has altered the interpretation of the ECN bits and the expected treatment of traffic compared to classic ECN behavior.
...
*Note: this describes the operation of ECN prior to the definition of L4S in RFC 9330, which reserves the use of the codepoint `01` for L4S traffic.*
...
NetBSD 4.0 implements ECN support for TCP; it can be activated through the sysctl interface by setting 1 as value for the sysctl parameter `net.inet.tcp.ecn.enable`.

Likewise, on OpenBSD, the sysctl `net.inet.tcp.ecn` can be used.
