...
source: https://docs.timescale.com/use-timescale/latest/compression/about-compression/
domain: timescaledb
...
Understand the compression algorithms used when converting data from the rowstore to the columnstore
...
- **Integers, timestamps, boolean and other integer-like types**: a combination of the following compression methods is used: delta encoding, delta-of-delta, simple-8b, and run-length encoding.
- **Columns that do not have a high amount of repeated values**: XOR-based compression with some dictionary compression.
- **All other types**: dictionary compression.

This page gives an in-depth explanation of the compression methods used in hypercore.

## Integer compression

Section titled “Integer compression”
...
Section titled “Delta-of-delta encoding”
...
Section titled “XOR-based compression”
...
XOR is the binary function `exclusive or`. In this algorithm, successive floating point numbers are compared with XOR, and a difference results in a bit being stored. The first data point is stored without compression, and subsequent data points are represented using their XOR'd values.

## Data-agnostic compression

Section titled “Data-agnostic compression”

For values that are not integers or floating point, TimescaleDB uses dictionary compression.

### Dictionary compression

Section titled “Dictionary compression”
...
Section titled “JSONB compression”
...
- Set up hypercore: Enable and configure columnstore on your hypertables.
- Improve query and upsert performance: Tune `segmentby` and `orderby` for best compression and query speed.
- Basic compression with hypercore: Quick-start guide for enabling compression.
- Troubleshoot hypercore: Common issues and solutions.
- Understand hypercore: How the hybrid row-columnar engine works.
- Hypercore API overview: All columnstore functions and policies.
