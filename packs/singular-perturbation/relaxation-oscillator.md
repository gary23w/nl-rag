...
source: https://en.wikipedia.org/wiki/Relaxation_oscillator
...
First let's assume that $V_{dd}=-V_{ss}$ for ease of calculation. Ignoring the initial charge up of the capacitor, which is irrelevant for calculations of the frequency, note that charges and discharges oscillate between ${\frac {V_{dd}}{2}}$ and ${\frac {V_{ss}}{2}}$ . For the circuit above, Vss must be less than 0. Half of the period (T) is the same as time that $V_{\text{out}}$ switches from Vdd. This occurs when V− charges up from $-{\frac {V_{dd}}{2}}$ to ${\frac {V_{dd}}{2}}$ .
