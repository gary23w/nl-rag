...
source: https://en.wikipedia.org/wiki/Voltage_divider
...
If the current in the output wire is zero then the relationship between the input voltage, *V*in, and the output voltage, *V*out, is:
...
A resistive divider is the case where both impedances, *Z*1 and *Z*2, are purely resistive (Figure 2).

Substituting *Z*1 = *R*1 and *Z*2 = R2 into the previous expression gives:
...
If *V*out = 6 V and *V*in = 9 V (both commonly used voltages), then:
...
Any ratio *V*out / *V*in greater than 1 is not possible. That is, using resistors alone it is not possible to either invert the voltage or increase *V*out above *V*in.
