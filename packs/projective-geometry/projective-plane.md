...
source: https://en.wikipedia.org/wiki/Projective_plane
...
A **projective plane** is a rank 2 incidence structure $({\mathcal {P}},{\mathcal {L}},I)$ consisting of a set of **points** ${\mathcal {P}}$ , a set of **lines** ${\mathcal {L}}$ , and a symmetric relation I on the set ${\mathcal {P}}\cup {\mathcal {L}}$ called **incidence**, having the following properties:
...
It follows from the definition that the number of points $s+1$ incident with any given line in a projective plane is the same as the number of lines incident with any given point. The (possibly infinite) cardinal number s is called **order** of the plane.
...
1. To each parallel class of lines (a maximum set of mutually parallel lines) associate a single new point. That point is to be considered incident with each line in its class. The new points added are distinct from each other. These new points are called *points at infinity*.
2. Add a new line, which is considered incident with all the points at infinity (and no other points). This line is called *the* *line at infinity*.
...
The real projective plane **RP**2 arises when *K* is taken to be the real numbers, **R**. As a closed, non-orientable real 2-manifold, it serves as a fundamental example in topology.
...
The complex projective plane **CP**2 arises when *K* is taken to be the complex numbers, **C**. It is a closed complex 2-manifold, and hence a closed, orientable real 4-manifold. It and projective planes over other fields (known as **pappian planes**) serve as fundamental examples in algebraic geometry.

The quaternionic projective plane **HP**2 is also of independent interest.
...
The projective planes that can not be constructed in the preceding manner are called non-Desarguesian planes, and the Moulton plane given above is an example of one. Many such finite and infinite planes are known.

The PG(2, *K*) notation is reserved for Desarguesian planes. When *K* is a field, a very common case, they are also known as *field planes* and if the field is a finite field they can be called *Galois planes*.

## Subplanes

A **subplane** of a projective plane $({\mathcal {P}},{\mathcal {L}},I)$ is a pair of subsets $({{\mathcal {P}}'},{{\mathcal {L}}'})$ where ${{\mathcal {P}}'}\subseteq {\mathcal {P}}$ , ${{\mathcal {L}}'}\subseteq {\mathcal {L}}$ and $({{\mathcal {P}}'},{{\mathcal {L}}'},I')$ is itself a projective plane with respect to the restriction $I'$ of the incidence relation I to $({{\mathcal {P}}'}\cup {{\mathcal {L}}'})\times ({{\mathcal {P}}'}\cup {{\mathcal {L}}'})$ .

(Bruck 1955) proves the following theorem. Let Π be a finite projective plane of order *N* with a proper subplane Π0 of order *M*. Then either *N* = *M*2 or *N* ≥ *M*2 + *M*.

A subplane $({{\mathcal {P}}'},{{\mathcal {L}}'})$ of $({\mathcal {P}},{\mathcal {L}},I)$ is a **Baer subplane** if every line in ${\mathcal {L}}\setminus {{\mathcal {L}}'}$ is incident with exactly one point in ${\mathcal {P}}'$ and every point in ${\mathcal {P}}\setminus {{\mathcal {P}}'}$ is incident with exactly one line of ${\mathcal {L}}'$ .

A finite Desarguesian projective plane of order q admits Baer subplanes (all necessarily Desarguesian) if and only if q is square; in this case the order of the Baer subplanes is ${\sqrt {q}}$ .
...
is the ternary operator of the planar ternary ring.
...
Another type of collineation of PG(2, *K*) is induced by any automorphism of *K*, these are called **automorphic collineations**. If *α* is an automorphism of *K*, then the collineation given by (*x*0, *x*1, x2) → (*x*0*α*, *x*1*α*, *x*2*α*) is an automorphic collineation. The fundamental theorem of projective geometry says that all the collineations of PG(2, *K*) are compositions of homographies and automorphic collineations. Automorphic collineations are planar collineations.
...
+ 1 points on each line, and

N

+ 1 lines through each point.
...
- 2 : all isomorphic to PG(2, 2), configuration (73) with 168 automorphisms
- 3 : all isomorphic to PG(2, 3), configuration (134) with 1516 automorphisms
- 4 : all isomorphic to PG(2, 4), configuration (215) with 120,960 automorphisms, doubled by prime power, 4 = 22
- 5 : all isomorphic to PG(2, 5), configuration (316) with 372,000 automorphisms
...
- 7 : all isomorphic to PG(2, 7), configuration (578) with 5,630,688 automorphisms
- 8 : all isomorphic to PG(2, 8), configuration (739) with 98,896,896 automorphisms, 6 higher by prime power, 8 = 23
...
Projective planes may be thought of as projective geometries of dimension two. Higher-dimensional projective geometries can be defined in terms of incidence relations in a manner analogous to the definition of a projective plane.
