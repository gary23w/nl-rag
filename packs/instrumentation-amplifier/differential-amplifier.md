...
source: https://en.wikipedia.org/wiki/Differential_amplifier
...
where $A_{\text{c}}$ is called the common-mode gain of the amplifier.
...
In a perfectly symmetric differential amplifier, $A_{\text{c}}$ is zero, and the CMRR is infinite. Note that a differential amplifier is a more general form of amplifier than one with a single input; by grounding one input of a differential amplifier, a single-ended amplifier results.
