...
source: https://en.wikipedia.org/wiki/Advertising_research
...
Post-testing/Tracking studies provide either periodic or continuous in-market research monitoring a brand’s performance, including brand awareness, brand preference, product usage and attitudes. Traditional post-testing approaches include Starch readership scores, which measure the percentage of a publication's audience who noted, associated, and read most of a given advertisement, and brand-lift surveys, which use control-exposed survey designs to quantify advertising-driven shifts in awareness, favorability, and purchase intent.

Some post-testing approaches simply track changes over time, while others use various methods to quantify the specific changes produced by advertising—either the campaign as a whole or by the different media utilized. Beyond survey-based tracking, causal lift methodologies have gained prominence as advertisers seek evidence of actual behavioral change rather than attitudinal shifts. Randomized holdout experiments, in which a randomly selected group of users is withheld from advertising exposure while a matched treatment group is exposed, provide a direct causal estimate of incremental conversions or revenue attributable to a campaign, free from the selection bias that affects observational attribution models. Because only experiments, not models, can establish causality, holdout-based lift tests are increasingly regarded as the gold standard against which attribution outputs are calibrated. Where individual-level randomization is impractical—for example, in linear television, out-of-home, or broad digital campaigns—geo experiments offer a causal alternative by randomly assigning non-overlapping geographic regions to treatment or control conditions and comparing aggregate outcomes. Larsen et al. introduce GeoLift, an open-source framework that uses synthetic control methods to construct a counterfactual for treated regions from a weighted combination of control regions, improving precision when the number of available geos is small.

Synthetic control methods, formalized by Abadie and Gardeazabal and extended by Abadie, Diamond, and Hainmueller, construct such a counterfactual by finding a convex combination of unexposed units whose pre-treatment trajectory closely matches that of the treated unit, enabling causal inference in settings with a single or small number of treated regions. Athey and Imbens provide a broader treatment of modern causal inference methods applicable to advertising contexts, including panel data estimators, difference-in-differences designs, and machine-learning-assisted heterogeneous treatment effect estimation, all of which underpin the current generation of geo and holdout experiment analyses.
