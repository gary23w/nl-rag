---
title: "ATR 72"
source: https://en.wikipedia.org/wiki/ATR_72
...
Two subtypes were marketed as the 210 series (-210): the -211 (and with an enlarged cargo door, called the -212) is a -200 with PW127 engines producing 2,750 shp (2,050 kW) each for improved performance in hot and high-altitude conditions. The subtypes differ in the type of doors and emergency exits.
...
Marketing name for ATR 72-212A with different equipment fit. The -600 series aircraft was announced in October 2007; the first deliveries were planned for the second half of 2010.
...
engines, which enable a 5% increase in takeoff power via a "boost function" used only when called for by takeoff conditions. The flight deck features five wide
...
of earlier versions). A multi-purpose computer (MPC) aims at increasing flight safety and operational capabilities, and new
...
(RNP) capabilities. It also features lighter seats and larger overhead baggage bins. In December 2015, the EASA approved a new high-density seating layout, raising the maximum capacity from 74 to 78 seats.
...
) mission system with similar on-board equipment, but with additional
...
For the ASW and ASuW missions, it is armed with lightweight
...
systems, enabling the type to perform maritime
...
. Its capabilities are airborne ocean surface surveillance, telemetry recording and relay. Modified with AN/APS-143 Multi-Mode X-Band Maritime Surveillance Radar with Integrated IFF Interrogator to detect objects in the
...
of the -600, 8 November 2017 launch with 30 firm orders from FedEx plus 20 options.

The first flight of the variant took place on 16 September 2020;

it was EASA certified by early December.

In December 2020, FedEx Express received the first one of its order of 30, to be operated by
...
, coupled with a dedicated cargo conversion.
...
turboprops (turbofans were also studied for a time) and would have had a cruising speed as high as
...
. The ATR-82 project (as it was dubbed) was suspended when

AI(R)

was formed in early 1996.
...
, the aircraft would have a range of about
...
This proposed version targeted the increasing demand of worldwide cargo and express mail markets, where the aim is to allow operators to supplement their passengers flights with freighter flights. In Quick Change configuration, the smoke detector is equipped alongside other modifications required in order to meet the certification for full freight operations. The aircraft was equipped with a larger cargo door (1.27
...
in] high) and low door-sill height of an average 1.2

m (4

ft), facilitating containerised freight loading. It takes 30 minutes to convert the ATR 42, and 45 minutes to convert the ATR 72. Each optimized container has
...
of usable volume and maximum payload is 435
...
| **Range** **(Max pax)** | 862 nmi (1,596 km) | 791 nmi (1,465 km) | 772 nmi (1,430 km) | 758 nmi (1,404 km) |
