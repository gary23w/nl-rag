...
source: https://en.wikipedia.org/wiki/Lane_departure_warning_system
domain: adas-systems
...
General Motors introduced Lane Departure Warning on its 2008 model-year Cadillac STS, DTS, and Buick Lucerne models. The General Motors system warns the driver with an audible tone and a warning indicator on the dashboard. BMW also introduced Lane Departure Warning on the 5 Series (E60) and 6 Series, using a vibrating steering wheel to warn the driver of unintended departures. In late 2013 BMW updated the system with Traffic Jam Assistant appearing first on the redesigned BMW X5, this system works below 25 miles per hour (40 km/h). Volvo introduced the lane departure warning system and the driver alert control on its 2008 model-year S80, the V70, and XC70 executive cars. Volvo's lane departure warning system uses a camera to track road markings and sound an alarm when drivers depart their lane without signaling. The systems used by BMW, Volvo, and General Motors are based on core technology from Mobileye.
...
| Infiniti | Q50 | Available fly-by-wire (Direct Adaptive Steering) autonomous steering, lane keeping (Lane Assist), (Intelligent Cruise control) adaptive cruise control, and Predictive Forward Collision Warning system |   |
