...
A cutset X of G is called a non-trivial cutset if X does not contain the neighborhood N(*u*) of any vertex *u* ∉ *X*. Then the *superconnectivity* $\kappa _{1}$ of G is $\kappa _{1}(G)=\min\{|X|:X{\text{ is a non-trivial cutset}}\}.$

A non-trivial edge-cut and the *edge-superconnectivity* $\lambda _{1}(G)$ are defined analogously.
