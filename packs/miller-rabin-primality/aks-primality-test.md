...
This theorem is a generalization to polynomials of Fermat's little theorem. In one direction it can easily be proved using the binomial theorem together with the following property of the binomial coefficient:
...
| $(X+a)^{n}\equiv (X^{n}+a){\pmod {X^{r}-1,n}}$ |   | 2 |
|---|---|---|

which is the same as:

| $(X+a)^{n}-(X^{n}+a)=(X^{r}-1)g+nf$ |   | 3 |
|---|---|---|
...
2. Find the smallest *r* such that ord*r*(*n*) > (log2 *n*)2. If *r* and *n* are not coprime, then output *composite*.
...
5. For *a* = 1 to $\left\lfloor {\sqrt {\varphi (r)}}\log _{2}(n)\right\rfloor$ do if (*X*+*a*)*n* ≠ *X**n*+*a* (mod *X**r* − 1,*n*), then output *composite*;
6. Output *prime*.

Here ord*r*(*n*) is the multiplicative order of *n* modulo *r*, log2 is the binary logarithm, and $\varphi (r)$ is Euler's totient function of *r*.
...
consisting of $n^{r}$ elements. This ring contains only the r monomials $\{X^{0},X^{1},\ldots ,X^{r-1}\}$ , and the coefficients are in $\mathbb {Z} /n\mathbb {Z}$ which has n elements, all of them codable within $\log _{2}(n)$ bits.
