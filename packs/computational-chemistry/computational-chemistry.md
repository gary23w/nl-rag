...
source: https://en.wikipedia.org/wiki/Computational_chemistry
...
CCSD scales as ${\mathcal {O}}(M^{6})$ where M is the number of basis functions. This intense computational demand arises from the inclusion of single and double excitations in the electron correlation calculation. With the addition of perturbative triples in CCSD(Tthe complexity increases to ${\mathcal {O}}(M^{7})$ . This elevated complexity restricts practical usage to smaller systems, typically up to 20-25 atoms in conventional implementations.
