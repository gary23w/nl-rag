...
A common ultrasound application is an automatic door opener, where an ultrasonic sensor detects a person's approach and opens the door. Ultrasonic sensors are also used to detect intruders; the ultrasound can cover a wide area from a single point. The flow in pipes or open channels can be measured by ultrasonic flowmeters, which measure the average velocity of flowing liquid.

In rheology, an acoustic rheometer relies on analyzing the propagation of ultrasound to determine a fluid's viscosity.

In fluid mechanics, fluid flow can be measured using an ultrasonic flow meter based on the Doppler effect.
...
Although range finding underwater is performed at both sub-audible and audible frequencies for great distances (1 to several kilometers), ultrasonic range finding is used when distances are shorter and the accuracy of the distance measurement is desired to be better. Ultrasonic measurements may be limited through barrier layers with large salinity, temperature or vortex differences. Ranging in water varies from about hundreds to thousands of meters, but can be performed with centimeters to meters for accuracy.
...
The potential for ultrasonic imaging of objects, in which a 3 GHz sound wave could produce resolution comparable to an optical image, was recognized by Sergei Sokolov in 1939. Such frequencies were not possible at the time, and what technology did exist produced relatively low-contrast images with poor sensitivity. Ultrasonic imaging uses frequencies of 2 megahertz and higher; the shorter wavelength allows resolution of small internal details in structures and tissues. The power density is generally less than 1 watt per square centimetre to avoid heating and cavitation effects in the object under examination. Ultrasonic imaging applications include industrial nondestructive testing, quality control and medical uses. Imaging can be viewed as ranging with added angular information.
...
In Doppler ultrasonography, the speed of objects is measured along with their positions. This is especially useful for analyzing blood flow in the heart (Doppler echocardiography).
