...
source: https://en.wikipedia.org/wiki/Weight-balanced_tree
...
weight[n.left] ≥ α·weight[n]

and

weight[n.right] ≥ α·weight[n]
...
If α is given its maximum allowed value, the worst-case height of a weight-balanced tree is the same as that of a red–black tree at $2\log _{2}n$ .
...
is on two weight-balanced trees
...
and will return a tree containing all elements in
...
to be greater than all keys in

t

1

and smaller than all keys in

t

2

. If the two trees have the balanced weight,
...
To split a weight-balanced tree into two smaller trees, those smaller than key

x

, and those larger than key

x

, first draw a path from the root by inserting
...
will be found on the right. By applying

Join

, all the subtrees on the left side are merged bottom-up using keys on the path as intermediate nodes from bottom to top to form the left tree, and the right part is symmetric. For some applications,
...
Here balance $(x,y)$ means two weights T and T are balanced. expose(v)=(l, k, r) means to extract a tree node T's left child T, the key of the node T and the right child T. Node(l, k, r) means to create a node of left child T, key T and right child T.
