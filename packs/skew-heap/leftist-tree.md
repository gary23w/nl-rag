---
title: "Leftist tree"
source: https://en.wikipedia.org/wiki/Leftist_tree
domain: skew-heap
...
In case of Min HBLTs, assume we have two trees rooted at A and B where A.key $\leq$ B.key. Otherwise we can swap A and B so that the condition above holds.
...
Insertion is done using the merge operation. An insertion of a node into an already existing Min HBLT, creates a HBLT tree of size one with that node and merges it with the existing tree.
