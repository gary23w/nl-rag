...
source: https://en.wikipedia.org/wiki/Futures_and_promises
...
- In C++ (since C++11) a `std::future` provides a read-only view. The value is set directly by using a `std::promise`, or set to the result of a function call using `std::packaged_task` or `std::async`.
...
- C++, starting with C++11 via `std::future` and `std::promise`
  - Compositional C++
- Crystal
- Dart (with `Future`/`Completer` classes and the keywords `await` and `async`)
- Elm via the *Task* module
- Haskell via libraries `Control.Concurrent.Async` (I-vars) and `Control.Concurrent.MVar` (M-vars)
...
- Some Lisps
  - Clojure
  - MultiLisp
- .NET via `System.Threading.Tasks.Task`
  - C#, since .NET Framework 4.5, via the keywords `async` and `await`
- Kotlin, although `kotlin.native.concurrent.Future` is only usually used when writing Kotlin that is intended to run natively
- Nim
- Oxygene
- Oz version 3
...
- Racket
- Raku
- Rust (future as `std::future::Future`, promise achieved via `.await`)
...
- For Common Lisp:
  ...
  - PCall
- For C++:
  - Boost library, via `boost::thread`
  - Dlib
  - Folly
  - HPX
  - POCO C++ Libraries (Active Results), via `Poco::Thread` and `Poco::Runnable`
  - Qt, via `::QFuture<T>`
  - Seastar
  - stlab
...
- For Lua:
  - The cqueues module contains a Promise API.
