---
title: "URL Pattern API - Web APIs"
...
- Parentheses need to be escaped in range expressions within URLPattern even though they don't in RegExp. `new URLPattern({ pathname: "([()])" }); // throws new URLPattern({ pathname: "([\\(\\)])" }); // ok new RegExp("[()]"); // ok new RegExp("[\\(\\)]"); // ok`
...
The pattern below matches either `/product` or `/product/` but because group delimiter are non-capturing by default, the result is not found in a corresponding match group.
