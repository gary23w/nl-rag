...
`pn` is available as a short alias for `pnpm`, and `pnx` as a short alias for `pnpm dlx`. You can use them anywhere you'd use `pnpm` or `pnpx`:
...
| npm command | pnpm equivalent`npm install``pnpm install``npm i <pkg>``pnpm add <pkg>``npm run <cmd>``pnpm <cmd>``npx <pkg>``pnx <pkg>` |
|---|---|
