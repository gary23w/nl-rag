...
source: https://en.wikipedia.org/wiki/Sheafification
domain: sheaf-theory
...
Let ${\mathcal {F}}$ and ${\mathcal {G}}$ be two sheaves of sets (respectively abelian groups, rings, etc.) on X . A *morphism* ${\displaystyle \varphi$ consists of a morphism $\varphi _{U}:{\mathcal {F}}(U)\to {\mathcal {G}}(U)$ of sets (respectively abelian groups, rings, etc.) for each open set U of X , subject to the condition that this morphism is compatible with restrictions. In other words, for every open subset V of an open set U , the following diagram is commutative.
