...
source: https://en.wikipedia.org/wiki/Membrane_potential
...
By convention it is written as Vm=Vinside−Voutside, so a negative membrane potential means the cell interior is negative relative to the outside. Essentially, It equals the interior potential minus the exterior potential. This is the energy per charge which is required to move a very small positive charge at constant velocity across the cell membrane from the exterior to the interior. Note, though, that if the charge is allowed to change velocity, the change of kinetic energy and production of radiation must be taken into account.
...
Although the entire cell is considered negative inside, the fluids in the cell are electrically neutral. The presence of the voltage is due to the thin layer of charge that accumulates at the surfaces of the cell membrane. The cell membrane is only a few nanometers thick, and as such, even a voltage of 100 mV across this thickness generates an electric field that is extremely strong on the cell membrane.

## Description

All animal cells are surrounded by a membrane composed of a lipid bilayer with proteins embedded in it. The membrane serves both as a capacitor and as a diffusion barrier to the movement of ions. Ion pumps actively push ions across the membrane and establish concentration gradients across the membrane, and ion channels allow ions to move across the membrane down their electrochemical gradients. Ion channels and ion pumps are transmembrane proteins, and are electrically analogous to resistors and current sources (batteries), and contribute to the voltage across the two sides of the membrane.
...
- *E*eq,K+= equilibrium potential for potassium, measured in volts
...
The interactions that generate the resting potential are modeled by the Goldman equation. This is similar in form to the Nernst equation shown above, in that it is based on the charges of the ions in question, as well as the difference between their inside and outside concentrations. However, it also takes into consideration the relative permeability of the plasma membrane to each ion in question. In reality, there is some degree of permeability for several ions simultaneously, so the resting membrane potential is not the same as any one Nernst potential. Rather, it is near a balance point of permeability for the prominent permeant ions (usually potassium, sodium, and chloride). This can be represented by the Goldman-Hodgkin-Katz (GHK) voltage equation conceptually: ions with greater permeability contribute more to Vm. This is why the resting potential is typically close to EK (because PK is typically the largest at rest), but slightly positive compared to ENa depending on sodium leak and other electrical activity.
...
The classic approach is intracellular recording: one electrode is placed inside the cell and a reference electrode remains outside, allowing direct measurement of VmPerkins KL. Cell-attached voltage-clamp and current-clamp recording and stimulation techniques in brain slices.

Modern electrophysiology frequently uses patch-clamp approaches, where a fine glass micropipette is pressed against the membrane to form a very high-resistance seal (giga-seal). This seal isolates a tiny patch of membrane so researchers can record specific electrical signals and, depending on the configuration, control which side of the membrane is exposed to different solutions.

Patch-clamp experiments are usually run in one of two modes: current-clamp, where the experimenter injects a defined current and measures how the cell's membrane potential (Vm) changes over time and voltage-clamp, where the experimenter holds the membrane at a chosen voltage and measures the ionic currents required to keep it there. Voltage-clamp is useful for isolating sodium, potassium, calcium, or synaptic currents.

### Main patch-clamp configurations

*Cell-on-cell recording:* After the giga-seal forms, the membrane remains intact. This is commonly used for single-channel recordings.

*Whole-cell recording:* Starting from a cell-attached seal, the small membrane patch under the pipette tip is ruptured (by suction or a brief electrical pulse). This gives the pipette electrical access to the cell interior, letting researchers record whole-cell currents (voltage-clamp) or whole-cell voltage responses (current-clamp).

*Inside-out patch:* From a cell-attached seal, the pipette is pulled away so a small patch is excised and the intracellular face of the membrane is exposed to the solution. This is especially useful for testing how channels respond to intracellular ligands because the solution on the cytoplasmic side can be controlled.

*Outside-out patch:* After entering whole-cell mode, the pipette is taken back and the membrane reseals at the tip so the extracellular face is exposed to the solution. This is used for studying ligand-gated channels and synaptic receptors because one can add neurotransmitters to the outside-facing surface fast and measure the resulting currents.

*Perforated patch:* Rather than fully breaking the membrane patch, perforated patching uses pore-forming compounds (such as nystatin or amphotericin B) to electrically connect the pipette to the cell while minimizing the washout of intracellular substances.

*Loose-patch recording:* In some situations, a lower-resistance seal is employed to make the recording less invasive. This is more used for recording action potentials or local activity without significantly disrupting the membrane.
