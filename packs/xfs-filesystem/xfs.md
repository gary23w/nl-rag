...
Silicon Graphics began development of XFS ("X" was meant to be filled in later but never was) in 1993 for its UNIX System V based IRIX operating system. The file system was released under the GNU General Public License (GPL) in May 1999.

### Second generation XFS

A team led by Steve Lord at SGI ported XFS to Linux, and first support by a Linux distribution came in 2001. This support gradually became available in almost all Linux distributions.
...
Linux kernel 5.10, released in December 2020, included the new on-disk format XFS v5. This was a hard break, since the deprecated XFS v4 can not be converted to XFS v5. Data on partitions formatted with XFS v4 has to be backed up to another partition or media in order to restore it after formatting the old partition with XFS v5, which completely wipes all data on it. The support for XFS v4 will be removed from the Linux kernel in September 2030.

XFS v5 introduced "bigtime", to store inode timestamps as a 64-bit nanosecond counter instead of the traditional 32-bit seconds counter. This postpones the previous Year 2038 problem until the year 2486. It also introduced metadata checksums.

The Gentoo Handbook, Gentoo Linux's official installation manual, has recommended XFS as the "all-purpose all-platform filesystem" since 28 Jun 2023, succeeding Ext4.
...
XFS supports a separate device called the realtime device, which uses a simple deterministic bitmap allocator instead of the more scalable but more complex B+ tree based allocator normally used in XFS. The realtime device was originally added to support Guaranteed-rate I/O, but acquired other uses later. Today the most common usage is to place metadata on an SSDs while placing data on a hard drive.

### Data block sharing using reflinks

In Linux 4.9, XFS added support for sharing identifical blocks between files, which provides a fast way to clone files and to allower userspace tools to implement out of line Data deduplication. The reflink support is implemented using a new per-allocation group B+ tree tracking the number of references for each physical block, and changes to the write path to perform Copy-on-write when overwriting blocks with an elevated block count.

### Online file system check and autonomous self-healing

As of Linux 6.10, XFS supports online file system checking and repair for all metadata. To support efficient rebuilds of the freespace tracking data structures, a reverse mapping (rmap) B+tree was added, and to provide redundant metadata for the file hierachy, parent pointers that are stored as extended attributes were added as well. As of Linux 7.0, XFS support autonomous self-healing based on these features, with the xfs_healer program automatically initiating repair action as needed.

### Zoned XFS

Linux 6.15 added a new zoned allocator to support Zoned Storage, including hard drives using shingled-magnetic recording (SMR) and NVM Express SSDs using the zoned command set. The zoned allocator works on the realtime device and only stores file data. Metadata uses the existing XFS allocator on the main device. To support SMR hard drives without conventional zones, zoned XFS adds support for an internal RT device, where the main and RT devices sit on the same Linux block device. The zoned allocator always writes out of place, and always writes sequentially in a limited number of open zones. and garbage collection is performed to evacuate valid blocks in partially used zones to create free zones used for further writes. Zoned XFS can use multiple open zones and performs intelligent data placement, leading to better performance than other file systems on zoned storage, and even beating conventional file systems on regular SSDs for some use cases.
...
Attempting on a kernel beyond 3.17 to mount an IRIX XFS volume and write or run Fsck will result in filesystem corruption. IRIX remains on directory structure v2, whereas XFS introduced v3-v5 in later versions and dropped directory structure v2 around kernel 3.17. Data loss and corruption will occur on later Linux kernels.

## Disadvantages

- XFS filesystems cannot (as of December 2023) be shrunk in place, although several possible workarounds have been discussed.
- Journaling cannot be disabled. However, XFS can write to an external journal on a separate block device instead.
