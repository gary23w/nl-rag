...
source: https://en.wikipedia.org/wiki/Open_Sound_System
domain: pulseaudio
...
OSS implements the /dev/audio interface. Detailed access to individual sound devices is provided via the /dev/oss directory. OSS also has MIDI support in /dev/sequencer, /dev/music (both legacy) and /dev/mini.
...
NetBSD supports a compatibility mode for the OSS API, by providing the `soundcard.h` header file and the `libossaudio` library, which internally operate using the native Sun-like audio interface. The replacement was first seen in NetBSD 1.3 of 1998.
