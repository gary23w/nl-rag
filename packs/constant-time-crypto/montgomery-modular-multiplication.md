...
where q is the quotient $\lfloor ab/N\rfloor$ and r, the remainder, is in the interval [0, *N* − 1]. The remainder r is *ab* mod *N*. Determining r can be done by computing q, then subtracting *qN* from *ab*. For example, again with $N=17$ , the product 7 ⋅ 15 is determined by computing $7\cdot 15=105$ , dividing $\lfloor 105/17\rfloor =6$ , and subtracting $105-6\cdot 17=105-102=3$ .
...
For an integer T with $0\leq T<NR$ (as is typical when T arises from multiplying two residues), take its reductions
...
$-N\;\leq \;{\frac {T-{\bigl (}T_{R}N^{-1}{\bmod {R}}{\bigr )}N}{R}}\;<\;N.$
