...
source: https://en.wikipedia.org/wiki/Branch_(computer_science)
...
Some early word-oriented CPU architectures implement conditional "skip the next instructions" operations, branching to NSI, NSI+1 or NSI+2 (PC+1. PC+2, PC+3) depending on the results of the compare. Some simple CPU architectures, still found in microcontrollers, may not implement a conditional jump, but rather only skip and unconditional jump operations. A conditional jump or call is thus implemented as a conditional skip of an unconditional jump or call instruction.
