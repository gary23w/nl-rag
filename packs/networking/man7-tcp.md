...
TCP_DEFER_ACCEPT (since Linux 2.4)
       ...
       complete the connection.  The kernel will round this value
       up to the next larger TCP retransmit duration.

       On a client socket, if nonzero, delays sending the final
       ACK of the three-way handshake until the socket has data to
       send, in which case the ACK is piggybacked on the first
       data segment, or a delayed-ACK timeout is reached.
