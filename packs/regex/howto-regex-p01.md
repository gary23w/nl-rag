---
title: "Regular expression HOWTO (part 1/2)"
...
In addition, special escape sequences that are valid in regular expressions, but not valid as Python string literals, now result in a `SyntaxWarning` and will eventually become a `SyntaxError`, which means the sequences will be invalid if raw string notation or escaping the backslashes isn’t used.
...
The `r` prefix, making the literal a raw string literal, is needed in this example because escape sequences in a normal “cooked” string literal that are not recognized by Python, as opposed to regular expressions, now result in a `SyntaxWarning` and will eventually become a `SyntaxError`. See The backslash plague.
