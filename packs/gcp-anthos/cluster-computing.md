...
source: https://en.wikipedia.org/wiki/Cluster_(computing)
domain: gcp-anthos
...
In the early-to-mid-1970s, the General Electric Mark II adopted a high-reliability cluster technology, in which up to eight processing systems (each with its own copy of the operating system) had access to multiple file systems with file-level locking.

The first commercial loosely coupled cluster computer with integrated system-wide manageability and record-level file locking was the *Tandem NonStop* (a 1976 high-availability commercial product) closely followed by Datapoint Corporation's "Attached Resource Computer" (ARC) system, developed in 1977, and using ARCnet as the cluster interface. In 1984, AT&T released its B3 series computers including a fault-tolerant clustered computing system of two processors. Clustering per se did not really take off until Digital Equipment Corporation released their VAXcluster product in 1984 for the VMS operating system which, unlike earlier manufacturers, used the term "cluster" in its marketing. The Tandem NonStop, ARC and VAXcluster products not only supported parallel computing, but also shared file systems and peripheral devices. The idea was to provide the advantages of parallel processing, while maintaining data reliability and uniqueness. Another noteworthy commercial cluster was the *IBM S/390 Parallel Sysplex* (circa 1994, primarily for business use).
...
Clusters were used for the majority of the world's automated teller machine transactions, credit cards, emergency 911 calls, stock exchanges, largest data warehouses and more by the 1990s if not earlier.
...
Scalability is enhanced by adding systems to a cluster, which is the primary basis of massive parallelism. Unlike symmetric multi-processor systems, where scalability eventually fades as more processors are added (and thus either becomes more expensive or not scalable enough), whereas, for some applications, scalability can be maintained in a loosely-coupled cluster system. This scalability may require cluster-aware middleware or database to work well.
...
If a large number of computers are clustered together, this lends itself to the use of distributed file systems and RAID, both of which can increase the reliability and speed of a cluster.
...
Microsoft Windows Cluster Server software, based on the Windows Server platform, originally released in 1997/98, provides cluster support for high-performance computing including the job scheduler, MSMPI library and management tools.
