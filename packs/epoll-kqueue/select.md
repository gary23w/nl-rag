---
title: "select(2)"
source: https://man7.org/linux/man-pages/man2/select.2.html
domain: epoll-kqueue
...
caller to wait for three classes of events on the specified set of
file descriptors.  Each of the fd_set arguments may be specified
as NULL if no file descriptors are to be watched for the
...
FD_ISSET()
       ...
       the file descriptor fd is present in set, and zero if it is
       not.
...
by the one pointed to by sigmask, then does the "select" function,
and then restores the original signal mask.  (If sigmask is NULL,
...
other end is monitored by select() in the main program.  (To avoid
possibly blocking when writing to a pipe that may be full or
reading from a pipe that may be empty, nonblocking I/O is used
...
nfds.  However, in the glibc implementation, the fd_set type is
