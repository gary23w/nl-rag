---
title: "Type system"
source: https://en.wikipedia.org/wiki/Type_hint
domain: pydantic-validation
...
- ${\displaystyle \tau [\alpha$ (resp. $\tau [x:=N]$ ) describes the type which results from replacing all occurrences of the type variable α (resp. term variable x) in $\tau$ by the type σ (resp. term N).
...
| Polymorphic | $\forall {}\alpha .\tau$ | If $M:\forall {}\alpha .\tau$ , then ${\displaystyle M:\tau [\alpha$ for any type σ. |
| Existential | $\exists {}\alpha .\tau$ | If $M:\exists {}\alpha .\tau$ , then ${\displaystyle M:\tau [\alpha$ for some type σ. |
| Recursive | $\mu \alpha .\tau$ | If $M:\mu \alpha .\tau$ , then ${\displaystyle M:\tau [\alpha$ . |
