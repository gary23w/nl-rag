...
source: https://en.wikipedia.org/wiki/Link_function
...
The *dispersion parameter*, $\tau$ , typically is known and is usually related to the variance of the distribution. The functions $h(\mathbf {y} ,\tau )$ , $\mathbf {b} ({\boldsymbol {\theta }})$ , $\mathbf {T} (\mathbf {y} )$ , $A({\boldsymbol {\theta }})$ , and $d(\tau )$ are known. Many common distributions are in this family, including the normal, exponential, gamma, Poisson, Bernoulli, and (for fixed number of trials) binomial, multinomial, and negative binomial.

For scalar $\mathbf {y}$ and ${\boldsymbol {\theta }}$ (denoted y and $\theta$ in this case), this reduces to

$f_{Y}(y\mid \theta ,\tau )=h(y,\tau )\exp \left({\frac {b(\theta )T(y)-A(\theta )}{d(\tau )}}\right).\,\!$

${\boldsymbol {\theta }}$ is related to the mean of the distribution. Original formulation by John Nelder and Robert Wedderburn sets $T(y)=y$ , which is a key requirement for having unbiased estimating equations for $\theta$ that yields many attractive properties, including consistency of maximum likelihood estimate under distributional misspecification. If $\mathbf {b} ({\boldsymbol {\theta }})$ is the identity function, then the distribution is said to be in canonical form (or *natural form*). Note that any distribution can be converted to canonical form by rewriting ${\boldsymbol {\theta }}$ as ${\boldsymbol {\theta }}'$ and then applying the transformation ${\boldsymbol {\theta }}=\mathbf {b} ({\boldsymbol {\theta }}')$ . It is always possible to convert $A({\boldsymbol {\theta }})$ in terms of the new parametrization, even if $\mathbf {b} ({\boldsymbol {\theta }}')$ is not a one-to-one function; see comments in the page on exponential families.

If, in addition, $\mathbf {T} (\mathbf {y} )$ and $\mathbf {b} ({\boldsymbol {\theta }})$ are the identity, then ${\boldsymbol {\theta }}$ is called the *canonical parameter* (or *natural parameter*) and is related to the mean through

${\boldsymbol {\mu }}=\operatorname {E} (\mathbf {y} )=\nabla _{\boldsymbol {\theta }}A({\boldsymbol {\theta }}).\,\!$

For scalar $\mathbf {y}$ and ${\boldsymbol {\theta }}$ , this reduces to
...
When using a distribution function with a canonical parameter $\theta ,$ the canonical link function is the function that expresses $\theta$ in terms of $\mu ,$ i.e. $\theta =g(\mu ).$ For the most common distributions, the mean $\mu$ is one of the parameters in the standard form of the distribution's density function, and then $g(\mu )$ is the function as defined above that maps the density function into its canonical form. When using the canonical link function, $g(\mu )=\theta =\mathbf {X} {\boldsymbol {\beta }},$ which allows $\mathbf {X} ^{\rm {T}}\mathbf {Y}$ to be a sufficient statistic for ${\boldsymbol {\beta }}$ .
...
| Continuous binomial | real: $(0,1)$ or $[0,1]$ | modeling continuous proportions (%) | cobit | no closed form | $\mu =1/(1-\exp(-\mathbf {X} {\boldsymbol {\beta }}))-1/(\mathbf {X} {\boldsymbol {\beta }})\,\!$ |
...
When the response data, *Y*, are binary (taking on only values 0 and 1), the distribution function is generally chosen to be the Bernoulli distribution and the interpretation of *μ*i is then the probability, *p*, of *Y*i taking on the value one.
...
When a response Y is double-bounded continuous random variable, after standardizing it into the unit interval [0,1], an example of generalized linear model is continuous binomial (cobin) regression based on continuous binomial distribution, a two-parameter generalization of continuous Bernoulli distribution. An additional parameter $\lambda$ is inversely proportional to variance,

$\operatorname {var} (Y_{i})=\lambda ^{-1}A''(A^{-1}(\mu _{i})),\,$

where $A(\theta )=\log\{(e^{\theta }-1)/\theta \}$ . It is related to fractional regression models based on quasi-maximum likelihood if the variance function is chosen differently.
...
Generalized additive models (GAMs) are another extension to GLMs in which the linear predictor *η* is not restricted to be linear in the covariates **X** but is the sum of smoothing functions applied to the *xi*s:
