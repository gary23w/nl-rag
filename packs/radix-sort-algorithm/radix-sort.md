...
MSD radix sorts are most suitable for sorting strings or fixed-length integer representations. A sequence like **[b, c, e, d, f, g, ba]** would be sorted as **[b, ba, c, d, e, f, g]**. If lexicographic ordering is used to sort variable-length integers in base 10, then numbers from 1 to 10 would be output as **[1, 10, 2, 3, 4, 5, 6, 7, 8, 9]**, as if the shorter keys were left-justified and padded on the right with blank characters to make the shorter keys as long as the longest key. MSD sorts are not necessarily stable.
...
Notice that an implicit digit

0

is prepended for the two 2s so that 802 maintains its position between them.
...
is prepended to all of the 1- or 2-digit numbers.
...
Radix sort operates in $O(d\cdot n)$ time, where n is the number of keys, and d is the key length in digits (e.g. 8-bit digits). LSD variants can achieve a lower bound for d of 'average key length' when splitting variable length keys into groups as discussed above.

LSD Radix sort can run in linear time $\Theta (d(n+k))$ , where d is the number of digits in each array element with each digit in the range of 0 to $k-1$ , such as 0 to 255 for an 8-bit digit. This holds when d is constant, i.e. $\Theta (1)$ and k is upper bounded by n , i.e. $O(n)$ . No matter how large the value of n , it makes d passes through the data. MSD Radix Sort uses just one pass in the best case, and in the worst case, it performs like LSD Radix sort.
...
Performance of LSD radix sort correlates with the number of passes over the array, which is $\Theta (2d(n+k))$ . Two passes over the array are performed per digit: one pass for counting, followed by the permutation pass. The number of passes can be reduced to $\Theta ((d+1)(n+k))$ by performing counting for all the digits of the key in a single pass. This is possible because the counts are always over the entire array, and permuting elements of the array does not change the resulting counts.
