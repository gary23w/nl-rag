...
The last step of bucket sort, which is **concatenating** all the sorted objects in each bucket, requires $O(k)$ time. Therefore, the total complexity is $O\left(n+{\frac {n^{2}}{k}}+k\right)$ . Note that if k is chosen to be $k=\Theta (n)$ , then bucket sort runs in $O(n)$ average time, given a uniformly distributed input.
