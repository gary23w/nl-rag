...
– Abnormal termination of the process. The process is terminated with all the consequences of
...
– Abnormal termination of the process. Additionally, implementation-defined abnormal termination actions, such as creation of a core file, may occur.
...
signal is sent to a process to tell it to

abort

, i.e. to terminate. The signal is usually initiated by the process itself when it calls
...
indicates that the CPU has executed an explicit "trap" instruction (without a defined function), or an unimplemented instruction (when emulation is unavailable).
...
are typically the same signal, and receipt of that signal may indicate any of the conditions above.
...
signals are sent to a process when the corresponding time limit is reached. The process sets these time limits by calling
...
is based on real or clock time;

SIGVTALRM

is based on CPU time used by the process; and

SIGPROF

is based on CPU time used by the process and by the system on its behalf (known as a
...
may be used internally by the implementation of the
...
signal is sent to a process when it causes a
...
signal is sent to a process when a

child process

terminates

, is stopped, or resumes after being stopped. One common usage of the signal is to instruct the operating system to clean up the resources used by a child process after its termination without an explicit call to the
...
signal instructs the operating system to

continue

(restart) a process previously paused by the
...
signal. One important use of this signal is in
...
signal is sent to a process when an exceptional (but not necessarily erroneous) condition has been detected in the floating-point or integer arithmetic hardware. This may include
...
signal is sent to a process when its controlling terminal is closed. It was originally designed to notify the process of a
...
). In modern systems, this signal usually means that the controlling

pseudo or virtual terminal
...
(who have no controlling terminal) interpret receipt of this signal as a request to reload their configuration files and flush/reopen their logfiles instead of exiting.
...
signal is sent to a process when it attempts to execute an

illegal

, malformed, unknown, or privileged
...
signal is sent to a process by its controlling terminal when a user wishes to

interrupt

the process. This is typically initiated by pressing
...
signal is sent to a process to cause it to terminate immediately (
...
, this signal cannot be caught or ignored, and the receiving process cannot perform any clean-up upon receiving this signal. The following exceptions apply:
...
is used as a last resort when terminating processes in most system

shutdown

procedures if it does not voluntarily exit in response to

SIGTERM

. To speed the computer shutdown procedure, Mac OS X 10.6, aka
...
to applications that have marked themselves "clean" resulting in faster shutdown times with, presumably, no ill effects.

The command

killall -9

has a similar, while dangerous effect, when executed e.g. in Linux; it does not let programs save unsaved data. It has other options, and with none, uses the safer SIGTERM signal.
...
signal is sent to a process when it attempts to write to a
...
signal is sent when an event occurred on an explicitly watched file descriptor.

Using it effectively leads to making

asynchronous I/O

requests since the kernel will

poll

the descriptor in place of the caller. It provides an alternative to active
...
signals are intended to be used for user-defined purposes. They are
...
signal is sent to a process by its controlling terminal when the user requests that the process
...
signal is sent to a process when it passes a bad argument to a
...
can be received by applications violating the Linux

Seccomp

security rules configured to restrict them.

SIGSYS

can also be used to emulate foreign system calls, e.g. emulate Windows system calls on Linux.
...
signal is sent to a process to request its
...
signal, it can be caught and interpreted or ignored by the process. This allows the process to perform nice termination releasing resources and saving state if appropriate.
...
signal is sent to a process by its controlling
...
). It is commonly initiated by the user pressing
...
, the process can register a signal handler for, or ignore, the signal.
...
signals are sent to a process when it attempts to read
...
. Typically, these signals are received only by processes under
...
do not have controlling terminals and, therefore, should never receive these signals.
...
signal is sent to a process when an exception (or

trap

) occurs: a condition that a
...
is executed, or when a particular
...
signals are sent to a process to indicate
...
signal is sent to a process when it has used up the
...
a certain predetermined user-settable value.

The arrival of a

SIGXCPU

signal provides the receiving process a chance to quickly save any intermediate results and to exit gracefully, before it is terminated by the operating system using the
...
signal is sent to a process when it grows a
...
signal is sent to a process when its controlling terminal changes its size (a
...
signal is sent to a process when an
...
instruction (EMT was the instruction for this purpose on the
...
signal is sent to a process when a status (

info

) request is received from the controlling terminal.
...
signal is sent to a process when the system experiences a
...
signal is sent to a process when a file lock is
...
signal is sent to a process when the coprocessor experiences a
...
coprocessor stack fault will generate
...
signal is sent to a process when a system call with an

unused

system call number is made. It is synonymous with
