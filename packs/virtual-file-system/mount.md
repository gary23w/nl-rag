...
ENOSPC Number of mounts will exceed the limit after this syscall
       in some mount namespaces.  This limit is controlled by
       /proc/sys/fs/mount-max, see proc_sys_fs(5) for details of
       this limit.  that this limit applies both to the mount
       namespace where the mount operation occurs and to any mount
       namespaces to which the mount will propagate.  See
       mount_namespaces(7) for details of mount namespaces and
       mount propagation.
