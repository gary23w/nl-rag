...
Actor priority only takes effect when Dramatiq is choosing which message to run. This only happens *after* messages are consumed from the queues. The actory priority option has **no effect on messages while they are waiting in a queue**. It only applies once messaged are consumed from the queues, and are waiting in-memory to be processed.
...
That way if messages for both `generate_report` and `sync_order_to_warehouse` have been consumed from the queue at the same time but there’s only capacity to run one of them, `generate_report` will always run *first*.
...
The `RabbitmqBroker` supports prioritizing messages while they are waiting in the queue with the `broker_priority` option.
...
To help surface actor exceptions, by default, the stub broker will re-raise exceptions from failed messages in your main thread when you call its `StubBroker.join` method:
