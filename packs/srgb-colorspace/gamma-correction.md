...
The concept of gamma can be applied to any nonlinear relationship. For the power-law relationship $V_{\text{out}}=V_{\text{in}}^{\gamma }$ , the curve on a log–log plot is a straight line, with slope everywhere equal to gamma (slope is represented here by the derivative operator):
...
A CRT, for example, converts a video signal to light in a nonlinear way, because the electron gun's intensity (brightness) as a function of applied video voltage is nonlinear. The light intensity *I* is related to the source voltage *V*s according to
...
- *luma* is the encoded video brightness signal, i.e., similar to the signal voltage *V**S*.
