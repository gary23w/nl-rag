---
title: "Denotational semantics of the Actor model"
...
The mathematical denotation for a system S is found by constructing increasingly better approximations from an initial empty denotation called ⊥S using some denotation approximating function **progression**S to construct a denotation (meaning ) for S as follows:
...
$\mathbf {progression} _{\mathtt {S}}(\lim _{i\to \infty }{\mathtt {x}}_{i})=\lim _{i\to \infty }\mathbf {progression} _{\mathtt {S}}({\mathtt {x}}_{i})$
...
The augmented Actor event diagrams [see
...
> from which to construct the power domain
...
below). The augmented diagrams are partial computation histories representing "snapshots" [relative to some frame of reference] of a computation on its way to being completed. For
...
[see William Wadge 1979]. Concretely, the completed elements are those having non pending events. Intuitively,
...
To repeat, the actor event diagram domain
...
For the actor event diagram domain
...
, the requirement that elements of
...
be downward-closed has a clear basis in intuition.
...
which defines a domain of resumptions [Gordon Plotkin 1976]. However,
...
, the meaning of program is a specification of the computations that may be performed by the program. The computations are represented formally by Actor event diagrams. Greif specified the event diagrams by means of causal axioms governing the behaviors of individual Actors [Greif 1975].
...
, which represent messages on the way to their targets. Each pending event must become an actual (realized) arrival event sooner or later, a requirement referred to as

finite delay

. Augmenting Actor event diagrams with sets of pending events helps to express the finite delay property, which is characteristic of true concurrency [Schwartz 1979].
...
From the Actor point of view, sequential computations are a special case of concurrent computations, distinguishable by their event diagrams. The event diagram of a sequential computation has an initial event, and no event activates more than one event. In other words, the activation ordering of a sequential computation is linear; the event diagram is essentially a conventional execution sequence. This means that the finite elements of
...
. Thus Actor semantics includes sequential programs as a special case, and agrees with conventional semantics of such programs.
...
2. The finite elements of **TimedDiagrams** are countable where an element xε**TimedDiagrams** is finite (isolated) if and only if D⊆**TimedDiagrams** is directed and x≤VD, there exists dεD with x≤d. In other words, x is finite if one must go through x in order to get up to or above x via the limit process.
...
- Note: Although Power[**TimedDiagrams**] is ordered by ⊆, limits are not given by *U*. I.e., (∀i∈ω Mi≤Mi+1) ⇒ *U*i∈ω Mi ⊆ ⊔i∈ω Mi E.g., If ∀i diε**TimedDiagrams** and di≤di+1 and Mi= {dk | k ≤i} then {{{1}}}
