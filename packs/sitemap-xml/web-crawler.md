...
Marc Najork and Janet Wiener performed an actual crawl on 328 million pages, using breadth-first ordering. They found that a breadth-first crawl captures pages with high Pagerank early in the crawl (but they did not compare this strategy against other strategies). The explanation given by the authors for this result is that "the most important pages have many links to them from numerous hosts, and those links will be found early, regardless of on which host or page the crawl originates."
...
Website owners may instruct crawlers on what resources they are allowed and forbidden to request with the robots.txt file. A crawler can then use that information to chose what links to follow and which to avoid.
...
$F_{p}(t)={\begin{cases}1&{\rm {if}}~p~{\rm {~is~equal~to~the~local~copy~at~time}}~t\\0&{\rm {otherwise}}\end{cases}}$
...
The first proposed interval between successive page loads was 60 seconds. However, if pages were downloaded at this rate from a website with more than 100,000 pages over a perfect connection with zero latency and infinite bandwidth, it would take more than 2 months to download only that entire Web site; also, only a fraction of the resources from that Web server would be used.
...
- Yahoo! Slurp was the name of the Yahoo! web crawler until Yahoo! contracted with Microsoft to use Bingbot instead.

### In-house web crawlers

- Applebot is Apple's web crawler. It supports Siri and other products.
...
- Grub was an open source distributed web crawler that Wikia Search used.
...
- mnoGoSearch was a crawler, indexer and a search engine written in C and licensed under the GPL which was in operation from 2000 to 2022.
- Open Search Server is a search engine and web crawler software release under the GPL.
- Scrapy, an open source web crawler framework, written in python (licensed under BSD).
