...
Mathematical analysis of quicksort shows that, on average, the algorithm takes O ( n log ⁡ n ) {\displaystyle O(n\log {n})} ({\displaystyle O(n\log {n})}) comparisons to sort *n* items. In the worst case, it makes O ( n 2 ) {\displaystyle O(n^{2})} ({\displaystyle O(n^{2})}) comparisons.
...
If this happens repeatedly in every partition, then each recursive call processes a list of size one less than the previous list. Consequently, it takes *n* − 1 nested calls before reaching a list of size 1. This means that the call tree is a linear chain of *n* − 1 nested calls. The ith call does *O*(*n* − *i*) work to do the partition, and ∑ i = 0 n ( n − i ) = O ( n 2 ) {\displaystyle \textstyle \sum _{i=0}^{n}(n-i)=O(n^{2})} ({\displaystyle \textstyle \sum _{i=0}^{n}(n-i)=O(n^{2})}), so in that case quicksort takes *O*(*n*2) time.
...
The following binary search tree (BST) corresponds to each execution of quicksort: the initial pivot is the root node; the pivot of the left half is the root of the left subtree, the pivot of the right half is the root of the right subtree, and so on. The number of comparisons of the execution of quicksort equals the number of comparisons during the construction of the BST by a sequence of insertions. So, the average number of comparisons for randomized quicksort equals the average cost of constructing a BST when the values inserted ( x 1 , x 2 , … , x n ) {\displaystyle (x_{1},x_{2},\ldots ,x_{n})} ({\displaystyle (x_{1},x_{2},\ldots ,x_{n})}) form a random permutation.

Consider a BST created by the insertion of a sequence ( x 1 , x 2 , … , x n ) {\displaystyle (x_{1},x_{2},\ldots ,x_{n})} ({\displaystyle (x_{1},x_{2},\ldots ,x_{n})}) of values forming a random permutation. Let C denote the cost of creation of the BST. We have C = ∑ i ∑ j < i c i , j {\displaystyle C=\sum _{i}\sum _{j<i}c_{i,j}} ({\displaystyle C=\sum _{i}\sum _{j<i}c_{i,j}}), where c i , j {\displaystyle c_{i,j}} ({\displaystyle c_{i,j}}) is a binary random variable expressing whether during the insertion of x i {\displaystyle x_{i}} ({\displaystyle x_{i}}) there was a comparison to x j {\displaystyle x_{j}} ({\displaystyle x_{j}}).

By linearity of expectation, the expected value E ⁡ [ C ] {\displaystyle \operatorname {E} [C]} ({\displaystyle \operatorname {E} [C]}) of C is E ⁡ [ C ] = ∑ i ∑ j < i Pr ( c i , j ) {\displaystyle \operatorname {E} [C]=\sum _{i}\sum _{j<i}\Pr(c_{i,j})} ({\displaystyle \operatorname {E} [C]=\sum _{i}\sum _{j<i}\Pr(c_{i,j})}).

Fix i and *j*<*i*. The values x 1 , x 2 , … , x j {\displaystyle {x_{1},x_{2},\ldots ,x_{j}}} ({\displaystyle {x_{1},x_{2},\ldots ,x_{j}}}), once sorted, define *j*+1 intervals. The core structural observation is that x i {\displaystyle x_{i}} ({\displaystyle x_{i}}) is compared to x j {\displaystyle x_{j}} ({\displaystyle x_{j}}) in the algorithm if and only if x i {\displaystyle x_{i}} ({\displaystyle x_{i}}) falls inside one of the two intervals adjacent to x j {\displaystyle x_{j}} ({\displaystyle x_{j}}).

Observe that since ( x 1 , x 2 , … , x n ) {\displaystyle (x_{1},x_{2},\ldots ,x_{n})} ({\displaystyle (x_{1},x_{2},\ldots ,x_{n})}) is a random permutation, ( x 1 , x 2 , … , x j , x i ) {\displaystyle (x_{1},x_{2},\ldots ,x_{j},x_{i})} ({\displaystyle (x_{1},x_{2},\ldots ,x_{j},x_{i})}) is also a random permutation, so the probability that x i {\displaystyle x_{i}} ({\displaystyle x_{i}}) is adjacent to x j {\displaystyle x_{j}} ({\displaystyle x_{j}}) is exactly 2 j + 1 {\displaystyle {\frac {2}{j+1}}} ({\displaystyle {\frac {2}{j+1}}}).
...
Stack-free versions of Quicksort have been proposed. These use O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) additional space (more precisely, one cell of the type of the sorted records, in order to exchange records, and a constant number of integer variables used as indices).
