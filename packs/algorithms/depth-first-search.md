...
Interactive depth-first search demonstration
...
A non-recursive implementation of DFS with worst-case space complexity O ( | E | ) {\displaystyle O(|E|)} ({\displaystyle O(|E|)}), with the possibility of duplicate vertices on the stack:
