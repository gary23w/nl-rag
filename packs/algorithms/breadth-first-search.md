...
Let G = ( V , E ) {\displaystyle G=(V,E)} ({\displaystyle G=(V,E)}) be a graph with n {\displaystyle n} ({\displaystyle n}) vertices. Recall that N ( v ) {\displaystyle N(v)} ({\displaystyle N(v)}) is the set of neighbors of v {\displaystyle v} ({\displaystyle v}). Let σ = ( v 1 , … , v m ) {\displaystyle \sigma =(v_{1},\dots ,v_{m})} ({\displaystyle \sigma =(v_{1},\dots ,v_{m})}) be a list of distinct elements of V {\displaystyle V} ({\displaystyle V}), for v ∈ V ∖ { v 1 , … , v m } {\displaystyle v\in V\setminus \{v_{1},\dots ,v_{m}\}} ({\displaystyle v\in V\setminus \{v_{1},\dots ,v_{m}\}}), let ν σ ( v ) {\displaystyle \nu _{\sigma }(v)} ({\displaystyle \nu _{\sigma }(v)}) be the least i {\displaystyle i} ({\displaystyle i}) such that v i {\displaystyle v_{i}} ({\displaystyle v_{i}}) is a neighbor of v {\displaystyle v} ({\displaystyle v}), if such a i {\displaystyle i} ({\displaystyle i}) exists, and be ∞ {\displaystyle \infty } ({\displaystyle \infty }) otherwise.

Let σ = ( v 1 , … , v n ) {\displaystyle \sigma =(v_{1},\dots ,v_{n})} ({\displaystyle \sigma =(v_{1},\dots ,v_{n})}) be an enumeration of the vertices of V {\displaystyle V} ({\displaystyle V}). The enumeration σ {\displaystyle \sigma } ({\displaystyle \sigma }) is said to be a BFS ordering (with source v 1 {\displaystyle v_{1}} ({\displaystyle v_{1}})) if, for all 1 < i ≤ n {\displaystyle 1<i\leq n} ({\displaystyle 1<i\leq n}), v i {\displaystyle v_{i}} ({\displaystyle v_{i}}) is the vertex w ∈ V ∖ { v 1 , … , v i − 1 } {\displaystyle w\in V\setminus \{v_{1},\dots ,v_{i-1}\}} ({\displaystyle w\in V\setminus \{v_{1},\dots ,v_{i-1}\}}) such that ν ( v 1 , … , v i − 1 ) ( w ) {\displaystyle \nu _{(v_{1},\dots ,v_{i-1})}(w)} ({\displaystyle \nu _{(v_{1},\dots ,v_{i-1})}(w)}) is minimal. Equivalently, σ {\displaystyle \sigma } ({\displaystyle \sigma }) is a BFS ordering if, for all 1 ≤ i < j < k ≤ n {\displaystyle 1\leq i<j<k\leq n} ({\displaystyle 1\leq i<j<k\leq n}) with v i ∈ N ( v k ) ∖ N ( v j ) {\displaystyle v_{i}\in N(v_{k})\setminus N(v_{j})} ({\displaystyle v_{i}\in N(v_{k})\setminus N(v_{j})}), there exists a neighbor v m {\displaystyle v_{m}} ({\displaystyle v_{m}}) of v j {\displaystyle v_{j}} ({\displaystyle v_{j}}) such that m < i {\displaystyle m<i} ({\displaystyle m<i}).
