...
source: https://en.wikipedia.org/wiki/Community_structure
...
Consider a stochastic block model with total n nodes, $q=2$ groups of equal size, and let $p_{\text{in}}$ and $p_{\text{out}}$ be the connection probabilities inside and between the groups respectively. If $p_{\text{in}}>p_{\text{out}}$ , the network would possess community structure since the link density inside the groups would be more than the density of links between the groups. In the sparse case, $p_{\text{in}}$ and $p_{\text{out}}$ scale as $O(1/n)$ so that the average degree is constant:
