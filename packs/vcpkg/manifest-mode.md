...
vcpkg has two operation modes: classic mode and manifest mode. For most users, we recommend manifest mode.

Manifest mode uses declarative JSON files to describe metadata about your project or package. Manifest files are required to have the `vcpkg.json` name.
...
- Read the `vcpkg.json` and `vcpkg-configuration.json` reference articles.
