...
|---|---|---|---|
| Store graph | O ( \| V \| + \| E \| ) {\displaystyle O(\|V\|+\|E\|)} ({\displaystyle O(\|V\|+\|E\|)}) | O ( \| V \| 2 ) {\displaystyle O(\|V\|^{2})} ({\displaystyle O(\|V\|^{2})}) | O ( \| V \| ⋅ \| E \| ) {\displaystyle O(\|V\|\cdot \|E\|)} ({\displaystyle O(\|V\|\cdot \|E\|)}) |
| Add vertex | O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) | O ( \| V \| 2 ) {\displaystyle O(\|V\|^{2})} ({\displaystyle O(\|V\|^{2})}) | O ( \| V \| ⋅ \| E \| ) {\displaystyle O(\|V\|\cdot \|E\|)} ({\displaystyle O(\|V\|\cdot \|E\|)}) |
| Add edge | O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) | O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) | O ( \| V \| ⋅ \| E \| ) {\displaystyle O(\|V\|\cdot \|E\|)} ({\displaystyle O(\|V\|\cdot \|E\|)}) |
| Remove vertex | O ( \| E \| ) {\displaystyle O(\|E\|)} ({\displaystyle O(\|E\|)}) | O ( \| V \| 2 ) {\displaystyle O(\|V\|^{2})} ({\displaystyle O(\|V\|^{2})}) | O ( \| V \| ⋅ \| E \| ) {\displaystyle O(\|V\|\cdot \|E\|)} ({\displaystyle O(\|V\|\cdot \|E\|)}) |
| Remove edge | O ( \| V \| ) {\displaystyle O(\|V\|)} ({\displaystyle O(\|V\|)}) | O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) | O ( \| V \| ⋅ \| E \| ) {\displaystyle O(\|V\|\cdot \|E\|)} ({\displaystyle O(\|V\|\cdot \|E\|)}) |
| Are vertices *x* and *y* adjacent (assuming that their storage positions are known)? | O ( \| V \| ) {\displaystyle O(\|V\|)} ({\displaystyle O(\|V\|)}) | O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) | O ( \| E \| ) {\displaystyle O(\|E\|)} ({\displaystyle O(\|E\|)}) |
| Remarks | Slow to remove vertices and edges, because it needs to find all vertices or edges | Slow to add or remove vertices, because matrix must be resized/copied | Slow to add or remove vertices and edges, because matrix must be resized/copied |

Adjacency lists are generally preferred for the representation of sparse graphs, while an adjacency matrix is preferred if the graph is dense; that is, the number of edges | E | {\displaystyle |E|} ({\displaystyle |E|}) is close to the number of vertices squared, | V | 2 {\displaystyle |V|^{2}} ({\displaystyle |V|^{2}}), or if one must be able to quickly look up if there is an edge connecting two vertices.

### More efficient representation of adjacency sets

The time complexity of operations in the adjacency list representation can be improved by storing the sets of adjacent vertices in more efficient data structures, such as hash tables or balanced binary search trees (the latter representation requires that vertices are identified by elements of a linearly ordered set, such as integers or character strings). A representation of adjacent vertices via hash tables leads to an amortized average time complexity of O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) to test adjacency of two given vertices and to remove an edge and an amortized average time complexity of O ( deg ⁡ ( x ) ) {\displaystyle O(\deg(x))} ({\displaystyle O(\deg(x))}) to remove a given vertex x of degree deg ⁡ ( x ) {\displaystyle \deg(x)} ({\displaystyle \deg(x)}). The time complexity of the other operations and the asymptotic space requirement do not change.
...
1D partitioning: Every processor gets n / p {\displaystyle n/p} ({\displaystyle n/p}) vertices and the corresponding outgoing edges. This can be understood as a row-wise or column-wise decomposition of the adjacency matrix. For algorithms operating on this representation, this requires an All-to-All communication step as well as O ( m ) {\displaystyle {\mathcal {O}}(m)} ({\displaystyle {\mathcal {O}}(m)}) message buffer sizes, as each PE potentially has outgoing edges to every other PE.

2D partitioning: Every processor gets a submatrix of the adjacency matrix. Assume the processors are aligned in a rectangle p = p r × p c {\displaystyle p=p_{r}\times p_{c}} ({\displaystyle p=p_{r}\times p_{c}}), where p r {\displaystyle p_{r}} ({\displaystyle p_{r}}) and p c {\displaystyle p_{c}} ({\displaystyle p_{c}}) are the amount of processing elements in each row and column, respectively. Then each processor gets a submatrix of the adjacency matrix of dimension ( n / p r ) × ( n / p c ) {\displaystyle (n/p_{r})\times (n/p_{c})} ({\displaystyle (n/p_{r})\times (n/p_{c})}). This can be visualized as a checkerboard pattern in a matrix. Therefore, each processing unit can only have outgoing edges to PEs in the same row and column. This bounds the amount of communication partners for each PE to p r + p c − 1 {\displaystyle p_{r}+p_{c}-1} ({\displaystyle p_{r}+p_{c}-1}) out of p = p r × p c {\displaystyle p=p_{r}\times p_{c}} ({\displaystyle p=p_{r}\times p_{c}}) possible ones.
