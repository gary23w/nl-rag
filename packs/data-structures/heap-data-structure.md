...
For a binary heap, in the array, the first index contains the root element. The next two indices of the array contain the root's children. The next four indices contain the four children of the root's two child nodes, and so on. Therefore, given a node at index i, its children are at indices ⁠ 2 i + 1 {\displaystyle 2i+1} ({\displaystyle 2i+1})⁠ and ⁠ 2 i + 2 {\displaystyle 2i+2} ({\displaystyle 2i+2})⁠, and its parent is at index ⌊(*i*−1)/2⌋ in an array starting from index ⁠ 0 {\displaystyle 0} ({\displaystyle 0})⁠, or at ⁠ 2 i {\displaystyle 2i} ({\displaystyle 2i})⁠, ⁠ 2 i + 1 {\displaystyle 2i+1} ({\displaystyle 2i+1})⁠, and ⌊*i*/2⌋, respectively, in an array starting from ⁠ 1 {\displaystyle 1} ({\displaystyle 1})⁠. This simple indexing scheme makes it efficient to walk "up" or "down" the tree.
...
1. Each insertion takes O(log(*k*)) in the existing size of the heap, thus ∑ k = 1 n O ( log ⁡ k ) {\displaystyle \sum _{k=1}^{n}O(\log k)} ({\displaystyle \sum _{k=1}^{n}O(\log k)}). Since log ⁡ n / 2 = ( log ⁡ n ) − 1 {\displaystyle \log n/2=(\log n)-1} ({\displaystyle \log n/2=(\log n)-1}), a constant factor (half) of these insertions are within a constant factor of the maximum, so asymptotically we can assume k = n {\displaystyle k=n} ({\displaystyle k=n}); formally the time is n O ( log ⁡ n ) − O ( n ) = O ( n log ⁡ n ) {\displaystyle nO(\log n)-O(n)=O(n\log n)} ({\displaystyle nO(\log n)-O(n)=O(n\log n)}). This can also be readily seen from Stirling's approximation.
...
4. Lower bound of Ω ( log ⁡ log ⁡ n ) , {\displaystyle \Omega (\log \log n),} ({\displaystyle \Omega (\log \log n),}) upper bound of O ( 2 2 log ⁡ log ⁡ n ) . {\displaystyle O(2^{2{\sqrt {\log \log n}}}).} ({\displaystyle O(2^{2{\sqrt {\log \log n}}}).})
