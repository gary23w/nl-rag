...
Queues can also be implemented as a purely functional data structure. There are two implementations. The first one only achieves O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) per operation *on average*. That is, the amortized time is O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}), but individual operations can take O ( n ) {\displaystyle O(n)} ({\displaystyle O(n)}) where *n* is the number of elements in the queue. The second implementation is called a **real-time queue** and it allows the queue to be persistent with operations in O(1) worst-case time. It is a more complex implementation and requires lazy lists with memoization.

### Amortized queue

This queue's data is stored in two singly-linked lists named f {\displaystyle f} ({\displaystyle f}) and r {\displaystyle r} ({\displaystyle r}). The list f {\displaystyle f} ({\displaystyle f}) holds the front part of the queue. The list r {\displaystyle r} ({\displaystyle r}) holds the remaining elements (a.k.a., the rear of the queue) *in reverse order*. It is easy to insert into the front of the queue by adding a node at the head of f {\displaystyle f} ({\displaystyle f}). And, if r {\displaystyle r} ({\displaystyle r}) is not empty, it is easy to remove from the end of the queue by removing the node at the head of r {\displaystyle r} ({\displaystyle r}). When r {\displaystyle r} ({\displaystyle r}) is empty, the list f {\displaystyle f} ({\displaystyle f}) is reversed and assigned to r {\displaystyle r} ({\displaystyle r}) and then the head of r {\displaystyle r} ({\displaystyle r}) is removed.

The insert ("enqueue") always takes O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) time. The removal ("dequeue") takes O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) when the list r {\displaystyle r} ({\displaystyle r}) is not empty. When r {\displaystyle r} ({\displaystyle r}) is empty, the reverse takes O ( n ) {\displaystyle O(n)} ({\displaystyle O(n)}) where n {\displaystyle n} ({\displaystyle n}) is the number of elements in f {\displaystyle f} ({\displaystyle f}). But, we can say it is O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) amortized time, because every element in f {\displaystyle f} ({\displaystyle f}) had to be inserted and we can assign a constant cost for each element in the reverse to when it was inserted.

### Real-time queue

The real-time queue achieves O ( 1 ) {\displaystyle O(1)} ({\displaystyle O(1)}) time for all operations, without amortization. This discussion will be technical, so recall that, for l {\displaystyle l} ({\displaystyle l}) a list, | l | {\displaystyle |l|} ({\displaystyle |l|}) denotes its length, that *NIL* represents an empty list and CONS ⁡ ( h , t ) {\displaystyle \operatorname {CONS} (h,t)} ({\displaystyle \operatorname {CONS} (h,t)}) represents the list whose head is *h* and whose tail is *t*.

The data structure used to implement our queues consists of three singly-linked lists ( f , r , s ) {\displaystyle (f,r,s)} ({\displaystyle (f,r,s)}) where *f* is the front of the queue and *r* is the rear of the queue in reverse order. The invariant of the structure is that *s* is the rear of *f* without its | r | {\displaystyle |r|} ({\displaystyle |r|}) first elements, that is | s | = | f | − | r | {\displaystyle |s|=|f|-|r|} ({\displaystyle |s|=|f|-|r|}). The tail of the queue ( CONS ⁡ ( x , f ) , r , s ) {\displaystyle (\operatorname {CONS} (x,f),r,s)} ({\displaystyle (\operatorname {CONS} (x,f),r,s)}) is then almost ( f , r , s ) {\displaystyle (f,r,s)} ({\displaystyle (f,r,s)}) and inserting an element *x* to ( f , r , s ) {\displaystyle (f,r,s)} ({\displaystyle (f,r,s)}) is almost ( f , CONS ⁡ ( x , r ) , s ) {\displaystyle (f,\operatorname {CONS} (x,r),s)} ({\displaystyle (f,\operatorname {CONS} (x,r),s)}). It is said almost, because in both of those results, | s | = | f | − | r | + 1 {\displaystyle |s|=|f|-|r|+1} ({\displaystyle |s|=|f|-|r|+1}). An auxiliary function a u x {\displaystyle aux} ({\displaystyle aux}) must then be called for the invariant to be satisfied. Two cases must be considered, depending on whether s {\displaystyle s} ({\displaystyle s}) is the empty list, in which case | r | = | f | + 1 {\displaystyle |r|=|f|+1} ({\displaystyle |r|=|f|+1}), or not. The formal definition is aux ⁡ ( f , r , Cons ⁡ ( _ , s ) ) = ( f , r , s ) {\displaystyle \operatorname {aux} (f,r,\operatorname {Cons} (\_,s))=(f,r,s)} ({\displaystyle \operatorname {aux} (f,r,\operatorname {Cons} (\_,s))=(f,r,s)}) and aux ⁡ ( f , r , NIL ) = ( f ′ , NIL , f ′ ) {\displaystyle \operatorname {aux} (f,r,{\text{NIL}})=(f',{\text{NIL}},f')} ({\displaystyle \operatorname {aux} (f,r,{\text{NIL}})=(f',{\text{NIL}},f')}) where f ′ {\displaystyle f'} ({\displaystyle f'}) is *f* followed by *r* reversed.

Let us call reverse ⁡ ( f , r ) {\displaystyle \operatorname {reverse} (f,r)} ({\displaystyle \operatorname {reverse} (f,r)}) the function which returns *f* followed by *r* reversed. Let us furthermore assume that | r | = | f | + 1 {\displaystyle |r|=|f|+1} ({\displaystyle |r|=|f|+1}), since it is the case when this function is called. More precisely, we define a lazy function rotate ⁡ ( f , r , a ) {\displaystyle \operatorname {rotate} (f,r,a)} ({\displaystyle \operatorname {rotate} (f,r,a)}) which takes as input three lists such that | r | = | f | + 1 {\displaystyle |r|=|f|+1} ({\displaystyle |r|=|f|+1}), and return the concatenation of *f*, of *r* reversed and of *a*. Then reverse ⁡ ( f , r ) = rotate ⁡ ( f , r , NIL ) {\displaystyle \operatorname {reverse} (f,r)=\operatorname {rotate} (f,r,{\text{NIL}})} ({\displaystyle \operatorname {reverse} (f,r)=\operatorname {rotate} (f,r,{\text{NIL}})}). The inductive definition of rotate is rotate ⁡ ( NIL , Cons ⁡ ( y , NIL ) , a ) = Cons ⁡ ( y , a ) {\displaystyle \operatorname {rotate} ({\text{NIL}},\operatorname {Cons} (y,{\text{NIL}}),a)=\operatorname {Cons} (y,a)} ({\displaystyle \operatorname {rotate} ({\text{NIL}},\operatorname {Cons} (y,{\text{NIL}}),a)=\operatorname {Cons} (y,a)}) and rotate ⁡ ( CONS ⁡ ( x , f ) , CONS ⁡ ( y , r ) , a ) = Cons ⁡ ( x , rotate ⁡ ( f , r , CONS ⁡ ( y , a ) ) ) {\displaystyle \operatorname {rotate} (\operatorname {CONS} (x,f),\operatorname {CONS} (y,r),a)=\operatorname {Cons} (x,\operatorname {rotate} (f,r,\operatorname {CONS} (y,a)))} ({\displaystyle \operatorname {rotate} (\operatorname {CONS} (x,f),\operatorname {CONS} (y,r),a)=\operatorname {Cons} (x,\operatorname {rotate} (f,r,\operatorname {CONS} (y,a)))}). Its running time is O ( r ) {\displaystyle O(r)} ({\displaystyle O(r)}), but, since lazy evaluation is used, the computation is delayed until the results are forced by the computation.
