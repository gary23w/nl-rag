...
needs (10×30×5) + (10×5×60) = 1500 + 3000 = 4500 operations, while
...
One simple solution is called memoization: each time we compute the minimum cost needed to multiply out a specific subsequence, we save it. If we are ever asked to compute it again, we simply give the saved answer, and do not recompute it. Since there are about *n*2/2 different subsequences, where *n* is the number of matrices, the space required to do this is reasonable. It can be shown that this simple trick brings the runtime down to O(*n*3) from exponential, which is more than efficient enough for real applications. This is top-down dynamic programming.
...
- ((AB)C) Polygon representation of (AB)C
- (A(BC)) Polygon representation of A(BC)
...
: (10×30×5) + (10×5×60) = 1500 + 3000 = 4500 multiplications
...
: (30×5×60) + (10×30×60) = 9000 + 18000 = 27000 multiplications
...
The algorithm starts by translating the problem to the polygon partitioning problem. To each vertex *V* of the polygon is associated a weight *w*. Suppose we have three consecutive vertices $V_{i-1},V_{i},V_{i+1}$ , and that $V_{\min }$ is the vertex with minimum weight $w_{\min }$ . We look at the quadrilateral with vertices $V_{\min },V_{i-1},V_{i},V_{i+1}$ (in clockwise order). We can triangulate it in two ways:

- $(V_{\min },V_{i-1},V_{i})$ and $(V_{\min },V_{i+1},V_{i})$ , with cost $w_{\min }w_{i-1}w_{i}+w_{\min }w_{i+1}w_{i}$
- $(V_{\min },V_{i-1},V_{i+1})$ and $(V_{i-1},V_{i},V_{i+1})$ with cost $w_{\min }w_{i-1}w_{i+1}+w_{i-1}w_{i}w_{i+1}$ .

Therefore, if

$w_{\min }w_{i-1}w_{i+1}+w_{i-1}w_{i}w_{i+1}<w_{\min }w_{i-1}w_{i}+w_{\min }w_{i+1}w_{i}$

or equivalently

${\frac {1}{w_{i}}}+{\frac {1}{w_{\min }}}<{\frac {1}{w_{i+1}}}+{\frac {1}{w_{i-1}}}$

we remove the vertex $V_{i}$ from the polygon and add the side $(V_{i-1},V_{i+1})$ to the triangulation. We repeat this process until no $V_{i}$ satisfies the condition above. For all the remaining vertices $V_{n}$ , we add the side $(V_{\min },V_{n})$ to the triangulation. This gives us a nearly optimal triangulation.
