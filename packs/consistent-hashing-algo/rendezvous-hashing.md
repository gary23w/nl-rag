...
domain: consistent-hashing-algo
...
The standard version of Rendezvous Hashing described above works quite well for moderate *n,* but when n is extremely large, the hierarchical use of Rendezvous Hashing achieves $O(\log n)$ running time. This approach creates a virtual hierarchical structure (called a "skeleton"), and achieves $O(\log n)$ running time by applying HRW at each level while descending the hierarchy. The idea is to first choose some constant m and organize the n sites into $c=\lceil n/m\rceil$ clusters $C_{1}=\left\{S_{1},S_{2}\dots S_{m}\right\},C_{2}=\left\{S_{m+1},S_{m+2}\dots S_{2m}\right\}\dots$ Next, build a virtual hierarchy by choosing a constant f and imagining these c clusters placed at the leaves of a tree T of virtual nodes, each with fanout f .
