...
A character is the smallest unit of text that has semantic value. In linguistics, this is called a grapheme and each of the various ways it may be written are called glyphs. (For example, the serif form g and the sans-serif form g are each a glyph of the grapheme ⟨g⟩, U+0067 g LATIN SMALL LETTER G.)
...
A character set is a collection of characters used to represent text. For example, the Latin alphabet and Greek alphabet are character sets.

### Coded character set

A coded character set is a character set with each item uniquely mapped to a numberic value.
...
A character repertoire is a set of characters that can be represented by a particular coded character set. The repertoire may be closed, meaning that no additions are allowed without creating a new standard (as is the case with ASCII and most of the ISO-8859 series); or it may be open, allowing additions (as is the case with Unicode and to a limited extent Windows code pages).

### Code point

A code point is the value or position of a character in a coded character set. A code point is represented by a sequence of code units. The mapping is defined by the encoding. Thus, the number of code units required to represent a code point depends on the encoding:
...
Code space is the range of numerical values spanned by a coded character set.

### Code unit

A code unit is the minimum bit combination that can represent a character in a character encoding (in computer science terms, it is the word size of the character encoding). Common code units include 7-bit, 8-bit, 16-bit, and 32-bit. In some encodings, some characters are encoded as multiple code units.
...
Hardware and software systems typically have a maximum "native" word length, such as 16 or 32 bits. A character encoding may define code points whose length exceeds a system's native word length. *A character encoding form* (CEF) is the mapping of code points to *code units* that break up a code point in a standardized way so that each code unit fits within the system's word length. For example, a computer that represents numbers in 16-bit units can only represent code points up to 65,535 per unit, but larger code points can be represented using multiple 16-bit units as defined by the CEF.
...
- Four composed characters: `a`, `b̲`, `c`, `𐐀`
- Five graphemes: `a`, `b`, `_`, `c`, `𐐀`
- Five Unicode code points: `U+0061`, `U+0062`, `U+0332`, `U+0063`, `U+10400`
...
- Nine UTF-8 code units (8-bit values, or bytes) `0x61`, `0x62`, `0xCC`, `0xB2`, `0x63`, `0xF0`, `0x90`, `0x90`, `0x80`
...
- MultiByteToWideChar/WideCharToMultiByte – Windows API functions for converting between ANSI and Unicode

## Common character encodings

The most used character encoding on the web is UTF-8, used in 98.9% of surveyed web sites, as of January 2026. In application programs and operating system tasks, both UTF-8 and UTF-16 are popular options.
