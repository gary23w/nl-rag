...
For example, setting *k* = *2* results in 2-fold cross-validation. In 2-fold cross-validation, the dataset is randomly shuffled into two sets *d*0 and *d*1, so that both sets are equal size (this is usually implemented by shuffling the data array and then splitting it in two). We then train on *d*0 and validate on *d*1, followed by training on *d*1 and validating on *d*0.
