...
Other important content ranking systems from this era include the HITS algorithm invented by Jon Kleinberg (used by Teoma and now Ask.com), the IBM CLEVER project, the TrustRank algorithm, "dwell time" systems invented by Karl T. Muth, the Hummingbird algorithm, and the SALSA algorithm.
...
The damping factor is subtracted from 1 (and in some variations of the algorithm, the result is divided by the number of documents (*N*) in the collection; in the technical literature this is sometimes referred to as "library size") and this term is then added to the product of the damping factor and the sum of the incoming PageRank scores. That is,
...
where $p_{1},p_{2},...,p_{N}$ are the pages under consideration, $M(p_{i})$ is the set of pages that link to $p_{i}$ , $L(p_{j})$ is the number of outbound links on page $p_{j}$ , and N is the total number of pages.
...
where the adjacency function $\ell (p_{i},p_{j})$ is the ratio between number of links outbound from page j to page i to the total number of outbound links of page j. The adjacency function is 0 if page $p_{j}$ does not link to $p_{i}$ , and normalized such that, for each *j*
...
Google's founders, in their original paper, reported that the PageRank algorithm for a network consisting of 322 million links (in-edges and out-edges) converges to within a tolerable limit in 52 iterations. The convergence in a network of half the above size took approximately 45 iterations. Through this data, they concluded the algorithm can be scaled very well and that the scaling factor for extremely large networks would be roughly linear in $\log n$ , where n is the size of the network.
...
where N is the total number of pages, and $p_{i};0$ is page i at time 0.
...
| $\mathbf {R} (t+1)=d{\mathcal {M}}\mathbf {R} (t)+{\frac {1-d}{N}}\mathbf {1}$ , |   | 1 |
|---|---|---|

where $\mathbf {R} _{i}(t)=PR(p_{i};t)$ and $\mathbf {1}$ is the column vector of length N containing only ones.
...
If the matrix ${\mathcal {M}}$ is a transition probability, i.e., column-stochastic and $\mathbf {R}$ is a probability distribution (i.e., $|\mathbf {R} |=1$ , $\mathbf {E} \mathbf {R} =\mathbf {1}$ where $\mathbf {E}$ is matrix of all ones), then equation (**2**) is equivalent to
...
where $\deg(p_{i})$ denotes the degree of vertex $p_{i}$ , and E is the edge-set of the graph, then, with $Y={1 \over N}\mathbf {1}$ , shows that:
