...
Flip-flops and latches can be divided into common types: SR (set-reset), D (data), T (toggle), and JK (see History section above). The behavior of a particular type can be described by the characteristic equation that derives the *next* output (*Q*next) in terms of the input signal(s) and/or the current output, Q .
...
| SR latch operation SRAction 00Q = 1, Q = 1; not allowed 01Q = 1 10Q = 0 11No change; random initial |   |
|---|---|
...
Latches are designed to be *transparent.* That is, input signal changes cause immediate changes in output. Additional logic can be added to a transparent latch to make it *non-transparent* or *opaque* when another input (an *enable* input) is not asserted.
...
| Gated SR latch operation EAction 0No action (hold state) 1The same as non-gated SR latch |   |
|---|---|
...
The truth table below shows that when the *e*nable input is 0, the D input has no effect on the output. When E is high, the output equals D.

| Gated D latch truth table E D Q Q Comment 0XQprevQprevNo change 1001Reset 1110Set |   |
|---|---|

- (Schematic diagram)A gated D latch based on an SR NAND latch
- (A gated D latch based on an SR NOR latch)A gated D latch based on an SR NOR latch
- (An animated gated D latch. Black and white mean logical '1' and '0', respectively.D = 1, E = 1: setD = 1, E = 0: holdD = 0, E = 0: holdD = 0, E = 1: reset)An animated gated D latch. Black and white mean logical '1' and '0', respectively.D = 1, E = 1: setD = 1, E = 0: holdD = 0, E = 0: holdD = 0, E = 1: reset
- (A gated D latch in pass transistor logic that is write-enabled when CLK=1, similar to the ones in the CD4042 or the CD74HC75 integrated circuits)A gated D latch in pass transistor logic that is write-enabled when CLK=1, similar to the ones in the CD4042 or the CD74HC75 integrated circuits
...
- (Earle latch uses complementary enable inputs: enable active low (E_L) and enable active high (E_H))Earle latch uses complementary enable inputs: enable active low (E_L) and enable active high (E_H)
- (An animated Earle latch. Black and white mean logical '1' and '0', respectively.D = 1, E_H = 1: setD = 0, E_H = 1: resetD = 1, E_H = 0: hold)An animated Earle latch. Black and white mean logical '1' and '0', respectively.D = 1, E_H = 1: setD = 0, E_H = 1: resetD = 1, E_H = 0: hold
...
Most D-type flip-flops in ICs have the capability to be forced to the set or reset state (which ignores the D and clock inputs). Usually, the S = R = 1 condition (illegal in SR latches) is resolved in D-type flip-flops. Setting S = R = 0 makes the flip-flop behave as described above. Here is the truth table for the other possible S and R configurations:
...
Removing the leftmost inverter in the circuit creates a D-type flip-flop that captures the D input on the *falling edge* of the clock signal. This has a truth table like this:
...
If the T input is high, the T flip-flop changes state ("toggles") on the triggering clock edge. If the T input is low, the flip-flop holds the previous value. This behavior is described by the characteristic equation:
...
The JK flip-flop behaves like an edge-triggered SR flip-flop (J: Set, K: Reset) that replaces the illegal S = R = 1 condition with a *flip* or toggle command. Specifically, the combination J = 1, K = 0 is a command to set the flip-flop on the triggering clock edge; the combination J = 0, K = 1 is a command to reset the flip-flop on the triggering clock edge; and the combination J = K = 1 is a command to toggle the flip-flop (i.e., change its output to the logical complement of its current value) on the triggering clock edge. Setting J = K = 0 maintains the current state.
...
The JK flip-flop is considered a universal flip-flop, because it can be configured to work as an SR flip-flop, a D flip-flop, or a T flip-flop. For instance, to synthesize a D flip-flop, simply set K equal to the complement of J (input J will act as input D). Similarly, to synthesize a T flip-flop, set K equal to J.
