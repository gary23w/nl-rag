...
source: https://www.erlang.org/doc/system/seq_prog.html
domain: erlang
...
Most operating systems have a command interpreter or shell. UNIX and Linux have many. Windows has the command prompt, PowerShell, and more. Erlang has its own shell where you can write and evaluate bits of Erlang code directly (see the `shell` manual page in STDLIB).
...
Erlang has many standard modules to help you do things. For example, the module `io` contains many functions that help in doing formatted input/output. To look up information about standard modules, the command `h(..)` can be used at the Erlang shell. Try the Erlang shell command:
...
To change the above program to one that works out the minimum value of the elements in a list, you only need to write `<` instead of `>`. (But it would be wise to change the name of the function to `list_min`.)
