...
- Video acceleration is provided for decoding and encoding, such as H.264 and H.265.

### OpenGL versus Vulkan

| OpenGL | Vulkan |
|---|---|
| There is one single global state machine. | Code is object-based with no global state. |
...
| GPU memory and synchronization are usually hidden. | Explicit control is available for memory management and synchronization. |
| There is extensive error checking. | Vulkan drivers do no error checking at runtime. There is a validation layer for developers. |
...
In 2016, AMD's GPU manufacturing competitor NVIDIA stated that "OpenGL is still a great option for a lot of use cases, as it comes at a much lower complexity and maintenance burden than Vulkan, while in many cases still providing great overall performance."
...
There is also the recommended driver called RADV in Mesa developed by Valve, Red Hat, Google and others. This driver as of March 2023 supports all GCN and RDNA cards. This RADV driver's support for GCN 1.0 through GCN 2.0 required its experimental support in the amdgpu kernel module to be enabled for kernels before 6.19 but it is now the default enabling Vulkan 1.3 on GCN 1 through 2 and Vulkan 1.4 on more recent

### NVIDIA

On Windows and Linux there is the NVIDIA developed Vulkan driver which supports Vulkan 1.2 on Kepler cards with no further updates planned after September 2021. Maxwell and newer cards support Vulkan 1.4.

NVK, an experimental, open source Vulkan driver for Linux based on nouveau, was announced in October 2022. It was merged into mainline Mesa in August 2023. The driver is currently Vulkan 1.4 conformant on Turing and newer cards, Vulkan 1.3 conformant on Maxwell, Pascal and Volta cards and Vulkan 1.2 conformant on Kepler cards.
...
Nor is Vulkan compatible with other graphics APIs such as Direct3D, Metal, and Mantle. However, implementations of those APIs exist atop of Vulkan:
