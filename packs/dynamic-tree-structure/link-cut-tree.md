---
title: "Link/cut tree"
source: https://en.wikipedia.org/wiki/Link/cut_tree
...
Let s(v) be the number of nodes under *v* in the tree of auxiliary trees. Then the potential function $\Phi =\sum _{v}\log {s(v)}$ . We know that the amortized cost of splaying is bounded by:
...
$3\left(\log {s(R)}-\log {s(v)}\right)+O({\text{number of preferred child changes}})$
...
In solving the maximum flow problem, link/cut trees can be used to improve the running time of Dinic's algorithm from $O(V^{2}E)$ to $O(VE\log V)$ .
