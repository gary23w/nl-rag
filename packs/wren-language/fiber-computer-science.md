...
source: https://en.wikipedia.org/wiki/Fiber_(computer_science)
domain: wren-language
license: CC-BY-SA-4.0
tags: wren language, class based programming, fiber concurrency, embeddable language, scripting language
fetched: 2026-07-02
---
...
Less support from the operating system is needed for fibers than for threads. They can be implemented in modern Unix systems using the library functions getcontext, setcontext and swapcontext in `<ucontext.h>`, as in GNU Portable Threads, or in assembler as `boost::fibers`.
...
- Boost (C++) has a `boost::fibers::fiber` class since Boost version 1.62
- D has a class `core.thread.fiber.Fiber` in its standard library.
