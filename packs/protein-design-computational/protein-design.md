...
source: https://en.wikipedia.org/wiki/Protein_design
...
$h=\sum _{j=d+1}^{n}[\min _{r_{j}}(E(r_{j})+\sum _{i=1}^{d}E(r_{i},r_{j})+\sum _{k=j+1}^{n}\min _{r_{k}}E(r_{j},r_{k}))]$
...
Monte Carlo is one of the most widely used algorithms for protein design. In its simplest form, a Monte Carlo algorithm selects a residue at random, and in that residue a randomly chosen rotamer (of any amino acid) is evaluated. The new energy of the protein, *E*new is compared against the old energy *E*old and the new rotamer is *accepted* with a probability of:
