...
| Inherited | no |
| Computed value | as each of the properties of the shorthand: `flex-grow`: as specified`flex-shrink`: as specified`flex-basis`: as specified, but with relative lengths converted into absolute lengths |
| Animation type | as each of the properties of the shorthand: `flex-grow`: a number`flex-shrink`: a number`flex-basis`: a length, percentage or calc(); |
