...
Authors:Andreas Rumpf, Zahary Karadjov Version:2.2.10 "Complexity" seems to be a lot like "energy": you can transfer it from the end-user to one/some of the other players, but the total amount seems to remain pretty much constant for a given task. -- Ran
...
- two immediate following underscores __ are not allowed: letter ::= 'A'..'Z' | 'a'..'z' | '\x80'..'\xff' digit ::= '0'..'9' IDENTIFIER ::= letter ( ['_'] (letter | digit) )*
...
import std/strutils type u4 = distinct uint8 proc `'u4`(n: string): u4 = result = (parseInt(n) and 0x0F).u4 var x = 5'u4
...
import std/strutils type u4 = distinct uint8 proc `'u4`(n: string; moreData: int): u4 = result = (parseInt(n) and 0x0F).u4 var x = 5'u4(123)
...
proc `^/`(x, y: float): float = result = x / y echo 12 ^/ 4 ^/ 8 echo 12 / 4 / 8
