...
An electric current flowing through a conductor generates a magnetic field surrounding it. The magnetic flux linkage Φ B {\displaystyle \Phi _{\mathbf {B} }} ({\displaystyle \Phi _{\mathbf {B} }}) generated by a given current I {\displaystyle I} ({\displaystyle I}) depends on the geometric shape of the circuit. Their ratio defines the inductance L {\displaystyle L} ({\displaystyle L}). Thus
...
Any change in the current through an inductor creates a changing flux, inducing a voltage across the inductor. By Faraday's law of induction, the voltage E {\displaystyle {\mathcal {E}}} ({\displaystyle {\mathcal {E}}}) induced by any change in magnetic flux through the circuit is given by
...
E = − L d I d t {\displaystyle \;{\mathcal {E}}=-L{\frac {dI}{dt}}\;} ({\displaystyle \;{\mathcal {E}}=-L{\frac {dI}{dt}}\;})
...
Because the induced voltage is positive at the current's entrance terminal, the inductor's current–voltage relationship is often expressed without a negative sign by using the current's exit terminal as the reference point for the voltage V ( t ) {\displaystyle V(t)} ({\displaystyle V(t)}) at the current's entrance terminal (as labeled in the schematic). The current–voltage relationship is then:

Current–voltage relationship (derivative form)

V ( t ) = L d I ( t ) d t {\displaystyle \;V(t)=L{\frac {\mathrm {d} I(t)}{\mathrm {d} t}}\;} ({\displaystyle \;V(t)=L{\frac {\mathrm {d} I(t)}{\mathrm {d} t}}\;})

which can be rewritten as:

Current–voltage relationship (integral form)

I ( t ) = I ( t 0 ) + 1 L ∫ t 0 t V ( τ ) d τ {\displaystyle \;I(t)=I(t_{0})+{\frac {1}{L}}\int _{t_{0}}^{t}V(\tau )\,\mathrm {d} \tau \;} ({\displaystyle \;I(t)=I(t_{0})+{\frac {1}{L}}\int _{t_{0}}^{t}V(\tau )\,\mathrm {d} \tau \;})
...
The work done per unit charge on the charges passing through the inductor is − E {\displaystyle -{\mathcal {E}}} ({\displaystyle -{\mathcal {E}}}). The negative sign indicates that the work is done *against* the emf, and is not done *by* the emf. The current I {\displaystyle I} ({\displaystyle I}) is the charge per unit time passing through the inductor. Therefore, the rate of work W {\displaystyle W} ({\displaystyle W}) done by the charges against the emf, that is the rate of change of energy of the current, is given by
...
From the constitutive equation for the inductor, − E = L d I d t {\displaystyle -{\mathcal {E}}=L{\frac {dI}{dt}}} ({\displaystyle -{\mathcal {E}}=L{\frac {dI}{dt}}}) so
...
In a ferromagnetic core inductor, when the magnetic field approaches the level at which the core saturates, the inductance will begin to change, it will be a function of the current L ( I ) {\displaystyle L(I)} ({\displaystyle L(I)}). Neglecting losses, the energy W {\displaystyle W} ({\displaystyle W}) stored by an inductor with a current I 0 {\displaystyle I_{0}} ({\displaystyle I_{0}}) passing through it is equal to the amount of work required to establish the current through the inductor:
...
{\displaystyle W=\int _{0}^{I_{0}}L_{d}(I)\,I\,dI\,,}

where L d ( I ) {\displaystyle L_{d}(I)} ({\displaystyle L_{d}(I)}) is the so-called "differential inductance": L d = d Φ B d I . {\displaystyle L_{d}={\frac {d\Phi _{\mathbf {B} }}{dI}}\,.} ({\displaystyle L_{d}={\frac {d\Phi _{\mathbf {B} }}{dI}}\,.})
...
W = 1 2 L I 0 2 {\displaystyle \;W={\frac {1}{2}}L{I_{0}}^{2}\;} ({\displaystyle \;W={\frac {1}{2}}L{I_{0}}^{2}\;})

For inductors with magnetic cores, the above equation is only valid for linear regions of the magnetic flux, at currents below the saturation level of the inductor, where the inductance is approximately constant. Where this is not the case, the integral form must be used with L d {\displaystyle L_{d}} ({\displaystyle L_{d}}) variable.
...
where L {\displaystyle L} ({\displaystyle L}) is the inductance, R {\displaystyle R} ({\displaystyle R}) is the DC resistance, and the product ω L {\displaystyle \omega L} ({\displaystyle \omega L}) is the inductive reactance
...
When there is a sinusoidal alternating current (AC) through an inductor, a sinusoidal voltage is induced. The amplitude of the voltage is proportional to the product of the amplitude ( I P {\displaystyle I_{P}} ({\displaystyle I_{P}})) of the current and the angular frequency ( ω {\displaystyle \omega } ({\displaystyle \omega })) of the current.
...
{\displaystyle {\begin{aligned}i(t)~&=~I_{\mathsf {P}}\ \sin(\omega \ t)\\{\frac {~\mathrm {d} \;\!i(t)~}{\mathrm {d} \;\!t}}~&=~I_{\mathsf {P}}\ \omega \ \cos(\omega \ t)\\v(t)~&=~L\ I_{\mathsf {P}}\ \omega \ \cos(\omega \ t)~.\end{aligned}}}
...
{\displaystyle i(t)=I\ e^{-{\tfrac {R}{L}}\ t}}
...
{\displaystyle X_{\mathsf {L}}={\frac {\ V_{\mathsf {P}}}{\ I_{\mathsf {P}}}}={\frac {\ \omega \ L\ I_{\mathsf {P}}}{\ I_{\mathsf {P}}}}}
...
{\displaystyle f_{\mathsf {3\ dB}}={\frac {R}{2\pi \ L}}}
...
- adding a voltage source in series with the inductor, having the value: L   I 0 {\displaystyle L\ I_{0}} ({\displaystyle L\ I_{0}}) where L {\displaystyle L} ({\displaystyle L}) is the inductance, and I 0 {\displaystyle I_{0}} ({\displaystyle I_{0}}) is the initial current in the inductor. (The source should have a polarity that is aligned with the initial current.)
- or by adding a current source in parallel with the inductor, having the value:   I 0 s {\displaystyle {\frac {\ I_{0}}{s}}} ({\displaystyle {\frac {\ I_{0}}{s}}}) where I 0 {\displaystyle I_{0}} ({\displaystyle I_{0}}) is the initial current in the inductor. s {\displaystyle s} ({\displaystyle s}) is the complex frequency.
...
{\displaystyle L_{\mathsf {eqv}}~=~\left(\sum _{i=1}^{n}{\frac {1}{\ L_{i}}}\right)^{-1}~=~\left({\frac {1}{\ L_{1}}}+{\frac {1}{\ L_{2}}}+\dots +{\frac {1}{\ L_{n}}}\right)^{-1}~.}
...
{\displaystyle L_{\mathsf {eqv}}~=~\sum _{i=1}^{n}L_{i}~\equiv ~L_{1}+L_{2}+\cdots +L_{n}~.}
...
as only a fraction of self flux is linked with the other. This fraction is called "coefficient of flux linkage" (   K   {\displaystyle \ K\ } ({\displaystyle \ K\ })) or "coupling coefficient" (with   0 ≤ K ≤ 1   {\displaystyle \ 0\leq K\leq 1\ } ({\displaystyle \ 0\leq K\leq 1\ })).
