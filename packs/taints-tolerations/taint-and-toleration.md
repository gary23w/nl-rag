...
No new Pods will be scheduled on the tainted node unless they have a matching toleration. Pods currently running on the node are
...
is a "preference" or "soft" version of
...
to avoid placing a Pod that does not tolerate the taint on the node, but it is not guaranteed.
...
When you create a Pod that uses
...
tolerations operators, the API server validates that the toleration values are valid integers. Taint values on nodes are not validated at node registration time. If a node has a non-numeric taint value (for example,
...
Last modified February 09, 2026 at 10:30 PM PST:

fix incorrect definition of Gt & Lt operators in taints % tolerations (1a61fe63a4)
