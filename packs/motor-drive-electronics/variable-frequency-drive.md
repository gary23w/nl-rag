...
Pulse-width modulation (PWM) variable-frequency drive projects started in the 1960s at Strömberg in Finland. Martti Harmoinen is regarded as the inventor of this technology. Strömberg managed to sell the idea of PWM drive to Helsinki Metro in 1973 and in 1982 the first PWM drive SAMI10 were operational.
...
| Machines | Induction^^^ Cage rotor CSI or VSI (six-step or PWM), cycloconverter, matrix WRIM Electro-mechanical Doubly fed WRIM Slip energy recovery (Kramer/Scherbius) Synchronous^^^ WFSM CSI (LCI), cycloconverter, VSI PM Axial or disk Radial Interior Surface Trapezoidal BLDM, Sinusoidal PMSM VSI SyRM VSI VRM^^^ Switched reluctance motor, Stepper motor VSI |
|---|---|
|   |   |

Table 2: Drives by detailed
...
|---|---|
|   |   |
| Indirect AC–AC | LV IGBT^ 2-level VSI PWM Great majority of LV drive makes 3-level VSI PWM Primarily Yaskawa (SCR/CSI or diode/VSI)+IGBT Six-step or PAM For example Baker-Hughes MV VSI GCT^ 3‑level NPC inverter Primarily ABB, GE IGBT^ 2‑level inverter Primarily GE-Converteam 3‑level NPC inverter Primarily Eaton, GE-Toshiba, Siemens Multilevel CHB inverter Primarily GE, Siemens (Robicon), Toshiba Flying‑capacitor inverter drive Primarily GE-Converteam NPC/H‑bridge inverter drive Primarily Toshiba CSI Primarily ABB, GE‑Converteam, Siemens SCR^^ LCI Primarily Allen‑Bradley SGCT^^ with AFE CSR PWM / CSI PWM inverter with CME SCR + SGCT 18-pulse CSI PWM drive GTO^^ Capacitor assisted CSI PWM drive (legacy) |
|   |   |
| Direct AC–AC | SCR Cycloconverter Primarily ABB, Siemens, GE Converteam IGBT Matrix Primarily Yaskawa |
|   |   |

Table 3: Topology diagrams

- (Simplified 2-Level Inverter Topology) Simplified 2-Level Inverter Topology
- (Simplified Neutral Point Clamped 3-Level Inverter Topology) Simplified Neutral Point Clamped 3-Level Inverter Topology
- (Simplified Cascaded H-bridge Inverter Topology) Simplified Cascaded H-bridge Inverter Topology
- (Simplified Flying Capacitor Inverter 4-Level Topology) Simplified Flying Capacitor Inverter 4-Level Topology
- (Simplified Neutral Point Clamped H-bridge Inverter Topology) Simplified Neutral Point Clamped H-bridge Inverter Topology
