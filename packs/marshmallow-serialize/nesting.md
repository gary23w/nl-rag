---
title: "Nesting schemas"
source: https://marshmallow.readthedocs.io/en/stable/nesting.html
...
Schemas can be nested to represent relationships between objects (e.g. foreign key relationships). For example, a `Blog` may have an author represented by a `User`. And a `User` may have many friends, each of which is a `User`.
