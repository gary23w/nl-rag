...
CLONE_PIDFD (since Linux 5.2)
       ...
       If CLONE_PIDFD is specified together with CLONE_THREAD, the
       obtained PID file descriptor refers to a specific thread,
       as opposed to a thread-group leader if CLONE_THREAD is not
       specified.  This feature is available since Linux 6.9.
...
E2BIG (clone3() only)
       The kernel does not support some functionality requested in
       this clone3() call: The size argument is larger than what
       the kernel supports, and there are non-zero values in the
       struct after the kernel-supported size.
