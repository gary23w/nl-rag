...
source: https://en.wikipedia.org/wiki/50_State_quarters
...
(state tree) sprigs, state outline
...
, nine stars (representing New Hampshire as the 9th state to ratify the U.S. Constitution)
...
, 11 stars (representing New York as the 11th state to ratify the U.S. Constitution), state outline with line tracing
...
(state bird); trumpet with musical notes, outline of
...
, state outline, 19 stars (representing Indiana as the 19th state to join the Union)
...
skyline; state outline; 21 stars, 11 on left edge and 10 on right
...
Schoolhouse, teacher and students planting a tree; based on the
...
skull in the center with mountains and the Missouri River in the background.
...
, state outline with star indicating location of state capital
...
(state wildflower) in background
