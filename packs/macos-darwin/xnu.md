---
title: "XNU - Wikipedia"
source: https://en.wikipedia.org/wiki/XNU
domain: macos-darwin
...
As of June 2026, XNU runs on ARM64 and x86-64 processors. PowerPC support was removed as of the Mac OS X Snow Leopard kernel. Support for IA-32 was removed as of the Mac OS X Lion kernel; support for 32-bit ARM was removed as of iOS 11.
...
XNU doesn't use Mach as a microkernel, rather including other subsystems (such as the BSD one) as a part of the kernel, instead of as a userspace daemons. Mach in XNU is used to manage virtual memory, facilitate Inter-process communication (IPC), and do Input/output (I/O) operations with the console.
...
IOKit is the device driver framework, written in a subset of C++ based on Embedded C++ that lacks lacks features such as exceptions, multiple inheritance, and templates. Using its object-oriented design, features common to any class of driver are provided within the framework, helping device drivers be written in less time and code. IOKit is multi-threaded, symmetric multiprocessing (SMP)-safe, and allows for hot-pluggable devices and automatic, dynamic device configuration.
