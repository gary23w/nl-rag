---
title: "Forecasting"
source: https://en.wikipedia.org/wiki/Forecasting
...
where $y_{1},...,y_{T}$ is the past data.
...
${\hat {y}}_{T+h|T}=y_{T}+{\frac {h}{T-1}}\sum _{t=2}^{T}(y_{t}-y_{t-1})=y_{T}+h\left({\frac {y_{T}-y_{1}}{T-1}}\right).$
...
The seasonal naïve method accounts for seasonality by setting each prediction to be equal to the last observed value of the same season. For example, the prediction value for all subsequent months of April will be equal to the previous value observed for April. The forecast for time $T+h$ is

${\hat {y}}_{T+h|T}=y_{T+h-m(k+1)}$

where m is the seasonal period (for example, for monthly data with yearly seasonality, $m=12$ ) and k is the integer part of $(h-1)/m$ .
...
${\begin{aligned}f_{n}(x_{t})={\dfrac {1}{(1+x_{t}^{n})}}\,,\qquad n\in {\mathbb {N} },\;x\in {\mathbb {R} }.\end{aligned}}$

The short term behaviour $x_{t}$ and the is the medium-long-term trend $y_{t}$ are

${\begin{aligned}{\left\{{\begin{array}{ll}x_{t+1}=\alpha f_{n}(x_{t})+\gamma \,y_{t}+\delta \\y_{t+1}=\beta \,y_{t}-\mu \,x_{t}+\eta \end{array}}\right.}\end{aligned}}$

where $\alpha ,\gamma ,\beta ,\mu ,\eta$ are some parameters.
...
Mean absolute error (MAE) or mean absolute deviation (MAD): $\ MAE=MAD={\frac {\sum _{t=1}^{N}|E_{t}|}{N}}$

Mean squared error (MSE) or mean squared prediction error (MSPE): $\ MSE=MSPE={\frac {\sum _{t=1}^{N}{E_{t}^{2}}}{N}}$

Root mean squared error (RMSE): $\ RMSE={\sqrt {\frac {\sum _{t=1}^{N}{E_{t}^{2}}}{N}}}$

Average of Errors (E): $\ {\bar {E}}={\frac {\sum _{t=1}^{N}{E_{t}}}{N}}$
...
Mean absolute percentage error (MAPE): $\ MAPE=100*{\frac {\sum _{t=1}^{N}|{\frac {E_{t}}{Y_{t}}}|}{N}}$

Mean absolute percentage deviation (MAPD): $\ MAPD={\frac {\sum _{t=1}^{N}|E_{t}|}{\sum _{t=1}^{N}|Y_{t}|}}$

### Scaled errors

Hyndman and Koehler (2006) proposed using scaled errors as an alternative to percentage errors.

Mean absolute scaled error (MASE): $MASE={\frac {\sum _{t=1}^{N}|{\frac {E_{t}}{{\frac {1}{N-m}}\sum _{t=m+1}^{N}|Y_{t}-Y_{t-m}|}}|}{N}}$
...
Some authors distinguish **forecastability** from **forecast accuracy**. In this usage, forecastability refers to the inherent predictability of a time series or process, while forecast accuracy refers to the performance of a particular forecasting method applied to it. Forecastability has also been discussed as a structural property of time series related to regularity and predictability.
