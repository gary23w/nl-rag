...
| `default_language_version` | (optional: default `{}`) a mapping from language to the default `language_version` that should be used for that language. This will only override individual hooks that do not set `language_version`. For example to use `python3.7` for `language: python` hooks: default_language_version: python: python3.7 |
...
Hook repositories must contain a dotnet CLI tool which can be `pack`ed and `install`ed as per this example. The `entry` should match an executable created by building the repository. Additional dependencies are not currently supported.
...
- `PRE_COMMIT_REMOTE_URL`: the url of the remote that is being pushed to (for example `[email protected]:pre-commit/pre-commit`)
