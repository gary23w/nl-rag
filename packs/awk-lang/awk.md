...
, and these can be displayed separately:
...
Unlike Python, all augmented assignments in `awk` such as `+=` `-=` `*=` `/=` `^=` `%=` are expressions. This allows for effective unlimited-length chaining. One of its biggest benefit is demonstrated by the algorithm for calculating Fibonacci numbers having complexity of `O(n)` because of the reason that it keeps adding back on itself: `a += b += a += b += a += b += ...` For the first 77 or so Fibonacci numbers, this approach is highly effective, since it bypasses having to examine bits, swap variables, or incur recursion overhead.
