...
. (To go to a help link, move the cursor to it and hit

CTRL-]

, or ctrl-click with mouse. Try it here:
...
Using Visual mode (selecting text)
...
Vim regexp patterns and search commands

key-mapping

Key mapping (shortcuts), abbreviations
...
Commands for using windows and buffers

tabpage

Commands for using tabpages
...
Nvim API via RPC, Lua and Vimscript
...
Spawn and control multiple processes
...
Vietnamese language support and editing

### Interop

provider

Builtin remote plugin hosts
...
Internal architecture, modules, data structures
...
Design guidelines (colorschemes etc.)
...
or a double mouse click, you can use CTRL-T,
...
Note that tags are within | characters, but when highlighting is enabled these characters are hidden (for aesthetics).

You can use

CTRL-]

on any word (even if it is not within "|") and Nvim will try to find help for it. Especially for options in single quotes, e.g.
