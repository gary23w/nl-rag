...
### Differences from bzip

Seward made the first public release of bzip, version 0.15, in July 1996. Due to software patent issues, bzip2 version 0.1 was released in August 1997, with Huffman coding replacing the arithmetic coding used by bzip. The format in which bzip2 is created was not compatible with bzip and any efforts to make both the programs compatibility were not met because of the patent issues. bzip is no longer available and is replaced by bzip2.

The compressor's stability and popularity grew over the next several years, and Seward released version 1.0 in late 2000. Following a nine-year hiatus of updates for the project since 2010, on 4 June 2019 Federico Mena accepted maintainership of the bzip2 project. Bzip2 version 1.0.8 was released 13 July 2019. Since June 2021, the maintainer is Micah Snyder.

### Version History

**Bzip2** file format version history:

- 0.9.0 - First public release.
- 0.9.5 - Added fallback sorting algorithm to give reasonable behavior for very repetitive inputs. --repetitive-best and --repetitive-fast no longer used.
- 1.0 - Large file support, decompression robustness, fix race condition, avoid library namespace pollution among other minor fixes.
- 1.0.8 - Currently working version which accepts many selectors, file handling over 4 GB and bzdiff, bzgrep cleaned up to avoid them using bash extensions.
...
A compressed block in bzip2 can be decompressed without having to process earlier blocks. This means that bzip2 files can be decompressed in parallel, making it a good format for use in big data applications with cluster computing frameworks like Hadoop and Apache Spark.

## Efficiency

bzip2 compresses most files more effectively than the older LZW (.Z) and Deflate (.zip and .gz) compression algorithms, but is considerably slower. LZMA is generally more space-efficient than bzip2 at the expense of even slower compression speed, while having faster decompression.
...
bzip2 performance is asymmetric, as decompression is relatively fast. Motivated by the long time required for compression, a modified version was created in 2003 called pbzip2 that used multi-threading to encode the file in multiple chunks, giving almost linear speedup on multi-CPU and multi-core computers. As of May 2010, this functionality has not been incorporated into the main project.
