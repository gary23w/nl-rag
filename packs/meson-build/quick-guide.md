...
source: https://mesonbuild.com/Quick-guide.html
domain: meson-build
...
**Description**: The default installation prefix for the python pip module installation might not be included in your shell environment `PATH`. The default install location varies depending on your OS, distro and Python version.

**Resolution**: This issue can be resolved by altering the default shell environment `PATH` to include the path where Pip installed the binaries. This can be, e.g. `/usr/local/bin` or `/home/<username>/.local/bin`.
