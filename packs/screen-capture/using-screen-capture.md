---
title: "Using the Screen Capture API - Web APIs"
...
You can write this code either using an asynchronous function and the `await` operator, as shown above, or using the `Promise` directly, as seen below.
...
You can then use the captured stream, `captureStream`, for anything that accepts a stream as input. The examples below show a few ways to make use of the stream.
...
The options object passed into `getDisplayMedia()` is used to set options for the resulting stream.
...
In this example, the contents of the captured screen area are streamed into a `<video>` element on the same page.
...
The object `displayMediaOptions` contains the options to pass into `getDisplayMedia()`; here, the `displaySurface` property is set to `window`, indicating that the whole window should be captured.
...
This example overrides certain `console` methods to output their messages to the `<pre>` block whose ID is `log`.
...
This allows us to use `console.log()` and `console.error()` to log information to the log box in the document.
...
4. A `<pre>` block into which logged text is placed by the intercepted `console` method.
