---
title: "Introduction to events - Learn web development"
...
|---|---|
| Learning outcomes: | What events are — a signal fired by the browser when something significant happens, which the developer can run some code in response to. Setting up event handlers using `addEventListener()` (and `removeEventListener()`) and event handler properties. Inline event handler attributes, and why you shouldn't use them. Event objects. |
