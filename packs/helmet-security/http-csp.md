...
1. Control which resources are loaded, to protect against XSS.
2. Restrict embedding, to protect against clickjacking.
3. Upgrade insecure requests, to help ensure that all resources are served over HTTPS.
4. Require the use of trusted types, to help defend against client-side XSS.
...
**Note:** A CSP can actually help protect against XSS in two different ways:

- It can help ensure that input is sanitized before being used in the client: we discuss this later on in Requiring trusted types.
...
- A `<script>` tag that links to a malicious source: `<script src="https://evil.example.com/hacker.js"></script>`
- A `<script>` tag that includes inline JavaScript: `<script> console.log("You've been hacked!"); </script>`
- An inline event handler: <img onmouseover="console.log(`You've been hacked!`)" src="thumbnail.jpg" alt="" />
- A `javascript:` URL: <iframe src="javascript:console.log(`You've been hacked!`)"></iframe>
- A string argument to an unsafe API like `eval()`: eval("console.log(`You've been hacked!`)");
...
- JavaScript included inside a `<script>` element in the page: `<script> console.log("Hello from an inline script"); </script>`
- JavaScript in an inline event handler attribute: `<img src="x" onerror="console.log('Hello from an inline event handler')" />`
- JavaScript in a `javascript:` URL: `<a href="javascript:console.log('Hello from a javascript: URL')">Click me</a>`
...
- `eval()` itself: `eval('console.log("hello from eval()")');`
- The `Function()` constructor: `const sum = new Function("a", "b", "return a + b");`
- The string argument to `setTimeout()` and `setInterval()`: `setTimeout("console.log('hello from setTimeout')", 1);`
...
Some APIs in the web platform are known as *injection sinks*. These are APIs that can be passed some input, usually in the form of a string, and that can interpret that input as code. In this guide we've already seen `eval()`, but there are many other injection sinks, such as `Element.innerHTML` or `Document.write()`.
...
When a CSP violation occurs, the browser sends the report as a JSON object to the specified endpoint via an HTTP `POST` operation, with a `Content-Type` of `application/reports+json`. The report is a serialized form of the `CSPViolationReport` object containing a `type` property with a value of `"csp-violation"`.
